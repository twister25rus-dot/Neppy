; NSIS installer hooks for Neppy (issue #4395, follow-up to #3605).
;
; Part 2 of #4395 — installer pre-install process kill. The default tauri NSIS
; template only stops the *main* binary (Neppy.exe) via CheckIfAppIsRunning
; before overwriting it. A leftover / wedged `neppy-core.exe` from the prior
...
; this is the correct place to guarantee no Neppy process is holding the
...
!macro NSIS_HOOK_PREINSTALL
  DetailPrint "Stopping any running Neppy helper processes before install..."
  ; Force-kill the embedded core / CLI / MCP process and its child tree.
  nsExec::Exec 'taskkill /F /IM neppy-core.exe /T'
  Pop $0
  ; Belt-and-suspenders: the main binary and its CEF child processes. The
  ; template's CheckIfAppIsRunning also covers Neppy.exe, but in a silent
