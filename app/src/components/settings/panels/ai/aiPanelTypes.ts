...
export function defaultEndpointFor(slug: string): string {
  ...
  switch (slug) {
    case 'openhuman':
      // De-vendored build: upstream returned https://api.openhuman.ai/v1.
      // Points at the local backend's OpenAI-compatible path instead.
      return 'http://127.0.0.1:8788/openai/v1';
