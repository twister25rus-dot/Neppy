...
// De-vendored build: upstream defaulted to https://api.tinyhumans.ai here.
// Both arms now resolve to loopback so a build with no VITE_BACKEND_URL cannot
// reach the vendor's servers. Mirrors DEFAULT_API_BASE_URL in src/api/config.rs.
const DEFAULT_BACKEND_URL =
  APP_ENV === 'staging' ? 'http://127.0.0.1:8788' : 'http://127.0.0.1:8788';
...
// De-vendored build: upstream baked in the vendor's OpenPanel project id, so
// analytics shipped to panel.tinyhumans.ai by default. Blank disables OpenPanel
// entirely (see the guard in services/analytics); set VITE_OPENPANEL_CLIENT_ID
// to opt back in against your own instance.
