...
Commands assume **repo root**. Root `package.json` is `neppy-repo` (private, pnpm-enforced).
...
- **Rust config**: TOML `Config` struct (`src/neppy/config/schema/types.rs`) with env overrides (`load.rs`).

### Agent access & security

The `[autonomy]` block (`src/neppy/config/schema/autonomy.rs`) drives `SecurityPolicy` (`src/neppy/security/policy.rs`). Tiers: `readonly` / `supervised` / `full` × `workspace_only` × `trusted_roots` × `allow_tool_install`. Edit via `config.update_autonomy_settings` RPC or Settings → Agent access.

**Two path roots** (`src/neppy/config/schema/types.rs`):
...
**AI config**: bundled prompts in `src/neppy/agent/prompts/` ship via `tauri.conf.json` resources and are read core-side (`app/src/lib/ai/` holds agent-context helpers, not prompt loaders).
...
| --- | --- | --- |
| `tinydocs-bus` | the `.docx` / `.pptx` spec types, their size limits and validation | the artifact pipeline, the `spawn_blocking` hop, and the generation deadline — `src/neppy/tools/impl/document/` |
| `tinywallet-bus` | the TinyWallet wire contract and bus member names, the BTC / EVM / Solana / Tron address formats, the EIP-712 and ERC-20 encoders, and the Tron verification codec | RPC endpoint resolution, transaction assembly and broadcast, key custody — `src/neppy/web3/` |
...
**`neppy::tinyplace/` was NOT deleted, and this is a deliberate stop, not an
...
**Skills runtime**: the QuickJS per-skill VM engine is gone. `src/neppy/skills/` holds skill metadata/tool descriptors; execution of installed `SKILL.md` workflows lives in `src/neppy/skills/runtime/` (starts/cancels runs, hosts the `skill_executor` agent, reuses `runtime::node`/`runtime::python`, which are clients for the `tinyruntime` module).

### Tool calling lives in tinyagents — `src/neppy/agent/dispatcher.rs` is a seam
...
- **Tool ownership**: domain tools live in that domain's `tools.rs`, re-exported via `src/neppy/tools/mod.rs`. Only cross-cutting families stay in `tools/impl/`.
...
- **`ToolGroups`** selects how each *tool group* reaches the model, one mode per compiled-in pack in `tools/toolpacks/registry.rs` (`src/neppy/tools/toolpacks/groups.rs`). Presets: `packed()` (default — every group withheld, byte-identical to before the type existed), `advertised()`, `none()`, plus `.with(id, mode)`. Also on `Harness::builder()`.
...
| ------- | ------- | ------- | ----- | ---------- |
| `voice` | OFF | ON | the `neppy::voice` family (incl. `voice::audio_toolkit`) — STT/TTS providers, dictation server, always-on listening, podcast audio + email | `hound`, `lettre` |
...
| `web3` | OFF | ON | the `neppy::web3` family (`web3`, `web3::wallet`, `web3::x402`) — crypto wallet (multi-chain sign/broadcast), swaps/bridges/dapp calls, x402 machine payments | `bitcoin`, `curve25519-dalek` |
| `media` | ON | ON | `neppy::media::generation` (the `media_generate_*` agent tools) + `neppy::media::image` scaffold | none (surface-only) |
...
- **The family root `pub mod mcp;` is UNGATED.** It cannot carry `#[cfg(feature = "mcp")]` for two independent reasons: `mcp::http_client` is always compiled (below), and the three facades each ship a `stub.rs` that must resolve in an `mcp`-less build. The gate is pushed down onto each member in `src/neppy/mcp/mod.rs` — the rule a family root with a stub or an ungated member must follow. `mcp::config_servers` is leaf-gated there; `mcp::http_client` is not gated at all.
...
### Loadable native modules — `src/neppy/modules/`
...
before touching `src/neppy/memory/`.

**The contract is `tinymemory-api`, and `crate::neppy::memory::api` is a
re-export of it — not a copy.** `3ee5a3cad` inlined that crate as 10,894 lines
under `src/neppy/memory/api/`, every file byte-identical to
...
- **Leaf-gated call sites** (each carries its own `#[cfg]`): the 5 controller-registration pushes in `src/core/all.rs` (channels controllers, `webview_apis`, `webview_notifications`, public + internal `whatsapp_data`), the `ChannelInboundSubscriber` + web-only-proactive block in `src/core/jsonrpc.rs`, `spawn_channels_service` in `src/core/runtime/services.rs`, the `whatsapp_data::global::init` block in `src/core/runtime/context.rs`, and the `whatsapp_data::tools::*` glob + 3 `WhatsAppData*Tool` registrations in `src/neppy/tools/{mod,ops}.rs`. The `whatsapp_data` `pub mod` declaration now lives in `channels/mod.rs` (still `#[cfg(feature = "channels")]`, because the parent stays ungated for the `traits`/`cli` carve-outs); `webview_apis` / `webview_notifications` moved under `desktop/` in the family reorg and stay leaf-gated there. String-match arms (`"channels" =>` descriptions, `whatsapp_data_` in `group_for_namespace`) stay **ungated** — they are data.
...
- **Both-ways tests.** `channels_controllers_{registered_when_feature_on,absent_when_feature_off}` in `src/core/all_tests.rs` pin the controller surface (the OFF half also asserts `channel`/web_chat survives), and `whatsapp_data_tools_{present_when_channels_on,absent_when_channels_off}` in `src/neppy/tools/ops_tests.rs` pin the 3 agent tools (that module has the full-tool-list machinery). CI's smoke lane runs `cargo check` only, so run `cargo test --lib --no-default-features core::all::tests` locally after touching any gated surface.
...
Update `src/neppy/platform/about_app/` when adding/removing/renaming user-facing features. Define E2E scenarios up front covering happy paths, failures, auth gates.
