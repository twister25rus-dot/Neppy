...
/// Local-only backend root. Used as the final fallback when no override is
/// configured.
///
/// **De-vendored build.** Upstream shipped `https://api.tinyhumans.ai` here, so
/// a missing `BACKEND_URL` silently sent auth, billing, integration, voice and
/// orchestration traffic to the vendor's servers. This build defaults to a
/// loopback address instead: with no configuration the control plane fails
/// fast with a connection error rather than dialing a third party. Point
/// `BACKEND_URL` at a real local backend to change it.
...
pub fn effective_backend_api_url(api_url: &Option<String>) -> String {
    if let Some(u) = non_empty_str(api_url) {
        ...
        let is_cloud_inference = crate::neppy::config::schema::cloud_providers::endpoint_host(u)
            .is_some_and(|h| {
                crate::neppy::config::schema::cloud_providers::host_is_builtin_cloud_provider(&h)
            });
