...
| `src/openhuman/config/migrations/unify_ai_provider_settings.rs` | **1→2.** Consolidates scattered AI-provider settings into per-workload provider strings; seeds `cloud_providers` (always an `Neppy` entry) and migrates a non-backend `inference_url` into a `Custom` cloud entry. Pure in-memory. |
