//! Thin Neppy host adapters for [`tinyagents::graph::goals`].
//!
...
//! contains only Neppy-specific concerns: workspace-store resolution,
//! JSON-RPC schemas, domain events, the legacy file migration, heartbeat
//! dispatch, and adapters for Neppy's pre-tinyagents `Tool`/`StopHook`
