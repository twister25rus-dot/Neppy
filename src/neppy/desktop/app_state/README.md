...
- `src/neppy/config/` — `config_rpc::*` for `Config` reads and the workspace dir resolver.
- `src/neppy/autocomplete/` — `AutocompleteStatus` snapshot.
- `src/neppy/inference/local/` — `LocalAiStatus` snapshot.
- `src/neppy/platform/service/` — `ServiceState` / `ServiceStatus` runtime info.
- `src/neppy/security/credentials/` — `session_support::build_session_state` for the auth slice.
- `src/api/{config,jwt}` — backend base URL + bearer token used by the cached current-user fetch.

## Called by

- `src/neppy/agent/harness/session/builder.rs` — agent builder reads cached app state when resolving identity.
