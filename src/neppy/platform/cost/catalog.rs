...
//!
//! - pre-fill [`crate::neppy::config::schema::ModelRegistryEntry`] rows so
//!   the Model Health dashboard shows real numbers instead of zeros, and
//! - power the fallback estimate in
//!   [`crate::neppy::agent::cost::lookup_pricing`] when a backend doesn't
...
/// [`crate::neppy::inference::model_context::context_window_for_model`] (which
...
pub fn unified_model_catalog(
    ...
    for entry in models.iter_mut() {
        if entry.max_input_tokens.is_none() {
            if let Some(window) =
                crate::neppy::inference::model_context::context_window_for_model(&entry.model_id)
            {
    ...
    sources.push(tinyagents::registry::ModelCatalogSource {
        ...
        url: "repo:src/neppy/platform/cost/catalog.rs".to_string(),
