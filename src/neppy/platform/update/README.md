...
| File | Role |
| --- | --- |
| `src/neppy/platform/update/mod.rs` | Export-focused. `pub use core::*`, `pub use ops as rpc`, `pub use types::*`, and the `all_update_controller_schemas` / `all_update_registered_controllers` re-exports. |
| `src/neppy/platform/update/core.rs` | Core logic: `current_version`, `platform_triple`, `check_available` (GitHub fetch + parse), `download_and_stage[_with_version]` (atomic staging), asset selection, semver compare, transport-failure classifier. |
| `src/neppy/platform/update/ops.rs` | RPC handlers (`update_version`/`update_check`/`update_apply`/`update_run`), mutation-policy enforcement, URL/asset-name validation, and `UpdateRunResult` builders per restart strategy. Aliased as `update::rpc`. |
| `src/neppy/platform/update/scheduler.rs` | `run(UpdateConfig)` periodic checker loop + `tick()`; publishes startup/health events. Floor `MIN_INTERVAL_MINUTES = 10`. |
| `src/neppy/platform/update/schemas.rs` | Controller registry: `all_controller_schemas`, `all_registered_controllers`, `schemas(fn)`, and `handle_*` thunks delegating to `ops`. |
| `src/neppy/platform/update/types.rs` | Serde types: `UpdateInfo`, `VersionInfo`, `UpdateRunResult`, `UpdateApplyResult`, `GitHubRelease`, `GitHubAsset`. |
| `src/neppy/platform/update/ops_tests.rs` | Sibling test suite for `ops.rs` (via `#[path]`). |
...
## Agent tools
Not owned here — the domain has no `tools.rs`. Two cross-cutting system tools wrap it: `src/neppy/tools/impl/system/update_check.rs` (read-only, calls `update::rpc::update_check`) and `src/neppy/tools/impl/system/update_apply.rs` (calls `update::rpc::update_run`).
...
## Dependencies
- `crate::neppy::config` — reads `UpdateConfig` (`enabled`, `interval_minutes`, `rpc_mutations_enabled`, `restart_strategy`) and `UpdateRestartStrategy`; `ops` loads it via `config::rpc::load_config_with_timeout`.
- `crate::neppy::platform::service` — `service::rpc::service_restart` to publish the self-restart for `SelfReplace`.
- `crate::neppy::platform::health` — `health::bus::register_health_subscriber` in the scheduler.
- `crate::neppy::util` — `utf8_safe_prefix_at_byte_boundary` for safe error-body truncation.
...
- `src/neppy/tools/impl/system/update_check.rs` and `update_apply.rs` — agent tools wrapping the RPC layer.
