...
use super::*;
use crate::neppy::config::UpdateRestartStrategy;
use crate::neppy::platform::update::types::{UpdateApplyResult, UpdateInfo};
...
async fn update_version_returns_cargo_pkg_version() {
    ...
    assert_eq!(v, crate::neppy::platform::update::current_version());
}
...
async fn update_version_returns_target_triple() {
    ...
    assert_eq!(triple, crate::neppy::platform::update::platform_triple());
}

#[tokio::test]
async fn update_version_asset_prefix_contains_target_triple() {
    let outcome = update_version().await;
    let triple = crate::neppy::platform::update::platform_triple();
    let prefix = outcome
        .value
        .get("asset_prefix")
        .and_then(|v| v.as_str())
...
async fn update_run_rejected_when_rpc_mutations_disabled() {
    ...
    let mut cfg = crate::neppy::config::Config {
        workspace_dir: tmp.path().join("workspace"),
        action_dir: tmp.path().join("workspace"),
        config_path: tmp.path().join("config.toml"),
        ..crate::neppy::config::Config::default()
    };
    cfg.update = crate::neppy::config::UpdateConfig {
