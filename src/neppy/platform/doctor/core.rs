...
fn doctor_model_targets() -> Vec<String> {
    crate::neppy::inference::provider::list_providers()
        .into_iter()
...
fn check_daemon_state(config: &Config, items: &mut Vec<DiagnosticItem>) {
    let cat = "daemon";
    let state_file = crate::neppy::platform::service::daemon::state_file_path(config);
...
fn check_embedding_model_health(config: &Config, items: &mut Vec<DiagnosticItem>) {
    ...
    let (provider, model, _dims) =
        crate::neppy::inference::embeddings::effective_embedding_settings(
            &config.memory,
