...
| File | Role |
| --- | --- |
| `src/neppy/platform/socket/mod.rs` | Module docstring + exports only. Re-exports `SocketManager`, `global_socket_manager`, `set_global_socket_manager`, and the `all_socket_controller_schemas` / `all_socket_registered_controllers` pair. |
| `src/neppy/platform/socket/manager.rs` | `SocketManager` handle + `SharedState`; global `OnceLock` accessor; `connect` / `connect_with_provider` / `disconnect` / `emit` / `emit_with_ack` / `get_state`; spawns the background `ws_loop`. Holds emit/shutdown channels, ACK waiters, and the loop join handle. |
| `src/neppy/platform/socket/ws_loop.rs` | The background reconnection loop and a single connection attempt: Engine.IO/Socket.IO handshake, Socket.IO ACK packet dispatch, redirect-following connect, ping-timeout deadline, backoff, invalid-token decision logic, failure-escalation logging. |
| `src/neppy/platform/socket/event_handlers.rs` | Inbound SIO event dispatch (`handle_sio_event`), SIO frame parsing (`parse_sio_event`), outbound frame helper (`emit_via_channel`). Maps event names → `DomainEvent` publishes. Redacts payload content from logs. |
| `src/neppy/platform/socket/token_provider.rs` | `TokenProvider` type alias + `static_token_provider`, `token_provider_from_config`, and `is_invalid_token_error` (strict double-anchor matcher). |
| `src/neppy/platform/socket/schemas.rs` | Controller schemas + RPC handlers for the `socket` namespace. |
| `src/neppy/platform/socket/types.rs` | `WsStream` alias, `ConnectionOutcome` enum, observability event-name constants; re-exports `ConnectionStatus` / `SocketState` from `crate::api::models::socket`. |
| `src/neppy/platform/socket/ws_loop_tests.rs` | Out-of-line test suite for `ws_loop.rs` (via `#[path = ...]`). |
...
- `crate::neppy::skills::webhooks` — `WebhookRouter` (attached for parse-error logging / response emission) and `WebhookRequest`.
- `crate::neppy::integrations::composio` — `ComposioTriggerEvent` DTO for `composio:trigger` deserialization.
- `crate::neppy::security::devices::tunnel_client` — `TunnelPeerStatus`, `TunnelFrame` DTOs for tunnel events.
- `crate::neppy::config` — `Config` + `rpc::load_config_with_timeout` for `connect_with_session`.
- `crate::neppy::util::utf8_safe_prefix_at_byte_boundary` — UTF-8-safe log truncation of raw packets.
...
- `src/neppy/platform/connectivity/rpc.rs` — connectivity/status surfacing.
- `src/neppy/skills/webhooks/{ops.rs,bus.rs}` — emit webhook responses back through the global manager.
- `src/neppy/security/devices/tunnel_client.rs` — emits tunnel frames/registration over the socket.
