...
fn usage_to_json(
    usage: crate::neppy::agent::harness::turn_subagent_usage::LastTurnUsage,
) -> serde_json::Value {
    serde_json::json!({
...
pub fn emit_register_agents() {
    let agents: Vec<AgentDescriptor> = crate::neppy::agent::registry::default_agents()
        .into_iter()
...
async fn describe_self(
    ...
    if let Some(entry) = crate::neppy::agent::registry::default_agents()
        .into_iter()
...
fn emit<T: serde::Serialize>(event: &str, payload: T) {
    ...
    let Some(mgr) = crate::neppy::platform::socket::global_socket_manager() else {
...
async fn emit_awaited<T: serde::Serialize>(event: &str, payload: T) -> bool {
    ...
    let Some(manager) = crate::neppy::platform::socket::global_socket_manager() else {
        log::debug!("[medulla] no socket manager — dropping {event}");
        return false;
    };
