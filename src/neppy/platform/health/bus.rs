    ...
    async fn handle(&self, event: &DomainEvent) {
        match event {
            DomainEvent::SystemStartup { component } => {
                crate::neppy::platform::health::mark_component_ok(component);
            }
            ...
            DomainEvent::HealthRestarted { component } => {
                crate::neppy::platform::health::bump_component_restart(component);
            }
            DomainEvent::ChannelConnected { channel } => {
                crate::neppy::platform::health::mark_component_ok(&format!("channel:{channel}"));
            }
