...
| File | Role |
| --- | --- |
| `src/neppy/platform/health/mod.rs` | Export-only: declares modules, re-exports `core::*`, `ops::*`, `pub use ops as rpc`, and the controller-schema pair (`all_health_controller_schemas` / `all_health_registered_controllers`). |
| `src/neppy/platform/health/core.rs` | The registry itself: `ComponentHealth`/`HealthSnapshot` types, the `OnceLock<HealthRegistry>` singleton (`started_at` + `Mutex<BTreeMap>`), and the mutators/`snapshot`/`snapshot_json` functions. |
| `src/neppy/platform/health/ops.rs` | RPC handler logic returning `RpcOutcome<T>`: `health_snapshot()` and `system_info()` (+ `SystemInfo` type). |
| `src/neppy/platform/health/schemas.rs` | Controller schemas + `handle_snapshot`/`handle_system_info` async handlers that delegate to `ops` and serialize via `into_cli_compatible_json`. |
| `src/neppy/platform/health/bus.rs` | `HealthSubscriber` (`EventHandler`) and `register_health_subscriber()`; maps domain events to registry mutations. |
...
- `src/neppy/channels/runtime/{startup,supervision}.rs` and `src/neppy/channels/tests/health.rs` — channel runtime updates component health.
- `src/neppy/cron/scheduler.rs`, `src/neppy/platform/update/scheduler.rs` — emit/consume health signals.
