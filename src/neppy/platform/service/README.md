...
| File | Role |
| --- | --- |
| `src/neppy/platform/service/mod.rs` | Export-focused module root; declares submodules and re-exports the public surface + controller-schema pair. |
| `src/neppy/platform/service/core.rs` | `ServiceState` / `ServiceStatus` types and the cross-platform `install`/`start`/`stop`/`status`/`uninstall` dispatchers that route to the mock or the per-OS impl via `cfg`. |
| `src/neppy/platform/service/ops.rs` | RPC handler layer (`service_install`, `service_start`, `service_stop`, `service_status`, `service_restart`, `service_shutdown`, `service_uninstall`, `daemon_host_get`/`set`) returning `RpcOutcome<T>`. Re-exported as `rpc`. |
| `src/neppy/platform/service/schemas.rs` | Controller schemas + `handle_*` adapters for the `service` namespace; `all_controller_schemas` / `all_registered_controllers`. |
| `src/neppy/platform/service/restart.rs` | Self-restart orchestration: `service_restart` (publishes event), `trigger_self_restart_now` (respawns current exe with original args), `apply_startup_restart_delay_from_env`, `RestartStatus`. |
| `src/neppy/platform/service/shutdown.rs` | Graceful-shutdown orchestration: `service_shutdown` (publishes event), `ShutdownStatus`. |
| `src/neppy/platform/service/bus.rs` | Event-bus subscribers `RestartSubscriber` / `ShutdownSubscriber` (filter domain `system`) and idempotent `register_*_subscriber` helpers; one-shot atomic gates so only the first request acts. |
| `src/neppy/platform/service/common.rs` | Shared OS helpers: service labels, `resolve_daemon_executable`, `daemon_program_args` (`["run"]`), `xml_escape`, command runners (`run_checked`/`run_capture`/`run_best_effort`/`run_check_silent`), Windows `CREATE_NO_WINDOW` suppression. |
| `src/neppy/platform/service/macos.rs` | LaunchAgent (`com.neppy.core.plist`) install/start/stop/status/uninstall via `launchctl`; migrates legacy labels. |
| `src/neppy/platform/service/linux.rs` | systemd user-unit install/lifecycle via `systemctl --user`. |
...
- `crate::neppy::config` — `Config` (paths, config dir) for every lifecycle/path operation; `config::rpc::load_config_with_timeout` in the schema handlers.
...
- `src/neppy/platform/doctor/core.rs` — reads `service::daemon::state_file_path`.
- `src/neppy/platform/update/ops.rs`, `src/neppy/config/ops.rs`, `src/neppy/desktop/app_state/ops.rs` — reference `neppy::platform::service` (status/lifecycle/restart paths).
