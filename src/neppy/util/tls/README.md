...
| File | Role |
| --- | --- |
| `src/neppy/util/tls/mod.rs` | Entire module. Module docstring (policy) + the single `tls_client_builder()` function. No `mod`/`pub mod` decls; no submodules. |
...
- External crate `reqwest` (and its `native-tls` / `rustls` feature backends) only. No `use crate::neppy::*` or `use crate::core::*` imports — this is a leaf utility module with zero internal dependencies.

## Used by

Every HTTP-client construction site that talks to external HTTPS endpoints, including:

- `src/neppy/config/schema/proxy.rs` — proxy-aware client builders (primary + fallback).
- `src/neppy/integrations/client.rs` and integration tools (`tools/searxng.rs`, `tools/brave.rs`, `tools/querit.rs`, `tools/seltz.rs`).
- `src/neppy/inference/provider/compatible.rs` — OpenAI-compatible inference provider clients.
- `src/neppy/integrations/composio/client.rs`, `src/neppy/desktop/app_state/ops.rs`.
- `src/api/rest.rs` (REST API client).

Registered in the domain tree via `pub mod tls;` in `src/neppy/mod.rs`.
