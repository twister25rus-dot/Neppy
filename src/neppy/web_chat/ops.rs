...
pub(crate) fn sentry_suppression_reason(detailed: &str) -> Option<&'static str> {
    if crate::neppy::agent::error::is_max_iterations_error(detailed) {
...
pub async fn start_chat(
    ...
    let mut message = if message.contains("[FILE:") || message.contains("[IMAGE:") {
        ...
        let (file_cfg, image_cfg) = match crate::neppy::config::rpc::load_config_with_timeout()
            .await
        {
            Ok(cfg) => {
                log::debug!(
            ...
            Err(err) => {
                ...
                (
                    crate::neppy::config::MultimodalFileConfig::default(),
                    crate::neppy::config::MultimodalConfig::default(),
                )
            }
        };
        let extracted =
            crate::neppy::agent::multimodal::inline_file_attachments(&message, &file_cfg).await;
        let processed =
            crate::neppy::agent::multimodal::stash_image_attachments(&extracted, &image_cfg).await;
        log::debug!(
    ...
    if let Some(gate) = crate::neppy::security::approval::ApprovalGate::try_global() {
        if let Some(request_id) = gate.pending_for_thread(&thread_id) {
            if let Some(decision) = crate::neppy::security::approval::parse_approval_reply(&message)
            {
    ...
    match crate::neppy::hooks::ops::prompt_submitted(
    ...
    .await
    {
        crate::neppy::hooks::PromptVerdict::Submit { additional_context } => {
        ...
        crate::neppy::hooks::PromptVerdict::Block(reason) => {
            log::info!(
    ...
    let parsed_mode = match queue_mode.as_deref() {
        Some("steer") => crate::neppy::agent::harness::run_queue::QueueMode::Steer,
        Some("followup") => crate::neppy::agent::harness::run_queue::QueueMode::Followup,
        Some("collect") => crate::neppy::agent::harness::run_queue::QueueMode::Collect,
        Some("parallel") => crate::neppy::agent::harness::run_queue::QueueMode::Parallel,
        _ => crate::neppy::agent::harness::run_queue::QueueMode::Interrupt,
    };
    ...
    if matches!(
        parsed_mode,
        crate::neppy::agent::harness::run_queue::QueueMode::Parallel
    ) {
        log::info!(
    ...
    if !matches!(
        parsed_mode,
        crate::neppy::agent::harness::run_queue::QueueMode::Interrupt
    ) {
        let in_flight = IN_FLIGHT.lock().await;
        if let Some(existing) = in_flight.get(&map_key) {
            let queued_msg = crate::neppy::agent::harness::run_queue::QueuedMessage {
                text: message.clone(),
                mode: parsed_mode,
    ...
    let turn_run_queue = crate::neppy::agent::harness::run_queue::RunQueue::new();
    ...
    let handle = tokio::spawn(async move {
        let approval_ctx = crate::neppy::security::approval::ApprovalChatContext {
            thread_id: thread_id_task.clone(),
            client_id: client_id_task.clone(),
        };
        let origin = crate::neppy::agent::turn_origin::AgentTurnOrigin::WebChat {
            thread_id: thread_id_task.clone(),
            client_id: client_id_task.clone(),
...
fn dispatch_followups(followups: Vec<crate::neppy::agent::harness::run_queue::QueuedMessage>) {
...
async fn spawn_parallel_turn(
    ...
    let run_queue = crate::neppy::agent::harness::run_queue::RunQueue::new();
...
pub async fn channel_web_cancel(
    ...
    let cancelled = if cancelled_request_id.is_some() {
        ...
        crate::neppy::agent::task_dispatcher::cancel_session_scoped(thread_id.trim(), request_id)
            .await
    };
