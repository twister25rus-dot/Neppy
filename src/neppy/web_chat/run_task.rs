...
use crate::neppy::threads::turn_state::TurnStateStore;
...
pub(crate) async fn run_chat_task(
    ...
    if let Ok(ref task_result) = result {
        ...
        if speak_reply && !trimmed_response.is_empty() {
            let opts = crate::neppy::voice::reply_speech::ReplySpeechOptions::default();
            match crate::neppy::voice::reply_speech::synthesize_reply(
                &config,
        ...
        if metadata.source.as_deref() == Some("ptt") {
            if let Some(session_id) = metadata.session_id {
                crate::neppy::voice::publish_ptt_transcript_committed(
                    thread_id.to_string(),
                    session_id,
