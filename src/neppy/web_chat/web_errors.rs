    ...
    while let Some(c) = chars.next() {
        match c {
            '"' => {
                ...
                let sanitized = crate::neppy::inference::provider::sanitize_api_error(trimmed);
                return Some(crate::neppy::util::truncate_with_ellipsis(
                    &sanitized,
                    MAX_DETAIL_CHARS,
                ));
            }
...
/// error string. Mirrors the typed [`crate::neppy::inference::
...
/// `composio`, etc.) — see `src/neppy/security/
...
pub(crate) fn classify_inference_error(err: &str) -> ClassifiedError {
    ...
    } else if crate::neppy::agent::error::is_max_iterations_error(err) {
        ClassifiedError {
    ...
    } else if crate::neppy::inference::provider::is_chat_template_rejection_message(err) {
    ...
    } else if crate::neppy::inference::provider::is_provider_config_rejection_message(err) {
...
/// String-flat mirror of
/// [`crate::neppy::inference::provider::error_classify::is_non_retryable_rate_limit`].
///
