...
use crate::neppy::threads::turn_state::{TurnStateMirror, TurnStateStore};
...
fn session_profile_user_attribution(config: &crate::neppy::config::Config) -> Option<String> {
    let state =
        crate::neppy::security::credentials::session_support::build_session_state(config).ok()?;
    state
        .user
        .as_ref()
...
fn span_projection_signature(
    spans: &[crate::neppy::agent::progress_tracing::TraceSpan],
) -> Vec<String> {
    spans
        .iter()
        .map(|span| {
...
async fn shadow_compare_journal_projection(
    request_id: &str,
    trace_ctx: crate::neppy::agent::progress_tracing::TraceContext,
    max_iterations: u32,
    live_spans: &[crate::neppy::agent::progress_tracing::TraceSpan],
) -> Option<Vec<tinyagents::harness::observability::AgentObservation>> {
    let Some(journal_run_id) =
        crate::neppy::agent::tinyagents::journal::take_request_journal_run(request_id)
    else {
        log::debug!(
    ...
    let observations = match crate::neppy::agent::tinyagents::journal::read_run_events(
        &journal_run_id,
        0,
    )
    .await
    ...
    let projected =
        crate::neppy::agent::progress_tracing::journal_projection::spans_from_observations(
            trace_ctx,
            max_iterations,
            &observations,
        );
...
pub(crate) fn spawn_progress_bridge(
    mut rx: tokio::sync::mpsc::Receiver<crate::neppy::agent::progress::AgentProgress>,
    client_id: String,
    thread_id: String,
    request_id: String,
    ...
    tokio::spawn(async move {
        ...
        {
            use crate::neppy::agent::progress_tracing::{
            ...
            let identity = crate::neppy::desktop::app_state::peek_cached_current_user_identity();
        ...
        if let Some(mut collector) = span_collector.take() {
            ...
            if let Some((trace_ctx, observations)) = journal_export {
                ...
                crate::neppy::agent::progress_tracing::export_run_trace_from_journal(
                    &config,
                    &trace_ctx,
                    &observations,
                ...
                crate::neppy::agent::progress_tracing::export_run_trace(&config, &live_spans).await;
            }
        }
...
mod tests {
    ...
    fn session_profile_attribution_prefers_email_from_stored_session() {
        ...
        let service = crate::neppy::security::credentials::AuthService::from_config(&config);
    ...
    async fn tool_call_completed_forwards_real_output_on_tool_result() {
        ...
        tx.send(
            crate::neppy::agent::progress::AgentProgress::ToolCallCompleted {
                call_id: "call-1".into(),
