...
//!
//! [`unbound_default_capabilities`]: crate::neppy::memory::binding::unbound_default_capabilities
...
use crate::neppy::config::schema::MemoryHooksConfig;
use crate::neppy::memory::source_scope::current_source_scope;
use crate::neppy::security::egress::emit_external_transfer;
use crate::neppy::security::egress::types::{DataKind, EgressDescriptor, EgressReason};
use crate::neppy::security::live_policy;
use crate::neppy::security::policy::ToolOperation;
...
impl GuardPolicy {
    ...
    /// [`crate::neppy::memory::source_scope`]'s empty-allowlist semantics
    /// exactly.
    ///
    ...
    pub fn redact_outbound<'a>(&self, content: &'a str) -> Cow<'a, str> {
        match self.class {
            ...
            DriverClass::External => {
                Cow::Owned(crate::neppy::memory::safety::sanitize_text(content).value)
            }
        }
    }
    ...
    pub fn redact_outbound_json(&self, value: serde_json::Value) -> serde_json::Value {
        match self.class {
            ...
            DriverClass::External => crate::neppy::memory::safety::sanitize_json(&value).value,
        }
    }
