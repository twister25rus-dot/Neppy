...
/// [`crate::neppy::memory::host`]. `origin` names the producer
...
pub(crate) fn publish_local_model_unavailable_user_error(origin: &str) {
    ...
    crate::neppy::web_chat::publish_web_channel_event(local_model_unavailable_user_error());
}
...
/// sinks — the in-process one in [`crate::neppy::memory::host`] and the
/// module bridge in `crate::neppy::modules::memory_host` — so the notice
...
pub(crate) fn publish_store_corrupt_user_error(origin: &str, quarantined_path: Option<&str>) {
    ...
    crate::neppy::web_chat::publish_web_channel_event(store_corrupt_quarantined_user_error());
}
