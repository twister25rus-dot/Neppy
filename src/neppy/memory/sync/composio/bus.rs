...
//!   [`crate::neppy::agent::triage::run_triage`] to produce a
//!   [`TriageDecision`] and then
//!   [`crate::neppy::agent::triage::apply_decision`] to act on it.
...
//!   the bus.
//!
//! [trigger_triage]: crate::neppy::agent::registry::agents
//!
//! ## Feature flag
//!
...
use crate::neppy::agent::triage::{
    apply_decision, remote_trigger_origin, run_triage, TriageOutcome, TriggerEnvelope,
};
use crate::neppy::agent::turn_origin::with_origin;
use crate::neppy::config::rpc as config_rpc;
use crate::neppy::integrations::composio::trigger_history;
...
async fn backend_composio_client(
    ...
    let live_config = crate::neppy::config::rpc::reload_config_from_paths(
        &config.config_path,
...
use crate::neppy::integrations::composio::ops;
...
async fn toolkit_is_memory_source_registrable(
    ...
    let Ok(binding) = crate::neppy::memory::binding::for_config(config) else {
        tracing::warn!(
            toolkit = %toolkit,
    ...
    async fn handle(&self, event: &DomainEvent) {
        ...
        tokio::spawn(async move {
            ...
            let (src_max_items, src_sync_depth_days) = {
                let registry_sources = crate::neppy::memory::sources::list_sources()
                    .await
                    .unwrap_or_default();
                registry_sources
                    .iter()
                    .find(|s| {
                        s.kind == crate::neppy::memory::sources::SourceKind::Composio
                            && s.connection_id.as_deref() == Some(connection_id.as_str())
                    })
                    .map(|s| (s.max_items, s.sync_depth_days))
                    .unwrap_or_else(|| {
                        crate::neppy::memory::sources::memory_sync_defaults_for_toolkit(
                            toolkit.as_str(),
                        )
                    })
            };
            ...
            match wait_for_connection_active(&backend_client, &connection_id).await {
                Ok(status) => {
                    ...
                    // credentials since.
                    let live_config = match crate::neppy::config::rpc::reload_config_from_paths(
                        &config.config_path,
                        &config.workspace_dir,
                    )
                    .await
            ...
            if !config.onboarding_completed {
                ...
                match crate::neppy::memory::binding::for_config(&config) {
                ...
                let host_config = match crate::neppy::config::rpc::reload_config_from_paths(
                    &config.config_path,
                    &config.workspace_dir,
                )
                .await
                ...
                let attempted = match crate::neppy::memory::binding::for_config(&host_config) {
                    Ok(binding) => match binding.provider().as_source_sync() {
                        Some(sync) => sync
            ...
            if let Err(e) = crate::neppy::memory::sources::upsert_composio_source(
                &toolkit,
                &connection_id,
                &label,
            )
