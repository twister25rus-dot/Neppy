...
//!   `pub(in crate::neppy::memory)` with a single in-family caller, so
...
//!   `pub(in crate::neppy::memory)`, so even a re-export under another name
...
const ALLOWED: &[(&str, &str, &str)] = &[
    ...
    (
        "src/neppy/memory/tree/retrieval/rpc.rs",
        "NullMemoryProvider::new(",
    ...
    (
        "src/neppy/agent/learning/startup.rs",
        "binding::for_workspace(",
    ...
    (
        "src/neppy/memory/guard/families.rs",
        ".get_document(",
    ...
    (
        ...
        ".profile_conn(",
        "sole in-family call; wraps the raw handle in ProfileStore. profile_conn is pub(in crate::neppy::memory), so the compiler — not this lint — is the primary enforcement",
    ),
