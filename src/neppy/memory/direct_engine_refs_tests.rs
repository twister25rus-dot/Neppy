...
//! through [`crate::neppy::memory::api::wire`]'s error table — the one
...
const ALLOWED: &[(&str, Verdict, &str)] = &[
    ...
    // ~24 engine names, so a file writing `crate::neppy::memory::UnifiedMemory`
    ...
    (
        "src/neppy/agent/harness/archivist/recap.rs",
        Verdict::NeedsWiderSeam,
    ...
    (
        "src/neppy/agent/harness/archivist/test_constructors.rs",
        Verdict::NeedsWiderSeam,
        "the engine-side chat provider seam (chat::ChatProvider); MemoryIngest has no provider-override door",
    ),
    (
        "src/neppy/agent/harness/archivist/tree_ingest.rs",
        Verdict::NeedsWiderSeam,
    ...
    (
        "src/neppy/agent/harness/archivist/types.rs",
        Verdict::NeedsWiderSeam,
        "the engine-side chat provider seam (chat::ChatProvider); MemoryIngest has no provider-override door",
    ),
    (
        "src/neppy/channels/tests/memory.rs",
        Verdict::NeedsWiderSeam,
    ...
    (
        "src/neppy/flows/memory_tools.rs",
        Verdict::NeedsWiderSeam,
        "holds or boots the in-process engine handle (store::UnifiedMemory); driver construction belongs to memory::binding and the seam has no door onto the live client",
    ),
    (
        "src/neppy/integrations/composio/ops/mod.rs",
        Verdict::NeedsWiderSeam,
        "holds or boots the in-process engine handle (store::MemoryClient); driver construction belongs to memory::binding and the seam has no door onto the live client",
    ),
    (
        "src/neppy/memory/read_rpc/mod.rs",
        Verdict::NeedsWiderSeam,
    ...
    (
        "src/neppy/memory/host_impls.rs",
        Verdict::HostSide,
...
fn direct_reference_scanner_is_not_vacuous() {
    let found = scan();
    assert!(
        found.contains("src/neppy/memory/mod.rs"),
