...
use crate::neppy::tinyplace::ops::{global_state, map_err};
use crate::neppy::tinyplace::payment::{
    ensure_backend_mint_matches, ensure_cluster_matches, fulfill_payment, PaymentContext,
};
use crate::neppy::util::redact::redact;
...
async fn wallet_usdc_balance(address: &str) -> (Option<Value>, String) {
    ...
    let mint = crate::neppy::web3::wallet::solana_cluster().usdc_mint();
    let endpoints = crate::neppy::web3::wallet::tinyplace_solana_rpc_endpoints();
    ...
    for rpc_url in &endpoints {
        ...
        let safe_url = crate::neppy::web3::wallet::redact_rpc_url(rpc_url);
...
pub(crate) fn handle_tinyplace_signal_provision(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let store = crate::neppy::tinyplace::signal_store::global_signal_store().await?;
        let client = global_state().client().await?;
...
pub(crate) fn handle_tinyplace_signal_send_message(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let store = crate::neppy::tinyplace::signal_store::global_signal_store_arc().await?;
        let client = global_state().client().await?;
        let signer = require_signer(client)?;
...
mod tests {
    ...
    // `crate::neppy::web3::wallet`, which the `web3` stub disables — so without
    ...
    async fn wallet_usdc_balance_reads_from_tinyplace_settlement_rpc() {
        let _lock = crate::neppy::config::TEST_ENV_LOCK
            .lock()
