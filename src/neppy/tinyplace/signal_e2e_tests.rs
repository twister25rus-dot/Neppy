...
use crate::neppy::tinyplace::signal_store::FileSessionStore;
...
async fn arc_test_store(
    ...
    Arc<crate::neppy::tinyplace::signal_store::FileSessionStore>,
    tempfile::TempDir,
) {
    let dir = tempfile::tempdir().unwrap();
    let secret_store = SecretStore::new(dir.path(), true);
    let identity = ed25519_seed_to_x25519_keypair(&seed);
    let store = crate::neppy::tinyplace::signal_store::FileSessionStore::new(
        identity,
...
async fn signal_session_cross_interop_low_level() {
    ...
    let alice_store_raw = crate::neppy::tinyplace::signal_store::FileSessionStore::new(
        alice_identity.clone(),
        alice_dir.path().join("signal"),
        alice_ss,
    )
    ...
    let bob_store_raw = crate::neppy::tinyplace::signal_store::FileSessionStore::new(
        bob_identity.clone(),
