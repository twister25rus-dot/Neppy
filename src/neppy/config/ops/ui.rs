...
pub async fn get_search_settings() -> Result<RpcOutcome<serde_json::Value>, String> {
    ...
    let result = serde_json::json!({
        ...
        "effective_engine": match config.search.effective_engine() {
            crate::neppy::config::SearchEngine::Disabled => "disabled",
            crate::neppy::config::SearchEngine::Managed => "managed",
            crate::neppy::config::SearchEngine::Parallel => "parallel",
            crate::neppy::config::SearchEngine::Brave => "brave",
            crate::neppy::config::SearchEngine::Querit => "querit",
            crate::neppy::config::SearchEngine::Exa => "exa",
        },
...
/// cron job via [`crate::neppy::cron::seed::seed_proactive_agents`].
pub async fn set_onboarding_completed(value: bool) -> Result<RpcOutcome<bool>, String> {
    ...
    if value && !was_completed {
        ...
        tokio::task::spawn_blocking(move || {
            if let Err(e) = crate::neppy::cron::seed::seed_proactive_agents(&seed_config) {
...
pub async fn load_and_apply_voice_server_settings(
        ...
        match mode.as_str() {
            "tap" => {
                config.voice_server.activation_mode =
                    crate::neppy::config::VoiceActivationMode::Tap;
            }
            "push" => {
                config.voice_server.activation_mode =
                    crate::neppy::config::VoiceActivationMode::Push;
            }
            _ => {
                return Err(format!(
    ...
    if let Some(engine) = update.stt_engine {
        ...
        let parsed = crate::neppy::config::SttEngine::parse(&engine).ok_or_else(|| {
