...
pub struct ModelSettingsPatch {
    ...
    pub model_routes: Option<Vec<crate::neppy::config::ModelRouteConfig>>,
    ...
    /// `auth-profiles.json` via [`crate::neppy::security::credentials::AuthService`]).
    /// Pass `Some(vec![])` to clear all third-party cloud providers.
    pub cloud_providers:
        Option<Vec<crate::neppy::config::schema::cloud_providers::CloudProviderCreds>>,
    ...
    pub model_registry: Option<Vec<crate::neppy::config::schema::ModelRegistryEntry>>,
...
pub struct MemorySettingsPatch {
    ...
    /// [`crate::neppy::config::schema::agent::MemoryContextWindow`]).
...
pub async fn apply_model_settings(
    ...
    if let Some(model) = update.default_model {
        ...
        if let Some(ref m) = config.default_model {
            if !crate::neppy::inference::provider::factory::is_known_neppy_tier(m) {
                log::warn!(
    ...
    if let Some(providers) = update.cloud_providers {
        use crate::neppy::config::schema::cloud_providers::is_slug_reserved;
    ...
    crate::neppy::memory::ops::maintenance::reembed_best_effort(config, "model settings").await;
    ...
    let requeued_note = if embedder_changed {
        match crate::neppy::memory::ops::maintenance::retry_failed(config).await {
            Ok(n) => n.to_string(),
...
pub async fn apply_memory_settings(
    ...
    if let Some(model) = update.embedding_model {
        ...
        if let Some(reason) =
            crate::neppy::inference::embeddings::non_embedding_model_reason(&model)
        {
    ...
    if let Some(window_label) = update.memory_window.as_deref() {
        if let Some(window) =
            crate::neppy::config::schema::MemoryContextWindow::from_str_opt(window_label)
        {
    ...
    crate::neppy::memory::ops::maintenance::reembed_best_effort(config, "memory settings").await;
...
pub async fn apply_local_ai_settings(
    ...
    if let Some(provider) = update.provider {
        config.local_ai.provider =
            crate::neppy::inference::local::provider::normalize_provider(&provider);
    }
    if let Some(base_url) = update.base_url {
        config.local_ai.base_url = match base_url {
            ...
            Some(base_url)
                if crate::neppy::inference::local::provider::normalize_provider(
                    &config.local_ai.provider,
                ) != "omlx"
                    && crate::neppy::inference::local::provider::provider_from_config(config)
                        == crate::neppy::inference::local::provider::LocalAiProvider::Ollama =>
            {
                Some(crate::neppy::inference::local::validate_ollama_url(
                    &base_url,
                )?)
            }
