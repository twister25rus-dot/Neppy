...
/// `parse_config_with_recovery` (see `src/neppy/config/schema/load.rs`)
...
fn seed_and_enrich_model_registry(config: &mut Config) {
    use crate::neppy::platform::cost::catalog;
...
pub(crate) async fn reset_local_data_for_paths(
    ...
    let active_user_marker = crate::neppy::config::active_user_marker_path(default_neppy_dir);
    tracing::debug!(
...
pub fn client_config_json(config: &Config) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "credits_bypass": {
            "chat": crate::neppy::inference::provider::factory::role_bypasses_managed_credits(
                "chat", config,
            ),
            "reasoning":
                crate::neppy::inference::provider::factory::role_bypasses_managed_credits(
                    "reasoning", config,
                ),
        },
...
pub fn agent_server_status() -> RpcOutcome<serde_json::Value> {
    let running = crate::neppy::platform::service::mock::mock_agent_running().unwrap_or(true);
...
pub async fn get_data_paths_for_user(
    ...
    let default_neppy_dir = default_neppy_dir();
    let current_neppy_dir = crate::neppy::config::user_neppy_dir(&default_neppy_dir, user_id);
    ...
    fn backfills_existing_entries_but_preserves_user_values_and_count() {
        let mut config = Config {
            model_registry: vec![
                ...
                crate::neppy::config::schema::ModelRegistryEntry {
