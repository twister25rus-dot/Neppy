...
async fn reset_local_data_removes_active_user_and_markers_only() {
    ...
    let user_marker = crate::neppy::config::active_user_marker_path(&default_neppy_dir);
...
async fn apply_memory_sync_settings_reset_to_default() {
    ...
    assert_eq!(
        ...
        crate::neppy::config::DEFAULT_MEMORY_SYNC_INTERVAL_SECS
    );
}
...
async fn apply_model_settings_requeues_failed_jobs_only_on_embedder_change() {
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
    let driver = crate::neppy::memory::binding::install_retrying_driver_for_test(&cfg, 1);
...
async fn apply_search_settings_accepts_disabled_engine() {
    ...
    assert_eq!(
        ...
        crate::neppy::config::SearchEngine::Disabled
    );
}
...
async fn apply_model_settings_replaces_model_routes_when_some_and_keeps_when_none() {
    ...
    use crate::neppy::config::ModelRouteConfig;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn apply_memory_settings_updates_all_provided_fields() {
    ...
    assert_eq!(
        ...
        Some(crate::neppy::config::schema::MemoryContextWindow::Extended)
    );
}
...
async fn apply_memory_settings_ignores_unknown_memory_window_label() {
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
    cfg.agent.memory_window = Some(crate::neppy::config::schema::MemoryContextWindow::Balanced);
...
async fn apply_memory_settings_round_trips_all_window_labels() {
    use crate::neppy::config::schema::MemoryContextWindow;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn autonomy_auto_approve_all_persists() {
    ...
    // too.
    let on_disk_cfg: crate::neppy::config::Config =
...
async fn apply_agent_settings_updates_timeout_and_persists_snapshot() {
    ...
    assert_eq!(
        crate::neppy::tools::timeout::tool_execution_timeout_secs(),
        300
    );
}
...
async fn apply_agent_paths_empty_input_clears_override() {
    ...
    assert_eq!(cfg.action_dir, crate::neppy::config::default_projects_dir());
    assert_eq!(
        outcome.value["action_dir_source"],
...
async fn ensure_agent_dirs_creates_missing_action_dir_and_trusted_root() {
    use crate::neppy::security::TrustedAccess;
...
fn ensure_usable_cwd_creates_missing_dir() {
    ...
    assert!(!dir.exists());
    crate::neppy::config::ensure_usable_cwd(&dir).expect("missing dir is created");
    assert!(dir.is_dir());
}
...
fn ensure_usable_cwd_rejects_a_file_with_descriptive_error() {
    ...
    let err = crate::neppy::config::ensure_usable_cwd(&file)
...
fn ensure_usable_cwd_errors_when_uncreatable() {
    ...
    let err = crate::neppy::config::ensure_usable_cwd(&target)
