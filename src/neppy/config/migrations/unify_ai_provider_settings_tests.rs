...
use super::*;
use crate::neppy::config::schema::{LocalAiConfig, LocalAiUsage};
...
fn make_legacy_config_local_on() -> Config {
    ...
    c.memory_tree.llm_backend = crate::neppy::config::schema::LlmBackend::Local;
    c
}

#[test]
fn empty_config_seeds_neppy_entry() {
    let mut c = Config::default();
...
#[test]
fn primary_cloud_defaults_to_neppy_id() {
    let mut c = Config::default();
    let stats = run(&mut c).expect("migration must succeed");
...
fn legacy_inference_url_becomes_custom_entry() {
    ...
    c.model_routes
        .push(crate::neppy::config::schema::ModelRouteConfig {
            hint: "reasoning".into(),
            model: "gpt-4o".into(),
        });
...
#[test]
fn neppy_inference_url_does_not_seed_custom() {
    let mut c = Config::default();
