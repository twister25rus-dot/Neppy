...
//!
//! So the route rides on [`Config`](crate::neppy::config::Config) — which
...
/// [`Config`](crate::neppy::config::Config) derives it for the whole struct.
...
pub struct EphemeralRoute {
    ...
    ///
    /// Read back only by
    /// [`lookup_key_for_slug`](crate::neppy::inference::provider::factory::lookup_key_for_slug)
...
/// [`provider_for_role`](crate::neppy::inference::provider::factory::provider_for_role)
...
pub fn apply(config: &mut crate::neppy::config::Config, route: EphemeralRoute) {
    let model = config
        .default_model
        .as_deref()
        .map(str::trim)
    ...
    let Some(model) = model else {
        log::warn!(
            ...
            crate::neppy::inference::provider::factory::redact_endpoint(&route.endpoint)
        );
        return;
    };
