...
fn decrypt_optional_secret(
    store: &crate::neppy::security::keyring::SecretStore,
    value: &mut Option<String>,
    field_name: &str,
) -> Result<()> {
    if let Some(raw) = value.clone() {
        if crate::neppy::security::keyring::SecretStore::is_encrypted(&raw) {
            ...
            match store.decrypt_and_migrate(&raw) {
                ...
                Err(e) => {
                    ...
                    crate::neppy::security::keyring_consent::policy::notify_decrypt_failure(
                        field_name,
                        &e.to_string(),
                    );
                    *value = None;
...
pub(super) fn decrypt_config_secrets(config: &mut Config, neppy_dir: &Path) -> Result<bool> {
    ...
    let store = crate::neppy::security::keyring::SecretStore::new(neppy_dir, true);
...
pub(super) fn encrypt_config_secrets(config: &mut Config) -> Result<()> {
    ...
    let store = crate::neppy::security::keyring::SecretStore::new(parent_dir, true);
