...
impl Config {
    ...
    pub(super) fn apply_env_overrides_from(
        ...
        crate::neppy::inference::embeddings::rate_limit::set_embedding_rate_limit(
    ...
    fn apply_runtime_env<E: super::env::EnvLookup + ?Sized>(&mut self, env: &E) {
        if let Some(tier_str) = env.get("OPENHUMAN_LOCAL_AI_TIER") {
            ...
            if !tier_str.is_empty() {
                if let Some(tier) =
                    crate::neppy::inference::presets::ModelTier::from_str_opt(&tier_str)
                {
    ...
    fn apply_learning_env<E: super::env::EnvLookup + ?Sized>(&mut self, env: &E) {
        ...
        if let Some(source) = env.get("OPENHUMAN_LEARNING_REFLECTION_SOURCE") {
            ...
            match normalized.as_str() {
                "local" => {
                    self.learning.reflection_source = crate::neppy::config::ReflectionSource::Local
                }
                "cloud" => {
                    self.learning.reflection_source = crate::neppy::config::ReflectionSource::Cloud
                }
                _ => {
                    tracing::warn!(
                        source = %source,
