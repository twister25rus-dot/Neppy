...
async fn read_config_with_recovery_or_default(config_path: &Path) -> Result<(String, bool)> {
    ...
    let result =
        crate::neppy::util::retry_with_backoff_async("read config file", 5, 20, reads).await;
    match result {
...
impl Config {
    ...
    pub(crate) async fn load_or_init_with_env_lookup(
        ...
        if config_path.exists() {
            ...
            crate::neppy::config::migrations::run_pending(&mut config).await;
            ...
            let mut config = Config {
                ...
                workspace_dir,
                action_dir: default_action_dir(),
                schema_version: crate::neppy::config::migrations::CURRENT_SCHEMA_VERSION,
                ..Default::default()
            };
            config.save().await?;
