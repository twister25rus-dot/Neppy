...
fn deserialize_params_parses_autonomy_update_with_trusted_roots() {
    ...
    assert_eq!(
        roots[0].access,
        crate::neppy::security::TrustedAccess::ReadWrite
    );
    // `access` defaults to Read when omitted.
    assert_eq!(roots[1].access, crate::neppy::security::TrustedAccess::Read);
}
...
async fn handle_get_autonomy_settings_returns_current_value() {
    ...
    let _ = crate::neppy::config::ops::load_and_apply_autonomy_settings(
        crate::neppy::config::ops::AutonomySettingsPatch {
