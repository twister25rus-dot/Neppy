...
- `src/neppy/security/encryption/` indirectly when secrets sections need at-rest crypto (read direction only).
...
- Hot consumers: `src/neppy/agent/` (model + autonomy), `src/neppy/channels/` (provider tokens), `src/neppy/memory/` (storage paths), `src/neppy/cron/` (scheduler poll), `src/neppy/inference/local/` (Ollama / device routing), `src/neppy/security/` (sandbox backend), `src/neppy/voice/`, `src/neppy/desktop/notifications/`, `src/neppy/tools/`, `src/neppy/security/encryption/`, `src/neppy/tree_summarizer/`, `src/neppy/hosted/referral/`.
