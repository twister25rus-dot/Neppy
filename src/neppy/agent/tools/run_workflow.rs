...
use crate::neppy::skills::runtime::{
...
fn outcome_to_result(
    ...
    outcome: Option<crate::neppy::skills::run_log::RunOutcome>,
) -> ToolResult {
    match outcome {
...
impl Tool for RunWorkflowTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(allow) = &self.skill_allowlist {
            ...
            let profile_local =
                crate::neppy::skills::profile_local_skill_ids(self.profile_skills_root.as_deref());
...
fn run_visible_to_profile(
    run: &crate::neppy::skills::run_log::ScannedRun,
...
impl Tool for AwaitWorkflowTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let profile_local_ids =
            crate::neppy::skills::profile_local_skill_ids(self.profile_skills_root.as_deref());
        let visible_run = crate::neppy::skills::run_log::scan_runs(&workspace, None, usize::MAX)
            .into_iter()
            .find(|run| {
                run.run_id == run_id
...
mod tests {
    ...
    fn detached_run_visibility_requires_profile_and_allowlist() {
        let run = crate::neppy::skills::run_log::ScannedRun {
