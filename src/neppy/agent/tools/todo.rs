...
//! one) via [`crate::neppy::threads::todos::ops`]; without a thread context the
//! tool falls back to a process-global scratch list. Returns a markdown
//! rendering so transcripts read cleanly.

use crate::neppy::agent::task_board::{TaskApprovalMode, TaskBoardCard, TaskCardStatus};
...
async fn default_task_approval_mode() -> Option<TaskApprovalMode> {
    ...
    // `task-sources`).
    match crate::neppy::config::ops::load_config_with_timeout().await {
...
fn current_location() -> BoardLocation {
    let Some(parent) = crate::neppy::agent::harness::fork_context::current_parent() else {
...
mod tests {
    ...
    fn scratch_lock() -> std::sync::MutexGuard<'static, ()> {
        crate::neppy::threads::todos::ops::scratch_test_lock()
    }

    async fn reset_scratch() {
        crate::neppy::threads::todos::ops::clear(&BoardLocation::Scratch)
            .await
