...
fn python_needed_eagerly(config: &Config) -> bool {
    config.runtime_python.enabled
        && !crate::neppy::runtime::python_server::enabled_backends(config).is_empty()
}
...
async fn runtime_python_server_is_done(config: &Config) -> bool {
    if crate::neppy::runtime::python_server::enabled_backends(config).is_empty() {
        return true;
    }
    let status = crate::neppy::runtime::python_server::status().await;
    status.running
}
...
async fn runtime_python_server_run(config: &Config) -> Result<(), String> {
    ...
    crate::neppy::runtime::python_server::ensure_started(config)
        .await
        .map(|_| {
...
async fn spacy_is_done(config: &Config) -> bool {
    ...
    crate::neppy::runtime::python_server::spacy_provisioned(config)
}
...
async fn spacy_run(config: &Config) -> Result<(), String> {
    ...
    crate::neppy::runtime::python_server::ensure_spacy(config)
        .await
        .map(|_| {
...
async fn kompress_is_done(config: &Config) -> bool {
    ...
    crate::neppy::runtime::python_server::kompress_provisioned(config)
}
...
async fn kompress_run(config: &Config) -> Result<(), String> {
    ...
    crate::neppy::runtime::python_server::ensure_kompress(config)
        .await
        .map(|_| {
...
#[cfg(feature = "runtime-node")]
fn build_node_bootstrap(config: &Config) -> crate::neppy::runtime::node::NodeBootstrap {
    crate::neppy::runtime::node::NodeBootstrap::new(std::sync::Arc::new(config.clone()))
}
