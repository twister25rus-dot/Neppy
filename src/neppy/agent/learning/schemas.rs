...
fn handle_rebuild_cache(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        use crate::neppy::agent::learning::stability_detector::StabilityDetector;
...
async fn get_cache() -> Result<crate::neppy::agent::learning::cache::FacetCache, String> {
    let guard = crate::neppy::memory::ops::guard::active_memory_guard()
        .await
        .map_err(|e| format!("memory unavailable: {e}"))?;
    Ok(crate::neppy::agent::learning::cache::FacetCache::new(guard))
}
...
fn handle_reset_cache(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let (deleted, pinned_preserved) =
            crate::neppy::agent::learning::cache::reset_non_pinned(&cache)
                .await
