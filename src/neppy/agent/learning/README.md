...
The module defines no `tools.rs`. However the `tool_effectiveness` stats it writes are surfaced by the cross-cutting `tool_stats` tool in `src/neppy/tools/impl/system/tool_stats.rs`, and `memory_tools` references learning namespaces.
...
- `crate::neppy::memory` / `memory_store` — the `Memory` trait, `MemoryClient`, categories; all KV persistence and the global memory client used by RPC handlers.
- `crate::neppy::agent::hooks` — `PostTurnHook` / `TurnContext` / `ToolCallRecord` implemented by the three hooks.
- `crate::neppy::agent::harness::session::transcript` — `SessionTranscript` parsing for transcript ingestion.
- `crate::neppy::inference::provider` / `inference::local` — LLM calls for reflection and profile summarisation.
- `crate::neppy::config` — `Config` / `LearningConfig` / `ReflectionSource` feature flags and `config::rpc` loader.
- `crate::neppy::agent::context::prompt` — `PromptContext` / `PromptSection` / `LearnedContextData` for prompt injection.
- `crate::neppy::integrations::composio` — `composio::client` (Gmail fetch for enrichment) and `composio::providers::profile_md` (`replace_managed_block` for `PROFILE.md`).
- `crate::neppy::integrations` — `build_client` / `IntegrationClient` for the Apify scrape call.
...
- `src/neppy/agent/harness/session/{builder,turn}.rs` and `agent_memory/memory_loader.rs` — wire the post-turn hooks, prompt sections, and learned-context loading into the agent loop.
- `src/neppy/channels/runtime/startup.rs` — likely registers schedulers/subscribers at startup.
- `src/neppy/memory/store/namespace_store/profile.rs`, `memory_sync/composio/providers/profile.rs`, `memory_tools/{capture,mod}.rs`, `tools/impl/system/tool_stats.rs`, `tools/schemas.rs` — consume facet/learning types.
