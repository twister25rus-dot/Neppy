...
use crate::neppy::agent::learning::candidate::{self, CueFamily, EvidenceRef, FacetClass};
use crate::neppy::config::{Config, LearningConfig, ReflectionSource};
...
impl ReflectionHook {
    ...
    async fn run_reflection(&self, prompt: &str) -> anyhow::Result<String> {
        match self.config.reflection_source {
            ReflectionSource::Local => {
                ...
                let service = crate::neppy::inference::local::global(&self.full_config);
                service
    ...
    fn emit_candidates_from_reflection(output: &ReflectionOutput, episodic_id: i64) {
        ...
        for (n, pref) in output.user_preferences.iter().enumerate() {
            ...
            buf.push(
                crate::neppy::agent::learning::candidate::LearningCandidate {
...
impl PostTurnHook for ReflectionHook {
    ...
    async fn on_turn_complete(&self, ctx: &TurnContext) -> anyhow::Result<()> {
        ...
        if self.config.enabled {
            ...
            let episodic_id = crate::neppy::agent::learning::candidate::global().len() as i64;
