...
use super::*;
use crate::neppy::agent::learning::candidate::FacetClass;
...
fn make_cache() -> FacetCache {
    crate::neppy::agent::learning::test_profile::in_memory_cache()
}
...
async fn reset_deletes_every_non_pinned_facet_and_keeps_the_pinned_ones() {
    let profile =
        std::sync::Arc::new(crate::neppy::agent::learning::test_profile::InMemoryProfile::new());
...
async fn a_failed_delete_is_reported_rather_than_counted_as_a_no_op() {
    ...
    let error = crate::neppy::agent::learning::cache::reset_non_pinned(&cache)
        .await
