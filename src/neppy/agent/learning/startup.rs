...
pub fn register_learning_subscribers(workspace_dir: std::path::PathBuf) {
    ...
    register_email_signature_once(&EMAIL_SIG_HANDLE, || {
        crate::neppy::agent::learning::extract::signature::register_email_signature_subscriber()
    });
...
) -> Option<crate::neppy::agent::learning::cache::FacetCache> {
    use crate::neppy::config::schema::MemorySubsystemConfig;
    match crate::neppy::memory::binding::for_workspace(
        workspace_dir,
        &MemorySubsystemConfig::default(),
    ) {
        Ok(binding) => Some(crate::neppy::agent::learning::cache::FacetCache::new(
            binding.guard(),
        )),
        Err(error) => {
...
fn register_with_memory(
    ...
    let rebuild_trigger = {
        use crate::neppy::agent::learning::scheduler::register_event_trigger;
        use crate::neppy::agent::learning::StabilityDetector;
        use std::sync::Arc;
        ...
        crate::neppy::agent::learning::scheduler::spawn_rebuild_loop(
            Arc::clone(&detector),
            crate::neppy::agent::learning::scheduler::DEFAULT_REBUILD_INTERVAL,
            shutdown_rx,
        );
    ...
    let profile_md = {
        use crate::neppy::agent::learning::ProfileMdRenderer;
        use std::sync::Arc;
        let Some(cache) = facet_cache_for(workspace_dir) else {
...
mod tests {
    ...
    use crate::neppy::agent::learning::candidate::Buffer;
    use crate::neppy::agent::learning::extract::signature::{
