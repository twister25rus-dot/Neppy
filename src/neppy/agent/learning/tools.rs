...
impl Tool for LearningSaveProfileTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let body = if summarize {
            crate::neppy::agent::learning::linkedin_enrichment::summarise_profile_with_llm(
                &config, &markdown,
            )
            .await
...
impl Tool for LearningEnrichProfileTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let result = crate::neppy::agent::learning::linkedin_enrichment::run_linkedin_enrichment(
            &config, preset,
        )
        .await
        .map_err(|e| anyhow::anyhow!("learning_enrich_profile: {e:#}"))?;
