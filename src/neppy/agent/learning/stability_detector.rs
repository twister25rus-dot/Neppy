...
use crate::neppy::agent::learning::cache::FacetCache;
use crate::neppy::agent::learning::candidate::{self, CueFamily, FacetClass, LearningCandidate};
...
impl StabilityDetector {
    ...
    pub async fn rebuild(&self, now: f64) -> anyhow::Result<RebuildOutcome> {
        ...
        for key in existing_by_key.keys() {
            if let Some(class) = crate::neppy::agent::learning::cache::class_from_key(key) {
        ...
        for ((class, full_key), cands) in &groups {
            ...
            let new_refs: Vec<crate::neppy::agent::learning::candidate::EvidenceRef> =
...
fn class_prefix(class: FacetClass) -> &'static str {
    crate::neppy::agent::learning::cache::class_prefix(class)
}
...
mod tests {
    use super::*;
    use crate::neppy::agent::learning::candidate::{
