...
//! `src/neppy/agent/tinyagents/mod.rs`) handles an empty registry by simply
...
use crate::neppy::config::MultimodalConfig;
use crate::neppy::cron::scheduler_gate::LlmPermit;
use crate::neppy::inference::provider::error_classify::{
...
async fn try_arm(
    ...
    let request = AgentTurnRequest {
        ...
        origin: crate::neppy::agent::turn_origin::AgentTurnOrigin::ExternalChannel {
...
///
/// Canonical sources:
/// - `src/neppy/agent/bus.rs` — `Blocked` / `ReviewBlocked` arms of the
///   `enforce_prompt_input` decision (the path the triage evaluator hits via
///   `agent.run_turn`).
/// - `src/neppy/agent/harness/session/runtime.rs` — same strings in the
///   tool-call loop, kept identical so this classifier covers both.
/// - `src/neppy/inference/local/ops.rs` — user-facing variants with the
...
fn truncate_payload(payload: &serde_json::Value, max_bytes: usize) -> String {
    ...
    let end = crate::neppy::util::floor_char_boundary(&pretty, max_bytes);
