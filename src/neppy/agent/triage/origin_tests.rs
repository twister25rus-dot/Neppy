...
use super::*;
use crate::neppy::agent::triage::envelope::TriggerEnvelope;
...
fn every_dispatch_site_scopes_an_origin() {
    const KNOWN_SITES: &[(&str, &str)] = &[
        ...
        ("src/neppy/memory/sync/composio/bus.rs", "remote"),
        ("src/neppy/integrations/task_sources/route.rs", "remote"),
        ("src/neppy/skills/webhooks/ops.rs", "remote"),
        ("src/neppy/skills/webhooks/bus.rs", "remote"),
        // Locally initiated — trust root.
        ("src/neppy/desktop/notifications/rpc.rs", "local"),
        ("src/neppy/agent/schemas.rs", "local"),
    ];
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            // being them.
            if rel.starts_with("src/neppy/agent/triage/") || rel.ends_with("_tests.rs") {
                continue;
            }
