...
use crate::neppy::agent::harness::subagent_runner::{self, SubagentRunOptions};
use crate::neppy::agent::orchestration::parent_context::build_root_parent;
    ...
    match run.decision.action {
        ...
        TriageAction::React | TriageAction::Escalate => {
            ...
            if let Some(link) = &envelope.card_link {
                use crate::neppy::agent::task_dispatcher::DispatchOutcome;
            ...
            let mut approval_gate_for_audit: Option<
                std::sync::Arc<crate::neppy::security::approval::ApprovalGate>,
            > = None;
            if let Some(gate) = crate::neppy::security::approval::ApprovalGate::try_global() {
                ...
                match outcome {
                    crate::neppy::security::approval::GateOutcome::Allow => {
                        approval_request_id = request_id;
                        if approval_request_id.is_some() {
                            approval_gate_for_audit = Some(gate);
                        }
                    }
                    crate::neppy::security::approval::GateOutcome::Deny { reason } => {
                        tracing::warn!(
                            action = %action_str,
                            target_agent = %target,
                            external_id = %envelope.external_id,
            ...
            if let (Some(gate), Some(req_id)) = (
                ...
                let (exec_outcome, err_text) = match &dispatch_result {
                    Ok(_) => (
                        crate::neppy::security::approval::ExecutionOutcome::Success,
                        None,
                    ),
                    Err(e) => (
                        crate::neppy::security::approval::ExecutionOutcome::Failure,
                        Some(e.to_string()),
                    ),
                };
...
async fn dispatch_linked_card(
    link: &TaskCardLink,
) -> Result<crate::neppy::agent::task_dispatcher::DispatchOutcome, String> {
    let snapshot = crate::neppy::threads::todos::ops::list(&link.location).await?;
    let card = snapshot
        .cards
        .into_iter()
        .find(|c| c.id == link.card_id)
        .ok_or_else(|| format!("card `{}` not found on board", link.card_id))?;
    crate::neppy::agent::task_dispatcher::dispatch_card(link.location.clone(), card).await
}
...
async fn gate_linked_card_terminal(envelope: &TriggerEnvelope, decision: &str) {
    use crate::neppy::agent::task_board::TaskCardStatus;
    use crate::neppy::threads::todos::ops;
...
mod tests {
    ...
    async fn seed_task_card() -> (
        tempfile::TempDir,
        crate::neppy::threads::todos::ops::BoardLocation,
        String,
    ) {
        use crate::neppy::threads::todos::ops::{self, BoardLocation, CardPatch};
