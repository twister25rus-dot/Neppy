...
//!
//! Relocated from `neppy::agent::profiles` / `::personality_paths` so the
//! domain is addressable on its own (`neppy::agent::profiles`).
//!
...
//! (see [`crate::neppy::config`]): core-managed identity/memory files live
