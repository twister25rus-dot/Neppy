...
/// [`SecurityPolicy::validate_path`](crate::neppy::security) at the file-tool
...
fn scan_command_segment(
    ...
    for raw in command.split(|c: char| {
        ...
        let expanded = crate::neppy::config::expand_tilde(token);
...
fn simple_cd_target(segment: &str, cwd: &Path) -> Option<PathBuf> {
    ...
    let expanded = crate::neppy::config::expand_tilde(target);
