...
use crate::neppy::integrations::composio::ComposioActionTool;
use crate::neppy::tools::{Tool, ToolCategory};
...
fn tool_specs_of<T: std::ops::Deref<Target = dyn crate::neppy::tools::Tool>>(
    tools: &[T],
...
async fn render_integrations_agent(config: &Config, toolkit: &str) -> Result<DumpedPrompt> {
    ...
    let base_tools: Vec<Box<dyn Tool>> = match &definition_snapshot.tools {
        crate::neppy::agent::harness::definition::ToolScope::Named(names) => {
        ...
        crate::neppy::agent::harness::definition::ToolScope::Wildcard => agent
            .tools()
            .iter()
            .map(|t| clone_tool_as_prompt_proxy(t.as_ref()))
            .collect(),
    ...
    if !rendered_tools.is_empty() {
        text.push_str("\n\n");
        text.push_str(
            &crate::neppy::agent::harness::subagent_runner::build_text_mode_tool_instructions(),
        );
    }
...
impl Tool for PromptProxyTool {
    ...
    fn permission_level(&self) -> crate::neppy::tools::PermissionLevel {
        crate::neppy::tools::PermissionLevel::None
    }
    async fn execute(
        &self,
