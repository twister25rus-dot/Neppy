...
async fn try_deliver(session: String) {
    ...
    if let Some(batch) = plan_delivery(&session) {
        ...
        if let (Some(thread_id), Some(notice)) = (
            ...
            if let Err(e) =
                crate::neppy::agent::task_dispatcher::run_system_turn_on_thread(thread_id, notice)
                    .await
            {
                log::warn!(
...
mod tests {
    use super::*;
    use crate::neppy::agent::orchestration::background_completions::record_completion;
