...
use crate::neppy::tools::traits::{Tool as _, ToolCallOptions, ToolResult};
...
pub(crate) async fn dispatch_subagent(
    ...
    let prompt: &str = {
        let images =
            crate::neppy::agent::harness::turn_attachments_context::current_turn_image_placeholders(
            );
        ...
        if !images.is_empty()
            && crate::neppy::inference::provider::factory::oh_tier_supports_vision(&subagent_model)
        {
            log::info!(
...
fn awaiting_outcome_to_tool_result(
    outcome: &crate::neppy::agent::harness::subagent_runner::SubagentRunOutcome,
    question: &str,
) -> ToolResult {
...
mod tests {
    ...
    use crate::neppy::agent::tools::AskClarificationTool;
