...
use crate::neppy::agent::tinyagents::thread_context::current_thread_id;
...
/// [`crate::neppy::agent::harness::fork_context::PARENT_CONTEXT`]
...
/// [`with_root_parent`](crate::neppy::agent::orchestration::parent_context::with_root_parent).
...
///   matched by [`crate::neppy::inference::provider::is_budget_exhausted_message`];
...
fn is_expected_billing_failure(message: &str) -> bool {
    crate::neppy::inference::provider::is_budget_exhausted_message(message)
            ...
            SubagentRunStatus::Completed => {
                ...
                if let (Some(parent), Some(thread_id)) = (current_parent(), current_thread_id()) {
                    if let Some(objective) = AgentPrepareContextTool::parse_proposed_goal(&bundle) {
                        match crate::neppy::threads::goals::store::set_if_absent(
...
mod tests {
    ...
    async fn execute_short_circuits_when_context_already_prepared() {
        let tool = AgentPrepareContextTool::new();
        let result = crate::neppy::agent::harness::with_agent_context_prepared_sources(
