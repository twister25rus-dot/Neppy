...
use crate::neppy::agent::harness::fork_context::{current_parent, with_parent_context};
use crate::neppy::agent::harness::run_queue::RunQueue;
use crate::neppy::agent::harness::subagent_runner::{
    run_subagent, SubagentRunOptions, SubagentRunStatus,
};
use crate::neppy::agent::messages::ChatMessage;
use crate::neppy::agent::orchestration::running_subagents::{self, SubagentStatus};
use crate::neppy::agent::orchestration::subagent_sessions::{
...
use crate::neppy::tools::traits::{PermissionLevel, Tool, ToolCallOptions, ToolResult};
...
impl Tool for SpawnAsyncSubagentTool {
    ...
    async fn execute_with_context(
        ...
        let parent_thread_id = crate::neppy::agent::tinyagents::thread_context::current_thread_id();
        ...
        let effective_action_root = workspace_descriptor
            ...
            .or_else(|| {
                crate::neppy::security::live_policy::current()
            ...
            if session.status == DurableSubagentStatus::Running {
                if let Some(ref running_task_id) = session.current_task_id {
                    match running_subagents::steer(
                        ...
                        crate::neppy::agent::harness::run_queue::QueueMode::Steer,
                    )
                    .await
                    {
                        Ok(()) => {
        ...
        crate::neppy::agent::orchestration::subagent_events::publish_subagent_spawned(
        ...
        // shell.
        let join = tokio::spawn(crate::neppy::agent::turn_origin::propagate(
            crate::neppy::agent::turn_workspace::propagate(async move {
                ...
                match result {
                    Ok(outcome) => match outcome.status {
                        SubagentRunStatus::Completed => {
                            ...
                            crate::neppy::agent::orchestration::background_completions::record_completion(
                            ...
                            crate::neppy::agent::orchestration::subagent_events::publish_subagent_completed(
                        ...
                        SubagentRunStatus::AwaitingUser { ref question, .. } => {
                            ...
                            crate::neppy::agent::orchestration::background_completions::record_awaiting_input(
                            background_parent_session.clone(),
                            outcome.task_id.clone(),
                            outcome.agent_id.clone(),
                            question,
...
mod tests {
    ...
    use crate::neppy::agent::harness::definition::AgentDefinitionRegistry;
    use crate::neppy::agent::harness::fork_context::{with_parent_context, ParentExecutionContext};
    use crate::neppy::config::AgentConfig;
    use crate::neppy::memory::{Memory, MemoryCategory, MemoryEntry, NamespaceSummary, RecallOpts};
    ...
    async fn guard_does_not_fire_when_parent_thread_is_bound() {
        ...
        let result = with_parent_context(parent_context(workspace.path()), async {
            crate::neppy::agent::tinyagents::thread_context::with_thread_id("t-parent", async {
                SpawnAsyncSubagentTool::new()
                    .execute(json!({
                        "agent_id": "researcher",
                        "prompt": "investigate x",
    ...
    fn parent_context(workspace_dir: &Path) -> ParentExecutionContext {
        ParentExecutionContext {
            ...
            turn_model_source: crate::neppy::agent::tinyagents::TurnModelSource::from_model(
