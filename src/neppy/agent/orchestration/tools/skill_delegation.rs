...
async fn fetch_live_connected_toolkit_slugs_once() -> Option<Vec<String>> {
    ...
    let config = crate::neppy::config::Config::load_or_init().await.ok()?;
    match crate::neppy::integrations::composio::fetch_connected_integrations_status(&config).await {
        crate::neppy::integrations::composio::FetchConnectedIntegrationsStatus::Authoritative(
            entries,
        ) => {
        ...
        crate::neppy::integrations::composio::FetchConnectedIntegrationsStatus::Unavailable => None,
    }
}
