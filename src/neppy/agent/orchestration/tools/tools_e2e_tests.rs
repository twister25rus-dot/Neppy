...
use crate::neppy::agent::context::prompt::{ConnectedIntegration, ToolCallFormat};
use crate::neppy::agent::harness::definition::AgentDefinitionRegistry;
use crate::neppy::agent::harness::{with_parent_context, ParentExecutionContext};
...
async fn archetype_delegation_defaults_to_async_with_durable_session_e2e() {
    ...
    assert!(
        crate::neppy::agent::orchestration::background_completions::has_pending(
            "tools-e2e-async-session"
        ),
        "finished result queued for background delivery"
    );
    let _ = crate::neppy::agent::orchestration::background_completions::take_pending(
        "tools-e2e-async-session",
    );
}
...
async fn continue_subagent_resumes_idle_durable_session_e2e() {
    ...
    subagent_sessions::mark_finished(
        ...
        "sub-earlier-task",
        &crate::neppy::agent::harness::subagent_runner::SubagentRunStatus::Completed,
        vec![
