...
use crate::neppy::agent::experience::store::{
    retrieve_across_stores, AgentExperienceStore, ExperienceQuery,
};
use crate::neppy::agent::experience::types::{AgentExperience, ExperienceHit};
...
///
/// [`MemoryGuard`](crate::neppy::memory::guard::MemoryGuard) truncates
...
/// # Home
///
/// This belongs in `src/neppy/memory/`, next to `binding`, not under
...
impl DriverMemory {
    ...
    pub fn for_subtree(config: &Config, memory_subdir: &str) -> Result<Arc<dyn Memory>, String> {
        let binding = crate::neppy::memory::binding::for_subtree(
...
impl Memory for DriverMemory {
    ...
    async fn recall(
        ...
        let scope = crate::neppy::memory::source_scope::as_bus_scope();
        Ok(self
            .provider
...
fn profile_memory_subdir(
    ...
    let state = crate::neppy::agent::profiles::load_profiles(workspace_dir)?;
    let profile = state
        .profiles
        .iter()
        .find(|profile| profile.id == profile_id)
        .ok_or_else(|| format!("agent profile '{profile_id}' not found"))?;
    let suffix = crate::neppy::agent::profiles::effective_memory_suffix(profile);
    Ok(crate::neppy::agent::profiles::memory_subdir_for_suffix(
        &suffix,
    ))
}
...
fn query_memory_subdirs(
    ...
    for profile in &state.profiles {
        if profile_id.is_none_or(|id| profile.id == id) {
            let suffix = crate::neppy::agent::profiles::effective_memory_suffix(profile);
            subdirs.insert(crate::neppy::agent::profiles::memory_subdir_for_suffix(
                &suffix,
            ));
        }
    }
...
mod tests {
    ...
    fn profile_memory_subdir_matches_live_session_derivation() {
        ...
        crate::neppy::agent::profiles::store::AgentProfileStore::new(
    ...
    async fn experience_store_round_trips_over_the_bound_driver() {
        ...
        let experience = AgentExperience {
            ...
            created_at_ms: 1,
            updated_at_ms: 1,
            source: crate::neppy::agent::experience::types::ExperienceSource::ToolLoop,
            agent_id: None,
            entrypoint: None,
            profile_id: None,
            ...
            outcome: crate::neppy::agent::experience::types::ExperienceOutcome::Success,
            error_class: None,
