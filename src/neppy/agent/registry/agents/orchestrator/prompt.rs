...
fn render_installed_skills(skills: &[Workflow]) -> String {
    ...
    for skill in skills {
        ...
        let desc = if skill.description.is_empty() {
            ...
            crate::neppy::util::sanitize::sanitize_for_llm(&skill.description, 240)
                .replace(['\n', '\t'], " ")
                .trim()
                .to_string()
        };
...
fn render_connected_mcp_servers() -> String {
    use crate::neppy::mcp::registry::connections;
...
fn format_connected_mcp_block(
    servers: &[crate::neppy::mcp::registry::connections::ConnectedServerOverview],
) -> String {
    if servers.is_empty() {
        return String::new();
    }
    ...
    for s in servers {
        ...
        let desc = if desc_raw.is_empty() {
            ...
            crate::neppy::util::sanitize::sanitize_for_llm(desc_raw, 240)
                .replace(['\n', '\t'], " ")
                .trim()
                .to_string()
        };
...
fn render_delegation_guide(
    ...
    let cross_chat_header_for_prompt =
        crate::neppy::memory::agent::memory_loader::CROSS_CHAT_HEADER.trim_end();
    let _ = write!(
        out,
...
mod tests {
    ...
    fn connected_mcp_block_lists_servers_with_description_and_routes_via_delegate() {
        ...
        use crate::neppy::mcp::registry::types::McpTool;
