...
//!
//! The tinyagents seam ([`crate::neppy::agent::tinyagents::middleware`]) drives this
...
///   (`src/neppy/tools/impl/filesystem/update_memory_md.rs`); only a
///   `MEMORY.md` edit reconciles the memory index, so a `SKILL.md` edit is not
///   an [`MemoryOp::IndexUpdate`] and must not close the cycle.
/// - the consolidated `memory_tree` tool (`src/neppy/memory/query/mod.rs`)
