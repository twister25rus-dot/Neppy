...
fn all_builtin_agent_definitions_have_expected_effective_max_iterations() {
    let defs = crate::neppy::agent::registry::agents::load_builtins()
    ...
    let expected: &[(&str, usize)] = &[
        ...
        #[cfg(feature = "flows")]
        ("workflow_builder", 50),
        // Compiled out with the `skills` gate — see `neppy::skills::stub`.
        #[cfg(feature = "skills")]
        ("skill_executor", 50),
        ...
        ("video_agent", 8),
        ("vision_agent", 6),
        // Compiled out with the `documents` gate — see `neppy::agent::registry::agents::loader::builtin_enabled`.
