...
use crate::neppy::agent::dispatcher::ToolExecutionResult;
use crate::neppy::memory::safety::{sanitize_text, SanitizationReport};
...
fn apply_tool_result_budget(content: String, budget_bytes: usize) -> (String, BudgetOutcome) {
    ...
    let mut cut = crate::neppy::util::floor_char_boundary(&content, head_capacity);
    if cut == 0 {
        cut = content
            .char_indices()
            .next()
...
fn bound_text_to_budget(content: String, budget_bytes: usize) -> (String, usize) {
    ...
    let cut = crate::neppy::util::floor_char_boundary(&output, budget_bytes);
    output.truncate(cut);
...
mod tests {
    ...
    use crate::neppy::tools::FileReadTool;
    use serde_json::json;
    use std::sync::Arc;
