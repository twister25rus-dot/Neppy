...
use crate::neppy::agent::context::prompt::{LearnedContextData, PromptContext, PromptTool};
use crate::neppy::agent::messages::{ChatMessage, ConversationMessage};
use crate::neppy::memory::MemoryCategory;
use crate::neppy::tools::agent_policy::render_tool_policy_boundary;
...
impl Agent {
    ...
    pub(in super::super) async fn fetch_learned_context(&self) -> LearnedContextData {
        ...
        if !self.learning_enabled && self.explicit_preferences_enabled {
            let general = crate::neppy::memory::preferences::load_general_preferences_on(
                &self.memory,
                crate::neppy::memory::preferences::STANDING_PREFS_LIMIT,
            )
            .await;
            tracing::debug!(
        ...
        let reflection_entries = self
            .memory
            .list(
                Some(crate::neppy::agent::learning::reflection::REFLECTIONS_NAMESPACE),
                Some(&MemoryCategory::Custom(
                    crate::neppy::agent::learning::reflection::REFLECTIONS_NAMESPACE.into(),
                )),
                None,
            )
            .await
    ...
    pub fn build_system_prompt(&self, learned: LearnedContextData) -> Result<String> {
        ...
        let agents_md = if self.config.agents_md_enabled {
            crate::neppy::agent::prompts::load_agents_md_layers(
                &self.workspace_dir,
                &self.action_dir,
            )
        } else {
