...
/// Wrapper around
/// [`crate::neppy::memory::tree::tree_runtime::store::collect_root_summaries_with_caps`]
...
/// preset by [`crate::neppy::config::schema::agent::AgentConfig::resolved_memory_limits`].
pub(super) fn collect_tree_root_summaries(
    workspace_dir: &std::path::Path,
    memory_subdir: &str,
    per_namespace_cap: usize,
    total_cap: usize,
) -> Vec<crate::neppy::agent::context::prompt::NamespaceSummary> {
    ...
    rows.into_iter()
        .map(|(namespace, body, updated_at)| {
            crate::neppy::agent::context::prompt::NamespaceSummary {
                namespace,
                body,
                updated_at,
            }
...
#[cfg(test)]
pub(crate) use crate::neppy::agent::context::prompt::LearnedContextData;
#[cfg(test)]
