...
use crate::neppy::agent::context::prompt::SystemPromptBuilder;
use crate::neppy::agent::dispatcher::{
    NativeToolDispatcher, PFormatToolDispatcher, XmlToolDispatcher,
};
use crate::neppy::agent::harness::definition::{AgentDefinitionRegistry, PromptSource, ToolScope};
use crate::neppy::agent::harness::session::types::Agent;
use crate::neppy::agent::host_runtime;
use crate::neppy::config::Config;
use crate::neppy::inference::provider;
use crate::neppy::memory::tool_memory::capture::ToolMemoryCaptureHook;
use crate::neppy::memory::Memory;
use crate::neppy::security::SecurityPolicy;
use crate::neppy::tools::{self, Tool};
use anyhow::Result;
use std::sync::Arc;
...
impl Agent {
    ...
    pub(crate) fn build_session_agent_inner(
        config: &Config,
        agent_id: &str,
        target_def: Option<&crate::neppy::agent::harness::definition::AgentDefinition>,
        profile_prompt_suffix: Option<String>,
        ...
        let audit = crate::neppy::security::get_or_create_workspace_audit_logger(
            crate::neppy::config::AuditConfig::default(),
        ...
        let memory_subdir = profile
            .map(|p| {
                crate::neppy::agent::profiles::memory_subdir_for_suffix(
                    &crate::neppy::agent::profiles::effective_memory_suffix(p),
                )
            })
            .unwrap_or_else(|| "memory".to_string());
        let memory_suffix = profile
            .map(crate::neppy::agent::profiles::effective_memory_suffix)
            .unwrap_or_default();
        let session_raw_subdir =
            crate::neppy::agent::profiles::session_raw_subdir_for_suffix(&memory_suffix);
        tracing::debug!(
        ...
        let memory: Arc<dyn Memory> =
            crate::neppy::agent::experience::ops::DriverMemory::for_subtree(config, &memory_subdir)
        ...
        let archivist_provider = crate::neppy::memory::binding::for_subtree(
        ...
        let shared_experience_memory = if memory_subdir == "memory" {
            ...
            Some(
                crate::neppy::agent::experience::ops::DriverMemory::for_config(config)
        ...
        let profile_skills_root: Option<std::path::PathBuf> = profile.and_then(|p| {
            crate::neppy::agent::profiles::profile_skills_root(&config.workspace_dir, &p.id)
        });
        ...
        let enabled_tools: Vec<String> = {
            use crate::neppy::desktop::app_state::load_stored_app_state;
        ...
        if !enabled_tools.is_empty() {
            crate::neppy::tools::filter_tools_by_user_preference(&mut tools, &enabled_tools);
        }
        ...
        let model_vision =
            crate::neppy::inference::model_context::model_supports_vision(&model_name, config);
        ...
        if let Err(reason) =
            crate::neppy::inference::provider::fallback_diagnostics::vision_preflight(
                &model_name,
                config,
            )
        {
            ...
            Some(def) => match &def.system_prompt {
                ...
                PromptSource::File { path } => {
                    ...
                    let body_text = if workspace_path.is_file() {
                        match crate::neppy::security::validate_path_within_root(
                            &workspace_path,
                            &prompt_root,
                        ) {
                            Ok(resolved) => {
        ...
        if config.learning.enabled {
            ...
            prompt_builder = prompt_builder
                .insert_section_before(
                    "user_memory",
                    Box::new(crate::neppy::agent::context::prompt::UserReflectionsSection),
                )
                .add_section(Box::new(
                    crate::neppy::agent::learning::LearnedContextSection::new(memory.clone()),
                ))
                .add_section(Box::new(
                    crate::neppy::agent::learning::UserProfileSection::new(memory.clone()),
                ));
        ...
        let workspace_notice = profile_workspace_descriptor
            ...
            .and_then(|descriptor| {
                profile.map(|p| {
                    crate::neppy::agent::profiles::cross_profile_workspace_notice(
                        &p.id,
                        &descriptor.root,
                    )
                })
        ...
        if profile_suffix.is_some() || workspace_notice.is_some() {
            ...
            let mut section = crate::neppy::agent::profiles::AgentProfilePromptSection::new(
        ...
        if config.learning.enabled {
            if config.learning.reflection_enabled {
                ...
                > = if config.learning.reflection_source
                    == crate::neppy::config::ReflectionSource::Cloud
                {
                ...
                post_turn_hooks.push(Arc::new(
                    crate::neppy::agent::learning::ReflectionHook::new(
            ...
            if config.learning.user_profile_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::neppy::agent::learning::UserProfileHook::new(
                        config.learning.clone(),
                        memory.clone(),
                    ),
                ));
            ...
            if config.learning.tool_tracking_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::neppy::agent::learning::ToolTrackerHook::new(
                        config.learning.clone(),
                        memory.clone(),
                    ),
                ));
            ...
            if config.learning.tool_memory_capture_enabled {
                ...
                post_turn_hooks.push(Arc::new(
                    crate::neppy::agent::experience::AgentExperienceCaptureHook::with_profile(
                        memory.clone(),
                        true,
        ...
        > = if config.learning.episodic_capture_enabled {
            ...
            post_turn_hooks
                .push(Arc::clone(&hook) as Arc<dyn crate::neppy::agent::hooks::PostTurnHook>);
            log::info!(
        ...
        post_turn_hooks.extend(crate::neppy::agent::hooks::embedder_post_turn_hooks());
        ...
        let prewarmed_integrations =
            crate::neppy::integrations::composio::cached_active_integrations(config);
        ...
        let prewarmed_integrations = match (
            ...
            (Some(list), Some(allow)) => {
                let filtered =
                    crate::neppy::agent::profiles::filter_integrations(&list, Some(allow));
                tracing::debug!(
                    before = list.len(),
                    after = filtered.len(),
        ...
        let (delegation_tools, filter_from_scope): (
            ...
            target_def,
            crate::neppy::agent::harness::definition::AgentDefinitionRegistry::global(),
        ) {
            (Some(def), Some(reg)) => {
        ...
        if config.learning.enabled {
            ...
            if has_retrieval {
                prompt_builder = prompt_builder
                    .add_section(Box::new(crate::neppy::agent::learning::MemoryAccessSection));
        ...
        let pformat_registry = crate::neppy::agent::pformat::build_registry(&tools);
        let dispatcher_kind =
            resolve_dispatcher_kind(&dispatcher_choice, supports_native, agent_id);
        let tool_dispatcher: Box<dyn crate::neppy::agent::dispatcher::ToolDispatcher> =
        ...
        let effective_tokenjuice_compression = target_def
            ...
            .unwrap_or(crate::neppy::inference::tokenjuice::AgentTokenjuiceCompression::Full);
        ...
        let payload_summarizer: Option<
            std::sync::Arc<
                dyn crate::neppy::agent::tinyagents::payload_summarizer::PayloadSummarizer,
            >,
        > = if agent_id == "orchestrator" && config.context.summarizer_payload_threshold_tokens > 0
        {
            match crate::neppy::agent::harness::definition::AgentDefinitionRegistry::global() {
                Some(reg) => match reg.get("summarizer") {
                    Some(summarizer_def) => {
                        ...
                        Some(std::sync::Arc::new(
                            crate::neppy::agent::tinyagents::payload_summarizer::SubagentPayloadSummarizer::new(
        ...
        let mut builder = Agent::builder()
            ...
            .personality_soul_md(profile.and_then(|profile| {
                crate::neppy::agent::profiles::resolve_personality_soul(
                    &config.workspace_dir,
                    profile,
                )
            }))
            .personality_memory_md(profile.and_then(|profile| {
                crate::neppy::agent::profiles::resolve_personality_memory_md(
                    &config.workspace_dir,
                    profile,
                )
            }))
            .profile_memory_storage(memory_subdir, session_raw_subdir)
            .workflows(crate::neppy::skills::load_workflow_metadata_for_profile(
                &config.workspace_dir,
                profile_skills_root.as_deref(),
            ))
        ...
        agent.last_seen_integrations_hash =
            crate::neppy::integrations::composio::connected_set_hash(&agent.connected_integrations);
...
fn resolve_target_definition(
    config: &Config,
    agent_id: &str,
) -> Result<Option<crate::neppy::agent::harness::definition::AgentDefinition>> {
    ...
    if let Some(entry) = crate::neppy::agent::registry::find_custom_in_config(config, agent_id) {
        log::info!(
        ...
        return Ok(Some(
            crate::neppy::agent::registry::definition_from_registry_entry(&entry),
        ));
    }
...
/// [`dedicated_workspace_dir`](crate::neppy::agent::profiles::dedicated_workspace_dir)),
...
pub(crate) fn derive_profile_workspace_descriptor(
    ...
    let (profile_id, dir) = profile.and_then(|p| {
        crate::neppy::agent::profiles::dedicated_workspace_dir(action_dir, p)
    ...
    Some(
        tinyagents::harness::workspace::WorkspaceDescriptor::new(dir).with_policy_id(
            crate::neppy::agent::profiles::workspace_policy_id(&profile_id),
        ),
    )
}
...
/// [`turn_workspace::with_workspace`](crate::neppy::agent::turn_workspace::with_workspace).
///
/// Returns a [`WorkspaceDescriptor`](tinyagents::harness::workspace::WorkspaceDescriptor)
...
{
    let root = crate::neppy::agent::turn_workspace::current()?;
    if !root.is_dir() {
        tracing::warn!(
            root = %root.display(),
...
mod profile_workspace_descriptor_tests {
    ...
    use crate::neppy::agent::profiles::store::built_in_default_profile;

    fn profile(id: &str, dedicated_workspace: bool) -> crate::neppy::agent::profiles::AgentProfile {
