...
fn recovery_tool_joins_a_named_allowlist() {
    ...
    use crate::neppy::inference::tokenjuice::RETRIEVE_TOOL_NAME as RECOVERY_TOOL_NAME;
...
fn automatic_memory_policy_does_not_synthesize_delegate_tools() {
    ...
    // the embedding seam; before the extraction this needed no setup.
    crate::neppy::memory::host_impls::install_for_tests();
    let defs = crate::neppy::agent::registry::agents::load_builtins().unwrap();
    let help = defs
        .iter()
...
fn test_config(tmp: &tempfile::TempDir) -> crate::neppy::config::Config {
...
fn builtin_def(id: &str) -> crate::neppy::agent::harness::definition::AgentDefinition {
...
fn custom_profile(id: &str, dedicated_memory: bool) -> crate::neppy::agent::profiles::AgentProfile {
    let mut profile = crate::neppy::agent::profiles::store::built_in_default_profile();
...
async fn build_session_agent_routes_dedicated_memory_to_profile_subtree() {
    ...
    let bound = crate::neppy::memory::binding::for_subtree(
...
async fn from_config_for_agent_synthesizes_custom_registry_entry_with_named_scope() {
    ...
    use crate::neppy::inference::tokenjuice::RETRIEVE_TOOL_NAME;
...
async fn build_session_agent_injects_default_profile_soul_into_prompt() {
    ...
    let profile = crate::neppy::agent::profiles::store::built_in_default_profile();
    let agent = Agent::build_session_agent_inner(
        &config,
        "orchestrator",
        None,
