use super::*;
use crate::neppy::agent::dispatcher::{PFormatToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use crate::neppy::agent::experience::{
    AgentExperience, AgentExperienceStore, ExperienceOutcome, ExperienceSource,
};
use crate::neppy::agent::hooks::{PostTurnHook, TurnContext};
...
use crate::neppy::inference::provider::{ChatResponse, UsageInfo};
...
fn make_agent(visible_tool_names: Option<HashSet<String>>) -> Agent {
    ...
    let mut builder = Agent::builder()
        ...
        .config(crate::neppy::config::AgentConfig {
            max_history_messages: 3,
            ..crate::neppy::config::AgentConfig::default()
        });
...
fn make_agent_with_builder(
    ...
    config: crate::neppy::config::AgentConfig,
    context_config: crate::neppy::config::ContextConfig,
) -> Agent {
...
fn build_parent_context_and_sanitize_helpers_cover_snapshot_paths() {
    ...
    agent.workflows = vec![crate::neppy::skills::Workflow {
        name: "demo".into(),
        ..Default::default()
    }];
...
fn collect_tree_root_summaries_maps_namespace_body_and_timestamp() {
    ...
    use crate::neppy::memory::tree::tree_runtime::store::write_node;
...
fn system_prompt_includes_tool_policy_boundary() {
    ...
    let mut config = crate::neppy::config::AgentConfig::default();
    config
        .channel_permissions
...
async fn turn_runs_full_tool_cycle_with_context_and_hooks() {
    ...
    let mut agent = make_agent_with_builder(
        ...
        hooks,
        crate::neppy::config::AgentConfig {
...
async fn turn_triggers_configured_memory_agent_before_parent_prompt() {
    ...
    let mut agent = Agent::builder()
        ...
        .auto_save(false)
        .event_context("turn-test-session", "turn-test-channel")
        .trigger_memory_agent(crate::neppy::agent::harness::definition::TriggerMemoryAgent::Always)
        .build()
        .unwrap();
    assert_eq!(
...
async fn turn_accepts_valid_required_output_without_repair() {
    ...
    let config = crate::neppy::config::AgentConfig {
        max_tool_iterations: 3,
        max_history_messages: 10,
        required_output: Some(crate::neppy::config::RequiredOutputContract {
...
async fn turn_synthesizes_required_output_when_reprompt_also_omits() {
    ...
    let config = crate::neppy::config::AgentConfig {
        max_tool_iterations: 3,
        max_history_messages: 10,
        required_output: Some(crate::neppy::config::RequiredOutputContract::new(
            "thoughts",
        )),
        ..crate::neppy::config::AgentConfig::default()
    };
    ...
    let first_block = crate::neppy::agent::harness::parse::extract_json_values(&response)
        .into_iter()
        .next();
    assert!(
        first_block
...
async fn turn_appends_required_output_block_when_streamed_to_preserve_consistency() {
    ...
    let drain = tokio::spawn(async move {
        while let Some(progress) = rx.recv().await {
            if let crate::neppy::agent::progress::AgentProgress::TextDelta { delta, .. } = progress
            {
...
async fn turn_checkpoint_rejects_pformat_wrapup_without_streaming_it() {
    ...
    let registry = crate::neppy::agent::pformat::build_registry(&tools);
...
fn make_real_memory(workspace: &std::path::Path) -> Arc<dyn Memory> {
    use crate::neppy::inference::embeddings::NoopEmbedding;
...
async fn fetch_learned_context_returns_empty_when_both_flags_off() {
    ...
    mem.store(
        ...
        crate::neppy::memory::MemoryCategory::Core,
        None,
    )
    .await
    .unwrap();
...
async fn fetch_learned_context_returns_general_prefs_when_explicit_flag_on_learning_off() {
    ...
    mem.store(
        crate::neppy::memory::preferences::USER_PREF_GENERAL_NAMESPACE,
        "package_manager",
