...
use super::*;
use crate::neppy::agent::harness::session::transcript::read_transcript_display;
...
fn first_display_message(
    path: &Path,
) -> crate::neppy::agent::harness::session::transcript::DisplayMessage {
    read_transcript_display(path)
        .unwrap()
        .records
        .into_iter()
        .find_map(|r| match r {
            crate::neppy::agent::harness::session::transcript::DisplayRecord::Message(m)
...
fn write_torture_transcript(dir: &TempDir) -> PathBuf {
    ...
    crate::neppy::agent::harness::session::transcript::attach_tool_failure_metadata(
        &mut failed_tool,
        Some("exit status 1"),
    );
    let reduced = vec![
    ...
    crate::neppy::agent::harness::session::transcript::append_interrupted_partial(
        &path,
        "half a sent",
        Some("req-3"),
        Some(1),
