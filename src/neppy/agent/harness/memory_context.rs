...
use crate::neppy::util::provenance_tag;
...
fn shorten_for_cross_chat(content: &str) -> String {
    if content.chars().count() > CROSS_CHAT_SNIPPET_CHARS {
        crate::neppy::util::truncate_with_ellipsis(content, CROSS_CHAT_SNIPPET_CHARS)
    } else {
        content.to_string()
    }
}
...
pub(crate) async fn build_context(
    ...
    if let Ok(entries) = crate::neppy::agent::tinyagents::retriever::recall_through_facade(
        mem,
        user_msg,
        5,
        crate::neppy::memory::RecallOpts::default(),
    )
    .await
    {
    ...
    let current_thread_id = crate::neppy::agent::tinyagents::thread_context::current_thread_id();
    let cross_session_opts = crate::neppy::memory::RecallOpts {
    ...
    .await
    {
        ...
        if !cross.is_empty() {
            ...
            context.push_str(crate::neppy::memory::agent::memory_loader::CROSS_CHAT_HEADER);
            for entry in &cross {
                let prov = entry
                    .session_id
                    .as_deref()
...
mod tests {
    ...
    impl Memory for MockMemory {
        ...
        async fn recall(
            ...
            query: &str,
            _limit: usize,
            opts: crate::neppy::memory::RecallOpts<'_>,
    ...
    async fn build_context_surfaces_cross_chat_block_with_provenance() {
        ...
        assert!(
            context
                .contains(crate::neppy::memory::agent::memory_loader::CROSS_CHAT_HEADER.trim_end()),
