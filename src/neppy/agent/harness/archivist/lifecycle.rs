...
impl ArchivistHook {
    ...
    /// only implementation is `NeppyChatHost` in `memory/host_impls.rs`,
    ...
    pub fn with_config(mut self, config: Config) -> Self {
        ...
        let probe = crate::neppy::inference::provider::create_chat_model_with_model_id(
    ...
    pub(super) async fn on_segment_closed(
        ...
        if let Some(ref cfg) = self.config {
            if cfg.learning.goals_enrichment_enabled && !summary.trim().is_empty() {
                ...
                crate::neppy::memory::goals::spawn_enrich_goals(
                    cfg.clone(),
    ...
    pub(super) async fn embed_segment_recap(&self, segment_id: &str, summary: &str, now: f64) {
        ...
        let Some(scoring) = provider.as_scoring() else {
            #[cfg(feature = "modules")]
            {
                ...
                if crate::neppy::modules::memory::ARTIFACT_CAPABILITIES
