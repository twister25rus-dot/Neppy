...
use crate::neppy::memory::api::provider::{ConversationSegment, EpisodicTurn};
    ...
    pub(super) async fn pipe_segment_to_tree(
        ...
        let messages = entries
            ...
            .filter_map(|e| {
                ...
                let (text, _image_refs) =
                    crate::neppy::agent::multimodal::parse_image_markers(&raw_text);
                if text.is_empty() {
                    return None;
                }
        ...
        match ingest_result {
            ...
            Err(e) => {
                ...
                if crate::neppy::memory::tree::health::user_error::is_corrupt_store_error(&rendered)
                {
                    tracing::error!(
                    ...
                    crate::neppy::memory::tree::health::user_error::notice_corrupt_store_once(
