...
use crate::neppy::agent::file_state::with_file_state_agent_id;
use crate::neppy::agent::harness::agent_graph::{AgentTurnRequest, AgentTurnUsage};
use crate::neppy::agent::harness::artifact_offload::{
...
use crate::neppy::agent::harness::fork_context::{
    current_parent, with_parent_context, ParentExecutionContext,
};
use crate::neppy::agent::harness::subagent_runner::extract_tool::ExtractFromResultTool;
use crate::neppy::agent::harness::subagent_runner::handoff::ResultHandoffCache;
use crate::neppy::agent::harness::subagent_runner::subagent_iter_cap_with_autonomous_lift;
use crate::neppy::agent::harness::subagent_runner::tool_prep::{
    build_text_mode_tool_instructions, filter_tool_indices, is_subagent_spawn_tool,
    load_prompt_source, top_k_for_toolkit,
};
use crate::neppy::agent::harness::subagent_runner::types::{
    SubagentMode, SubagentRunError, SubagentRunOptions, SubagentRunOutcome, SubagentRunStatus,
    SubagentUsage,
};
use crate::neppy::agent::harness::turn_dispatch_guard;
use crate::neppy::agent::harness::{
    current_spawn_depth, with_current_sandbox_mode, with_spawn_depth, MAX_SPAWN_DEPTH,
};
use crate::neppy::inference::provider::AGENT_TURN_MAX_OUTPUT_TOKENS;
use crate::neppy::memory::tree::retrieval::{fast_retrieve, FastRetrieveOptions, QueryResponse};
use crate::neppy::tools::{Tool, ToolCategory, ToolSpec};
...
async fn try_deterministic_memory_retrieval(
    ...
    let entities_empty = match crate::neppy::memory::binding::for_config(config) {
...
/// 2. Asks the turn's
///    [dispatch guard](crate::neppy::agent::harness::turn_dispatch_guard)
...
pub async fn run_subagent(
    ...
    Box::pin(async move {
        ...
        if let Err(reason) = crate::neppy::hooks::ops::subagent_starting(
            crate::neppy::hooks::context::TurnIdentity {
        ...
        let loaded_config: LoadedConfig = crate::neppy::config::Config::load_or_init()
            .await
...
async fn offload_outcome_artifacts(
    ...
    let policy = crate::neppy::security::live_policy::current();
    ...
    if policy
        .as_ref()
        .is_some_and(|p| p.autonomy == crate::neppy::security::AutonomyLevel::ReadOnly)
    {
        tracing::debug!(
            task_id = %task_id,
            agent_id = %outcome.agent_id,
...
type LoadedConfig = Result<std::sync::Arc<crate::neppy::config::Config>, String>;
...
async fn run_typed_mode(
    ...
    let live_integrations: Vec<crate::neppy::agent::context::prompt::ConnectedIntegration> = {
        ...
        if !signed_in {
            ...
            match config.as_ref() {
                Ok(cfg) => {
                    use crate::neppy::integrations::composio::FetchConnectedIntegrationsStatus;
                    match crate::neppy::integrations::composio::fetch_connected_integrations_status(
                        cfg,
                    )
                    .await
                    {
                ...
                .find(|ci| ci.connected && ci.toolkit.eq_ignore_ascii_case(tk))
            {
                let fresh_actions = match &client_kind {
                    Some(ComposioClientKind::Backend(client)) => {
                        match crate::neppy::integrations::composio::fetch_toolkit_actions(
                            client, tk, None,
                        )
                        .await
                        {
                ...
                let integration = crate::neppy::agent::context::prompt::ConnectedIntegration {
                ...
                let selected: Vec<&crate::neppy::agent::context::prompt::ConnectedIntegrationTool> =
                ...
                for action in selected {
                    dynamic_tools.push(Box::new(
                        crate::neppy::integrations::composio::ComposioActionTool::new(
                            arc_config.clone(),
                            action.name.clone(),
    ...
    let handoff_cache: Option<Arc<ResultHandoffCache>> = if is_integrations_agent_with_toolkit {
        ...
        let summarization_tier =
            crate::neppy::inference::provider::factory::summarization_tier_model().to_string();
        ...
        let (extract_source, extract_model) = match config.as_ref() {
            Ok(cfg) => {
                let route =
                    crate::neppy::inference::provider::provider_for_role("summarization", cfg);
                let r = route.trim();
                ...
                if route_is_managed && !parent.turn_model_source.is_local_provider() {
                    ...
                    match crate::neppy::inference::provider::create_chat_model_with_model_id(
                        "summarization",
                        cfg,
                        parent.temperature,
                    ) {
                        Ok((_model, resolved_model)) => (
                            crate::neppy::agent::tinyagents::TurnModelSource::new_crate_native(
                                "summarization",
    ...
    let filtered_specs =
        crate::neppy::agent::harness::session::dedup_visible_tool_specs(filtered_specs);
    ...
    let narrowed_integrations: Vec<crate::neppy::agent::context::prompt::ConnectedIntegration> =
    ...
    let agents_md = if parent.agent_config.agents_md_enabled {
        ...
        match local_dir {
            Some(dir) => {
                crate::neppy::agent::prompts::load_agents_md_layers(&parent.workspace_dir, &dir)
            }
            None => {
                // No resolvable project dir — still surface the workspace layer.
                crate::neppy::agent::prompts::AgentsMdContent {
                    global: crate::neppy::agent::prompts::load_agents_md(&parent.workspace_dir),
                    local: None,
                }
            }
        }
        ...
        crate::neppy::agent::prompts::AgentsMdContent::default()
    };
    ...
    let prompt_ctx = PromptContext {
        ...
        learned: crate::neppy::agent::context::prompt::LearnedContextData::default(),
        visible_tool_names: &visible_tool_names,
        tool_call_format: parent.tool_call_format,
        connected_integrations: &narrowed_integrations,
        connected_identities_md: crate::neppy::agent::prompts::render_connected_identities(),
        include_profile: !definition.omit_profile,
        include_memory_md: !definition.omit_memory_md,
        curated_snapshot: None,
        user_identity: crate::neppy::desktop::app_state::peek_cached_current_user_identity(),
    ...
    let now_str = crate::neppy::agent::prompts::current_datetime_line();
    ...
    let mut history: Vec<crate::neppy::agent::messages::ChatMessage> =
        if let Some(ref initial) = options.initial_history {
            ...
            vec![
                crate::neppy::agent::messages::ChatMessage::system(system_prompt),
                crate::neppy::agent::messages::ChatMessage::user(user_message),
            ]
        };
    ...
    let model_vision = config
        .as_ref()
        .ok()
        .map(|cfg| crate::neppy::inference::model_context::model_supports_vision(&model, cfg))
        .unwrap_or(false);
    tracing::debug!(
    ...
    use crate::neppy::agent::harness::agent_graph::AgentGraph;
    ...
    let status = if early_exit_tool.as_deref() == Some("ask_user_clarification") {
        ...
        if let Err(e) = std::fs::create_dir_all(&checkpoint_dir) {
            ...
            let checkpoint_data =
                crate::neppy::agent::harness::subagent_runner::types::SubagentCheckpointData {
                    task_id: task_id.to_string(),
                    agent_id: definition.id.clone(),
                    worker_thread_id: options.worker_thread_id.clone(),
        ...
        crate::neppy::agent::harness::subagent_runner::types::SubagentRunStatus::AwaitingUser {
            question,
            options: options_vec,
        }
        ...
        crate::neppy::agent::harness::subagent_runner::types::SubagentRunStatus::Completed
    };
    ...
    let usage = crate::neppy::agent::harness::subagent_runner::types::SubagentUsage {
    ...
    crate::neppy::agent::harness::turn_subagent_usage::record_subagent_usage(
        task_id,
        &definition.id,
        usage,
    );
...
mod fast_path_tests {
    ...
    use crate::neppy::memory::tree::retrieval::types::{NodeKind, QueryResponse, RetrievalHit};
    use chrono::Utc;
