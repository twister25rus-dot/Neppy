...
use crate::neppy::agent::harness::agent_graph::{
    AgentTurnRequest, AgentTurnResult, AgentTurnUsage,
};
use crate::neppy::agent::harness::subagent_runner::types::SubagentRunError;
use crate::neppy::agent::messages::{ChatMessage, ConversationMessage};
use crate::neppy::agent::progress::AgentProgress;
use crate::neppy::agent::tinyagents::{run_turn_via_tinyagents_shared, SubagentScope};
...
pub(super) async fn run_subagent_via_graph(
    ...
    run_queue: Option<Arc<crate::neppy::agent::harness::run_queue::RunQueue>>,
    ...
    handoff_cache: Option<
        std::sync::Arc<crate::neppy::agent::harness::subagent_runner::ResultHandoffCache>,
    >,
...
> {
    ...
    let dispatch_history = if (turn_models.supports_vision() || model_vision)
        && crate::neppy::agent::multimodal::has_image_placeholders(history)
    {
        crate::neppy::agent::multimodal::rehydrate_image_placeholders(history)
    } else {
        history.clone()
    };
    ...
    let transcript_snapshot: crate::neppy::agent::tinyagents::TranscriptSnapshotSink =
    ...
    let run_result = Box::pin(run_turn_via_tinyagents_shared(
        ...
        // attached, plus the live transcript-snapshot sink for error recovery.
        {
            ...
            if let Some(cache) = handoff_cache {
                mw.handoff = Some(crate::neppy::agent::tinyagents::HandoffConfig {
                    cache,
                    agent_id: agent_id.to_string(),
                    task_id: task_id.to_string(),
                });
                ...
                if let Some(u) = co.usage {
                    ...
                    let call_cost =
                        if u.charged_amount_usd.is_finite() && u.charged_amount_usd > 0.0 {
                            ...
                            crate::neppy::platform::cost::catalog::estimate_cost_usd(
                                model,
                                u.input_tokens,
                                u.output_tokens,
...
fn build_subagent_context_mw(
    tokenjuice_compression: AgentTokenjuiceCompression,
    config: Option<&crate::neppy::config::Config>,
) -> crate::neppy::agent::tinyagents::TurnContextMiddleware {
    let mut mw = crate::neppy::agent::tinyagents::TurnContextMiddleware::defaults();
...
fn persist_subagent_transcript(
    ...
    let meta = transcript::TranscriptMeta {
        ...
        cached_input_tokens: usage.cached_input_tokens,
        charged_amount_usd: usage.charged_amount_usd,
        thread_id: crate::neppy::agent::tinyagents::thread_context::current_thread_id(),
    ...
    for msg in conversation {
        if let ConversationMessage::ToolResults(results) = msg {
            for r in results {
                ...
                let body = crate::neppy::util::truncate_with_ellipsis(&r.content, 800);
...
mod tests {
    ...
    async fn subagent_runs_through_the_graph_engine_with_real_tools() {
        ...
        let (output, iterations, usage, early_exit, hit_cap, _breaker) = run_subagent_via_graph(
            crate::neppy::agent::tinyagents::TurnModelSource::from_model(provider),
            "mock-model",
            0.0,
            &mut history,
            parent_tools,
    ...
    async fn child_text_and_thinking_deltas_are_scoped_to_the_subagent() {
        ...
        let (output, _iters, _usage, _early, _hit_cap, _breaker) = run_subagent_via_graph(
            crate::neppy::agent::tinyagents::TurnModelSource::from_model(Arc::new(
    ...
    async fn ask_user_clarification_pauses_and_surfaces_the_question() {
        ...
        let (output, iterations, _usage, early_exit, _hit_cap, _breaker) = run_subagent_via_graph(
            crate::neppy::agent::tinyagents::TurnModelSource::from_model(provider.clone()),
            "mock-model",
            0.0,
            &mut history,
            parent_tools,
