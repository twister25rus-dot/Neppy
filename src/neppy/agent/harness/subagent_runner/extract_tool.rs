...
use crate::neppy::tools::{Tool, ToolCategory, ToolResult};
...
impl ExtractFromResultTool {
    ...
    async fn extract_chunk_char_budget(&self) -> usize {
        let window = self
            ...
            .await
            .or_else(|| crate::neppy::inference::context_window_for_model(&self.model));
...
mod tests {
    ...
    fn chunk_budget_tracks_context_window() {
        let summarization_window =
            crate::neppy::inference::context_window_for_model("summarization-v1");
