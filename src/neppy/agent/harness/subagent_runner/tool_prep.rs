...
//! this module so the debug-dump path in [`crate::neppy::agent::debug`] can
...
use crate::neppy::agent::context::prompt::PromptContext;
...
/// [`crate::neppy::agent::dispatcher::PFormatToolDispatcher`] renders and
...
///   ([`crate::neppy::tools::orchestrator_tools::collect_orchestrator_tools`]
...
pub(super) fn filter_tool_indices(
    ...
    parent_tools
        ...
        .filter(|(_, tool)| {
            ...
            if crate::neppy::inference::tokenjuice::is_recovery_tool(name) {
...
mod tests {
    ...
    fn unprefixed_delegate_name_overrides_are_treated_as_spawn_tools() {
        ...
        crate::neppy::agent::harness::definition::AgentDefinitionRegistry::init_global(tmp.path())
            .unwrap();
        for delegate in [
            "plan",
            "run_code",
...
mod recovery_visibility_tests {
    use super::*;
    use crate::neppy::inference::tokenjuice::LEGACY_RETRIEVE_TOOL_NAME as RECOVERY_TOOL_NAME;
    use crate::neppy::tools::{CurrentTimeTool, RetrieveToolOutputTool};

    fn tools() -> Vec<Box<dyn crate::neppy::tools::Tool>> {
        vec![
    ...
    fn names(idx: &[usize], tools: &[Box<dyn crate::neppy::tools::Tool>]) -> Vec<String> {
    ...
    match source {
        ...
        PromptSource::File { path } => {
            ...
            if workspace_path.is_file() {
                if let Ok(resolved) =
                    crate::neppy::security::validate_path_within_root(&workspace_path, &prompt_root)
                {
            ...
            // already shipped at `src/neppy/agent/prompts/` and
            ...
            if workspace_root_path.is_file() {
                if let Ok(resolved) = crate::neppy::security::validate_path_within_root(
                    &workspace_root_path,
                    workspace_dir,
                ) {
                    return std::fs::read_to_string(&resolved).map_err(|e| {
