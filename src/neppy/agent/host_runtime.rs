...
use crate::neppy::agent::platform_shell;
use crate::neppy::config::RuntimeConfig;
...
impl RuntimeAdapter for NativeRuntime {
    ...
    fn build_shell_command(
        ...
        // (#3353, Fix 2)
        crate::neppy::config::ensure_usable_cwd(workspace_dir)?;
...
fn maybe_hide_window(cmd: &mut tokio::process::Command, hide: bool) {
    ...
    crate::neppy::inference::local::process_util::apply_no_window(cmd);
    #[cfg(windows)]
    tracing::debug!(
...
pub struct DockerRuntime {
    config: crate::neppy::config::DockerRuntimeConfig,
}

impl DockerRuntime {
    fn new(config: crate::neppy::config::DockerRuntimeConfig) -> Self {
        Self { config }
    }
}
...
mod tests {
    use super::*;
    use crate::neppy::config::{DockerRuntimeConfig, RuntimeConfig};
