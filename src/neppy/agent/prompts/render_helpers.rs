...
pub fn render_subagent_system_prompt(
    ...
    parent_tools: &[Box<dyn crate::neppy::tools::Tool>],
    extra_tools: &[Box<dyn crate::neppy::tools::Tool>],
    archetype_body: &str,
...
pub fn render_subagent_system_prompt_with_format(
    ...
    if !matches!(tool_call_format, ToolCallFormat::Native) {
        out.push_str("## Tools\n\n");
        let render_one =
            |out: &mut String, tool: &dyn crate::neppy::tools::Tool| match tool_call_format {
...
pub fn default_workspace_file_content(filename: &str) -> &'static str {
    // The bundled identity files live at `src/neppy/agent/prompts/`
    // (owned by the `agent/` tree because they describe agent identity).
    // This module is under `src/neppy/agent/context/`, so the relative path
...
fn empty_prompt_context_for_static_sections() -> PromptContext<'static> {
    ...
    static EMPTY_WORKFLOWS: &[crate::neppy::skills::Workflow] = &[];
...
fn render_pformat_signature_for_box_tool(tool: &dyn crate::neppy::tools::Tool) -> String {
