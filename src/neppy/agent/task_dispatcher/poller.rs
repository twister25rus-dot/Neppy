...
use crate::neppy::agent::task_board::{TaskApprovalMode, TaskBoardCard};
use crate::neppy::config::Config;
use crate::neppy::threads::todos::ops::{self, BoardLocation, USER_TASKS_THREAD_ID};
use crate::neppy::threads::todos::runs::{self, RunLimits};
...
pub(crate) async fn poll_once() -> Result<bool, String> {
    ...
    let Some(_permit) = crate::neppy::cron::scheduler_gate::wait_for_capacity().await else {
    ...
    if config.task_sources.enabled {
        boards.push((
            BoardLocation::Thread {
                workspace_dir: config.workspace_dir.clone(),
                thread_id: crate::neppy::integrations::task_sources::TASK_SOURCES_THREAD_ID
                    .to_string(),
            },
            false,
        ));
...
/// see [`crate::neppy::agent::tools::todo`]) stamps `Required`, and that
