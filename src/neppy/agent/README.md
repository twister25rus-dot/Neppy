...
- Built-in archetypes live in `src/neppy/agent/registry/agents/`; this module stays focused on harness/runtime behavior.
...
- `src/neppy/providers/` — `ChatMessage`, `ChatResponse` send/receive against LLMs.
- `src/neppy/tools/` — `Tool` / `ToolSpec` execution surface invoked from the tool loop.
- `src/neppy/memory/` — episodic indexing + memory-loader context injection.
- `src/neppy/agent/context/` — prompt sections, tool-call format selection.
- `src/neppy/inference/local/` — `agent_chat` / `agent_chat_simple` execution backend.
- `src/neppy/config/` — runtime config load via `config::rpc::load_config_with_timeout`.
- `src/core/event_bus/` — emits `DomainEvent::Agent(*)` and `Trigger*` events; subscribers in `agent/bus.rs`.

## Called by

- `src/neppy/channels/runtime/dispatch.rs` and `web_chat/` — drive chat turns from inbound channel messages.
- `src/neppy/cron/scheduler.rs` — fire scheduled triggers through `triage::run_triage` + `apply_decision`.
- `src/neppy/skills/webhooks/ops.rs` — webhook ingestion routes through triage.
- `src/neppy/integrations/composio/bus.rs` — Composio trigger envelopes go through `agent::triage`.
- `src/neppy/desktop/notifications/rpc.rs` — surfaces agent runs to the UI.
- `src/neppy/agent/learning/{reflection,tool_tracker,user_profile}.rs` — read transcripts + tool outcomes.
- `src/neppy/agent/orchestration/tools/{dispatch,spawn_subagent}.rs` — `spawn_subagent` tool delegates here.
