...
async fn list_without_thread_filter_returns_all_threads() {
    use crate::neppy::agent::artifacts::store::save_artifact_meta;
    use crate::neppy::agent::artifacts::types::{ArtifactKind, ArtifactMeta, ArtifactStatus};
    let tmp = TempDir::new().unwrap();
    let config = test_config(&tmp);
...
async fn regenerate_rejects_non_presentation_kind() {
    use crate::neppy::agent::artifacts::store::{save_artifact_args, save_artifact_meta};
    use crate::neppy::agent::artifacts::types::{ArtifactKind, ArtifactMeta, ArtifactStatus};
    let tmp = TempDir::new().unwrap();
    let config = test_config(&tmp);
...
async fn regenerate_errors_when_args_missing() {
    use crate::neppy::agent::artifacts::store::create_artifact;
    use crate::neppy::agent::artifacts::types::ArtifactKind;
    let tmp = TempDir::new().unwrap();
    let config = test_config(&tmp);
...
async fn regenerate_reruns_producer_and_reuses_id() {
    use crate::neppy::agent::artifacts::store::{
        create_artifact, get_artifact, save_artifact_args,
    };
    use crate::neppy::agent::artifacts::types::{ArtifactKind, ArtifactStatus};
    let tmp = TempDir::new().unwrap();
    let config = test_config(&tmp);
