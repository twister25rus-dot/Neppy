...
fn handle_list_artifacts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::agent::artifacts::ops::ai_list_artifacts(
                &config,
                offset,
                limit,
                thread_id.as_deref(),
...
fn handle_get_artifact(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::agent::artifacts::ops::ai_get_artifact(&config, artifact_id.trim())
                .await?,
        )
    })
}
...
fn handle_delete_artifact(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let artifact_id = read_required::<String>(&params, "artifact_id")?;
        to_json(
            crate::neppy::agent::artifacts::ops::ai_delete_artifact(&config, artifact_id.trim())
                .await?,
        )
    })
}
...
fn handle_regenerate(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::agent::artifacts::ops::ai_regenerate(
                &config,
                artifact_id.trim(),
                thread_id.trim(),
                client_id.trim(),
