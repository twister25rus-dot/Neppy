...
mod tests {
    ...
    fn memory_limits_come_from_resolved_limits_not_the_legacy_field() {
        let mut c = base();
        c.agent.memory_window = Some(crate::neppy::config::schema::MemoryContextWindow::Maximum);
    ...
    fn required_output_contract_is_carried_across() {
        let mut c = base();
        c.agent.required_output = Some(crate::neppy::config::RequiredOutputContract {
    ...
    fn team_pins_apply_and_a_missing_team_is_a_no_op() {
        ...
        c.teams.insert(
            "research".into(),
            crate::neppy::config::TeamModelConfig {
