...
use crate::neppy::inference::provider::{ChatResponse, ProviderDelta, UsageInfo};
...
pub(crate) fn native_chat_messages(request: &ModelRequest) -> Vec<ChatMessage> {
    request
        .messages
        .iter()
        .map(crate::neppy::agent::message_convert::message_to_native_chat_message)
        .collect()
}

/// Build a [`PFormatRegistry`](crate::neppy::agent::pformat::PFormatRegistry)
...
) -> crate::neppy::agent::pformat::PFormatRegistry {
    request
        .tools
        .iter()
        .map(|t| {
            (
                t.name.clone(),
                crate::neppy::agent::pformat::PFormatToolParams::from_schema(&t.parameters),
            )
        })
        .collect()
}
...
fn response_to_model_response(
    ...
    pformat_registry: &crate::neppy::agent::pformat::PFormatRegistry,
    ...
    let (visible_text, tool_calls): (String, Vec<TaToolCall>) = if !response.tool_calls.is_empty() {
        ...
        let (prose, parsed) =
            crate::neppy::agent::harness::parse_tool_calls_with_pformat(text, pformat_registry);
        if parsed.is_empty() {
    ...
    if let Some(block) = crate::neppy::agent::message_convert::reasoning_content_block(
...
pub(crate) fn native_model_response(response: &ChatResponse) -> ModelResponse {
    response_to_model_response(
        response,
        &crate::neppy::agent::pformat::PFormatRegistry::default(),
        false,
    )
}
...
mod g1_usage_tests {
    ...
    fn empty_registry() -> crate::neppy::agent::pformat::PFormatRegistry {
        crate::neppy::agent::pformat::PFormatRegistry::default()
    }
