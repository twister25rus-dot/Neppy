...
//!
//! * [`crate::neppy::agent::prompts`] (re-exported as
//!   `crate::neppy::agent::context::prompt`) — `SystemPromptBuilder`,
...
//!   `src/neppy/agent/prompts/` are loaded (and synced to the workspace)
//!   by `IdentitySection` inside `SystemPromptBuilder::build`, so this adapter
//!   never reads them itself.
//! * [`crate::neppy::config::Config`] — supplies `workspace_dir`,
//!   `action_dir`, the default model, and the `agents_md_enabled` gate.
//! * [`crate::neppy::desktop::app_state::peek_cached_current_user_identity`] — the
...
use crate::neppy::config::{Config, DEFAULT_MODEL};
use crate::neppy::skills::Workflow;
...
pub struct NeppyContextComposer {
    ...
    // `crate::neppy::agent::harness::session::turn::context` (see the
...
impl ContextComposer for NeppyContextComposer {
    ...
    /// [`crate::neppy::agent::prompts::PromptSection`] itself fails — a
    ...
    async fn compose_system_prompt(&self, req: &TurnContextRequest) -> TinyAgentsResult<String> {
        ...
        // `crate::neppy::skill_registry` / `skills`, behind the `skills`
        ...
        let ctx = PromptContext {
            ...
            // from the profiles domain (`crate::neppy::profiles`); the
