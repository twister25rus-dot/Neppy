...
//! [`crate::neppy::memory`] directly. Instead the crate declares
//! [`AgentMemory`] and this module is the single place Neppy's memory
//! domains meet it.
//!
//! # Domains adapted
//!
//! - [`crate::neppy::memory`] — the `Memory` trait, `MemoryEntry`,
//!   `MemoryCategory`, `MemoryTaint`, `RecallOpts`.
//! - `crate::neppy::agent::tinyagents::retriever::recall_through_facade` — the
...
//! - [`crate::neppy::memory::safety`] — `sanitize_text`, the
//!   conservative secret + PII scrubber, applied on the way out of recall and on
//!   the way in to `remember`.
//! - [`crate::neppy::memory::agent::memory_loader::MemoryCitation`] — the
...
//!   `crate::neppy::flows::memory_tools` uses to keep a flow inside its
...
use crate::neppy::memory::agent::memory_loader::MemoryCitation;
use crate::neppy::memory::safety::sanitize_text;
use crate::neppy::memory::{Memory, MemoryCategory, MemoryEntry, MemoryTaint, RecallOpts};
...
impl AgentMemory for NeppyAgentMemory {
    ...
    async fn recall(&self, req: RecallRequest) -> TaResult<Vec<MemoryItem>> {
        ...
        let entries = crate::neppy::agent::tinyagents::retriever::recall_through_facade(
            self.memory.as_ref(),
            &req.query,
            limit,
            opts,
...
mod tests {
    use super::*;
    use crate::neppy::memory::NamespaceSummary;
    use std::sync::Mutex;
