...
//!    ([`crate::neppy::security::pii::scan`]) but exposes no verified
...
use crate::neppy::tools::agent_policy::{ToolPolicyAction, ToolPolicySession};
...
pub struct NeppySecurityGate {
    ...
    /// [`crate::neppy::security::live_policy::current`] so an autonomy
...
impl NeppySecurityGate {
    ...
    fn effective_policy(&self) -> Arc<SecurityPolicy> {
        crate::neppy::security::live_policy::current().unwrap_or_else(|| self.policy.clone())
    }
...
mod tests {
    ...
    async fn read_only_autonomy_denies_an_acting_tool() {
        ...
        assert!(
            reason.contains(crate::neppy::security::POLICY_BLOCKED_MARKER),
    ...
    fn policy_session(tool_name: &str, action: ToolPolicyAction) -> Arc<ToolPolicySession> {
        use crate::neppy::tools::agent_policy::{TaskProfile, TaskRiskLevel, ToolPolicyDecision};
