...
//! inference domain — `crate::neppy::inference::provider` (the per-role
...
use crate::neppy::inference::provider::{create_chat_model_with_model_id, role_for_model_tier};
...
impl NeppyModelResolver {
    ...
    /// `crate::neppy::agent::tinyagents::TurnModelSource::build` (see
    /// `build_turn_models_crate` in `src/neppy/tinyagents/mod.rs`). That is a
