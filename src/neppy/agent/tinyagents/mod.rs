...
pub(crate) use crate::neppy::agent::message_convert::chat_message_to_message;
#[cfg(feature = "flows")]
pub(crate) use crate::neppy::agent::message_convert::{reasoning_from_content, ta_call_to_oh_call};
use std::sync::Arc;
...
use crate::neppy::agent::harness::{run_queue::RunQueue, MAX_SPAWN_DEPTH};
use crate::neppy::agent::messages::{ChatMessage, ConversationMessage};
...
/// The builder-configured [`ToolPolicy`](crate::neppy::agent::tool_policy::ToolPolicy)
...
pub(crate) struct ToolPolicyEnforcement {
    pub policy: StdArc<dyn crate::neppy::agent::tool_policy::ToolPolicy>,
    ...
    pub session: crate::neppy::tools::agent_policy::ToolPolicySession,
...
pub(crate) async fn run_turn_via_tinyagents(
    ...
    resolved_tools: Vec<Arc<dyn crate::neppy::tools::Tool>>,
    ...
    let input = crate::neppy::agent::message_convert::history_to_messages(&history);
    ...
    let out_history = crate::neppy::agent::message_convert::messages_to_history(&run.messages);
    let conversation = crate::neppy::agent::message_convert::messages_to_conversation(
        crate::neppy::agent::message_convert::messages_since_request(
            &run.messages,
            request_base_len,
        ),
    );
    ...
    Ok(TinyagentsTurnOutcome {
        ...
        charged_amount_usd: crate::neppy::platform::cost::catalog::estimate_cost_usd(
            model,
...
pub(crate) async fn run_turn_via_tinyagents_shared(
    ...
    model: &str,
    history: Vec<ChatMessage>,
    tool_sets: Vec<Arc<Vec<Box<dyn crate::neppy::tools::Tool>>>>,
    ...
    if !registry_diagnostics.is_empty() {
        ...
        if !errors.is_empty() {
            ...
            return Err(anyhow::Error::new(
                crate::neppy::agent::error::AgentError::RegistryValidationFailed {
    ...
    if let Some(session_kv) = crate::neppy::agent::session_import::live::session_kv_store().await {
        stores.get_or_insert_with(StoreRegistry::new).register(
            crate::neppy::agent::session_import::live::TINYAGENTS_SESSION_KV_STORE,
            session_kv,
        );
        tracing::debug!(
    ...
    if pause_at_cap {
        if let (Some(events), Some(handle)) = (&events, &handle) {
            ...
            let dispatch_guard = subagent_scope
                .is_none()
                .then(crate::neppy::agent::harness::turn_dispatch_guard::current)
                .flatten();
    ...
    if subagent_scope.is_none() {
        if let Some(crate::neppy::agent::turn_origin::AgentTurnOrigin::WebChat {
            request_id: Some(request_id),
            ..
        }) = crate::neppy::agent::turn_origin::current()
        {
    ...
    let steer_thread_label = subagent_scope
        ...
        .or_else(|| match crate::neppy::agent::turn_origin::current() {
            Some(crate::neppy::agent::turn_origin::AgentTurnOrigin::WebChat {
                thread_id, ..
            }) => Some(thread_id),
            Some(crate::neppy::agent::turn_origin::AgentTurnOrigin::ExternalChannel {
                reply_target,
                ..
            }) => Some(reply_target),
            _ => None,
        ...
        Err(e) => {
            ...
            if let tinyagents::TinyAgentsError::LimitExceeded(msg) = &e {
                if msg.contains("model call") {
                    ...
                    return Err(anyhow::Error::new(
                        crate::neppy::agent::error::AgentError::MaxIterationsExceeded {
                            max: max_iterations,
                        },
                    ));
                }
    ...
    let (input_tokens, output_tokens, cached_input_tokens, charged_amount_usd) = bridge_totals
        .unwrap_or_else(|| {
            ...
            let charged = crate::neppy::platform::cost::catalog::estimate_cost_usd(
    ...
    Ok(TinyagentsTurnOutcome {
        text,
        history: crate::neppy::agent::message_convert::messages_to_history(&run.messages),
        conversation,
        model_calls: run.model_calls,
        tool_calls: run.tool_calls,
        input_tokens,
...
) -> Option<crate::neppy::agent::harness::subagent_runner::SubagentRunError> {
    match err {
        tinyagents::TinyAgentsError::SubAgentDepth(max_depth)
        | tinyagents::TinyAgentsError::RecursionLimit(max_depth) => Some(
            crate::neppy::agent::harness::subagent_runner::SubagentRunError::SpawnDepthExceeded {
...
pub(crate) struct TurnModels {
    ...
    error_slot: crate::neppy::agent::tinyagents::model::ModelErrorSlot,
...
/// / Motion A).
///
/// An [`Agent`](crate::neppy::agent::Agent) (and each channel/subagent turn
...
/// exactly one place — [`create_turn_model_source`](crate::neppy::inference::provider::factory::create_turn_model_source).
#[derive(Clone)]
...
struct CrateNativeSource {
    role: String,
    config: Arc<crate::neppy::config::Config>,
...
impl TurnModelSource {
    ...
    pub(crate) async fn effective_context_window(&self, model: &str) -> Option<u64> {
        ...
        let provider_string = self.crate_native.as_ref().map(|source| {
            source.primary_override.clone().unwrap_or_else(|| {
                crate::neppy::inference::provider::provider_for_role(&source.role, &source.config)
            })
        });
    ...
    pub(crate) fn is_local_provider(&self) -> bool {
        ...
        self.crate_native.as_ref().is_some_and(|source| {
            ...
            crate::neppy::inference::local::profile::is_local_provider_string(&provider)
        })
    }
    ...
    pub(crate) fn build(
        ...
        if let Some(cn) = &self.crate_native {
            let provider_string = cn.primary_override.clone().unwrap_or_else(|| {
                crate::neppy::inference::provider::provider_for_role(&cn.role, &cn.config)
            });
            let is_local =
                crate::neppy::inference::local::profile::is_local_provider_string(&provider_string);
    ...
    pub(crate) fn build_summarizer(
        ...
        if let Some(cn) = &self.crate_native {
            let built = match cn.primary_override.as_deref() {
                Some(ps) => {
                    crate::neppy::inference::provider::factory::create_turn_chat_model_from_string(
                        &cn.role,
                        ps,
                        &cn.config,
                        model,
                ...
                None => crate::neppy::inference::provider::factory::create_turn_chat_model(
                    &cn.role,
                    &cn.config,
                    model,
                    temperature,
...
fn assemble_turn_harness(
    ...
    let stop_hooks = crate::neppy::agent::stop_hooks::current_stop_hooks();
    ...
    match crate::neppy::agent::registry::agents::load_builtins() {
        Ok(builtins) => {
            for def in builtins {
                if registered_agents.insert(def.id.clone()) {
    ...
    let embedder_tool_hooks = crate::neppy::agent::hooks::embedder_tool_hooks();
...
fn record_unobserved_turn_usage(
    ...
    crate::neppy::platform::cost::record_provider_usage(
        model,
