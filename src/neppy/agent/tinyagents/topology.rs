...
pub(crate) fn all_graph_topologies() -> Vec<GraphTopologyReport> {
    ...
    if let Ok(t) = crate::neppy::agent::orchestration::agent_teams::member_graph_topology() {
    ...
    if let Ok(t) = crate::neppy::agent::orchestration::workflow_runs::scheduler_graph_topology() {
        out.push(describe("workflow_runs:scheduler", &t));
    }

    if let Ok(t) = crate::neppy::agent::registry::agents::researcher::graph::topology() {
        out.push(describe("agent:researcher", &t));
    }

    if let Ok(t) =
        crate::neppy::agent::orchestration::spawn_parallel_graph::spawn_parallel_graph_topology()
    {
...
mod tests {
    ...
    fn member_report_renders_mermaid_and_valid_json() {
        let t = crate::neppy::agent::orchestration::agent_teams::member_graph_topology()
