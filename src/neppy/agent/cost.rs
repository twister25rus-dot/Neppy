...
//!   [`crate::neppy::agent::progress::AgentProgress::TurnCostUpdated`];
...
///    ([`crate::neppy::platform::cost::catalog`]) — accurate per-model rates for
...
pub(crate) fn lookup_pricing(model: &str) -> ModelPricing {
    ...
    if let Some(price) = crate::neppy::platform::cost::catalog::lookup(model) {
        return ModelPricing {
