...
use crate::neppy::cron::{self, ActiveHours, DeliveryConfig};
...
fn test_job(command: &str) -> CronJob {
    CronJob {
        id: "test-job".into(),
        expression: "* * * * *".into(),
        schedule: crate::neppy::cron::Schedule::Cron {
            expr: "* * * * *".into(),
            tz: None,
            active_hours: None,
        },
...
async fn attributed_cron_build_applies_profile_temperature_and_prompt_defaults() {
    ...
    let prompt = built
        .agent
        .build_system_prompt(crate::neppy::agent::prompts::LearnedContextData::default())
...
async fn persist_job_result_success_deletes_one_shot() {
    ...
    let job = cron::add_agent_job(
        &config,
        Some("one-shot".into()),
        crate::neppy::cron::Schedule::At { at },
        "Hello",
...
fn publish_cron_user_error_broadcasts_metadata_only_for_each_kind() {
    use crate::neppy::web_chat::subscribe_web_channel_events;
