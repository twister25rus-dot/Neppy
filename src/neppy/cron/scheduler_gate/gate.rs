...
use crate::neppy::config::{Config, SchedulerGateConfig};
use crate::neppy::cron::scheduler_gate::policy::{decide, PauseReason, Policy};
use crate::neppy::cron::scheduler_gate::signals::Signals;
...
pub async fn wait_for_capacity() -> Option<LlmPermit> {
    loop {
        ...
        // `neppy::agent::triage::evaluator::tests::*` hangs reported
        // after #1516.
        if STATE.get().is_some() && is_signed_out() {
            let paused_ms = STATE
                .get()
...
mod tests {
    ...
    async fn wait_for_capacity_acquires_immediately_when_signed_out_and_uninit() {
        ...
        // `neppy::agent::triage::evaluator::tests::*` hangs that surfaced
