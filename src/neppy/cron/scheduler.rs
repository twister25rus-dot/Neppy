...
fn cron_alert_body(job: &CronJob, output: &str) -> String {
    ...
    crate::neppy::util::truncate_with_ellipsis(&body, 512)
}
...
fn is_budget_exhausted_failure(
    ...
    let signal = last_agent_error.unwrap_or(last_output);
    crate::neppy::inference::provider::is_budget_exhausted_message(signal)
}
...
fn publish_cron_user_error(kind: &str) {
    ...
    crate::neppy::web_chat::publish_web_channel_event(crate::core::socketio::WebChannelEvent {
            ...
            match build_agent_for_cron_job(&effective, job) {
                Ok(BuiltCronAgent { mut agent, profile }) => {
                    ...
                    if is_morning_briefing_job(job) {
                        ...
                        crate::neppy::agent::harness::with_task_recency_window(
...
fn resolve_cron_profile(
    config: &Config,
    job: &CronJob,
) -> anyhow::Result<Option<crate::neppy::agent::profiles::AgentProfile>> {
...
fn apply_cron_profile_runtime_defaults(
    config: &Config,
    job: &CronJob,
    profile: &crate::neppy::agent::profiles::AgentProfile,
) -> Config {
    let mut effective = config.clone();
...
fn push_cron_alert(config: &Config, job: &CronJob, output: &str) {
    use crate::neppy::desktop::notifications::store as notif_store;
