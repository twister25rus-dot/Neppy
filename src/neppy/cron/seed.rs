...
//! channels module's
//! [`crate::neppy::channels::proactive::ProactiveMessageSubscriber`]
...
fn prune_legacy_welcome(config: &Config, existing: &[crate::neppy::cron::CronJob]) {
...
mod tests {
    use super::*;
    use crate::neppy::cron::{add_agent_job_with_definition, list_jobs, Schedule, SessionTarget};
    ...
    fn seeds_morning_briefing_disabled_and_idempotent() {
        ...
        assert!(
            jobs.iter()
                .filter(|j| matches!(j.job_type, crate::neppy::cron::JobType::Agent))
                .all(|j| !j.enabled),
