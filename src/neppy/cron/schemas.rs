...
use crate::neppy::cron::CronJobPatch;
...
fn handle_add(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let schedule: crate::neppy::cron::Schedule = read_required(&params, "schedule")?;
        let name = params
            .get("name")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        ...
        let session_target = match session_target_str {
            "main" => crate::neppy::cron::SessionTarget::Main,
            "isolated" => crate::neppy::cron::SessionTarget::Isolated,
        ...
        let delivery: Option<crate::neppy::cron::DeliveryConfig> = match params.get("delivery") {
        ...
        let job = match job_type {
            "shell" => {
                ...
                crate::neppy::cron::store::add_shell_job(&config, name, schedule, &cmd)
                    .map_err(|e| e.to_string())?
            }
            "agent" => {
                let p = prompt.ok_or("'prompt' is required for agent jobs")?;
                crate::neppy::cron::store::add_agent_job_with_definition(
                    &config,
                    name,
                    schedule,
                    &p,
...
fn handle_list(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::neppy::cron::rpc::cron_list(&config).await?)
    })
}
...
fn handle_update(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::neppy::cron::rpc::cron_update(&config, job_id.trim(), patch).await?)
    })
}
...
fn handle_remove(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let job_id = read_required::<String>(&params, "job_id")?;
        to_json(crate::neppy::cron::rpc::cron_remove(&config, job_id.trim()).await?)
    })
}
...
fn handle_run(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let job_id = read_required::<String>(&params, "job_id")?;
        to_json(crate::neppy::cron::rpc::cron_run(&config, job_id.trim()).await?)
    })
}
...
fn handle_runs(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let limit = read_optional_u64(&params, "limit")?
            ...
            .transpose()?;
        to_json(crate::neppy::cron::rpc::cron_runs(&config, job_id.trim(), limit).await?)
    })
}
