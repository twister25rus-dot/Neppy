...
- `src/neppy/agent/` — `agent` job type runs through `agent::triage::TriggerEnvelope::from_cron` + `apply_decision`.
- `src/neppy/security/` — `SecurityPolicy::from_config` sandboxes shell jobs.
- `src/neppy/config/` — `Config` provides poll interval, workspace dir, autonomy policy.
- `src/neppy/platform/health/` — `health::bus::register_health_subscriber` on startup.
- `src/neppy/channels/` — `bus.rs` can fan delivery events into channels.
- `src/core/event_bus/` — `init_global`, `publish_global(DomainEvent::Cron(*))`.

## Called by

- `src/neppy/tools/impl/system/schedule.rs` — `schedule` tool exposes cron operations to agents.
