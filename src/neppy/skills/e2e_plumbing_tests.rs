...
use crate::neppy::config::{Config, MultimodalConfig, MultimodalFileConfig};
use crate::neppy::skills::ops_create::{
    create_workflow_inner, CreateWorkflowParams, WorkflowCreateInputDef,
};
use crate::neppy::skills::ops_types::WorkflowScope;
use crate::neppy::skills::registry::get_workflow;
use crate::neppy::skills::run_log;
use crate::neppy::skills::runtime::await_run_outcome;
...
async fn mock_llm_orchestrator_lists_and_runs_workflows_through_the_loop() {
    ...
    let tools: Arc<Vec<Box<dyn Tool>>> = Arc::new(vec![
        Box::new(crate::neppy::skills::tools::WorkflowListTool::new(
            config.clone(),
        )),
    ...
    let result = run_channel_turn_via_graph(
        crate::neppy::agent::tinyagents::TurnModelSource::from_model_with_profile(
            model,
            ModelProfile {
                tool_calling: true,
