...
use crate::neppy::skills::{preflight, registry, run_log};
...
{
    crate::neppy::memory::source_scope::with_source_scope(
...
pub async fn spawn_workflow_run_background_with_profile(
    ...
    let guidelines = match &skill.definition.system_prompt {
        crate::neppy::agent::harness::definition::PromptSource::Inline(s) => s.clone(),
        _ => String::new(),
    };
    ...
    let inherited_origin = crate::neppy::agent::turn_origin::current()
...
mod tests {
    ...
    async fn workflow_profile_installs_memory_source_scope() {
        ...
        let visible = with_profile_memory_source_scope(Some(&profile), async {
            crate::neppy::memory::source_scope::current_source_scope()
        })
        .await;
