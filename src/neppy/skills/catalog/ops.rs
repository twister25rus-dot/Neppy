...
pub async fn install_from_catalog(
    ...
    entry: &CatalogEntry,
) -> Result<crate::neppy::skills::ops_install::InstallWorkflowFromUrlOutcome, String> {
    tracing::info!(
        entry_id = %entry.id,
        source = %entry.source,
    ...
    let params = crate::neppy::skills::ops_install::InstallWorkflowFromUrlParams {
        url: entry.download_url.clone(),
        timeout_secs: Some(60),
    };

    crate::neppy::skills::ops_install::install_workflow_from_url(workspace_dir, params).await
}
