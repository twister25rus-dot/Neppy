...
use crate::neppy::skills::catalog::ops;
...
pub(super) fn handle_uninstall(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let workflow_params =
            crate::neppy::skills::ops_install::UninstallWorkflowParams { name: payload.name };
        let outcome = crate::neppy::skills::ops_install::uninstall_workflow(workflow_params, None)?;
        to_json(RpcOutcome::new(
            UninstallResult {
                name: outcome.name,
