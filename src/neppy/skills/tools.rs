...
mod tests {
    ...
    async fn run_history_is_scoped_to_profile_and_allowlist() {
        ...
        let path = crate::neppy::skills::run_log::run_log_path(tmp.path(), "private-flow", run_id);
        crate::neppy::skills::run_log::write_header_with_profile(
            &path,
            "private-flow",
            run_id,
            &json!({"secret": true}),
        ...
        let mut alice = crate::neppy::agent::profiles::built_in_profiles()
            .into_iter()
            .next()
