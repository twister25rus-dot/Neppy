...
- `src/neppy/config/` — workspace path resolution and trust-marker location.
- `src/neppy/agent/` — the `## Installed Skills` catalog rendered in `agent_registry/agents/orchestrator/prompt.rs`, fed by the skill list on `PromptContext` (`agent/harness/session/turn/context.rs`).
- `src/neppy/config/workspace/` — workspace-relative skill paths.
- `src/core/event_bus/` — emits `DomainEvent::Skill(*)` on install / uninstall.

## Called by

- `src/neppy/tools/traits.rs` — `ToolResult` / `ToolContent` shape shared with the tool registry.
- `src/neppy/config/workspace/ops.rs` — workspace bootstrap touches the skill directory layout.
- `src/neppy/agent/registry/agents/integrations_agent/prompt.rs` — integrations agent reads the skill catalog.
- `src/neppy/agent/harness/fork_context.rs` — fork context propagates injected skills.
- `src/neppy/agent/harness/session/turn.rs` — per-turn injection point.
- `src/neppy/agent/prompts/{mod,types}.rs` — render `## Available Skills` catalog section.
...
- Cross-cutting agent + skill behavior is covered indirectly by `src/neppy/agent/harness/session/{turn,runtime}_tests.rs`.
