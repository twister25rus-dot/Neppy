...
| File | Role |
| --- | --- |
| `src/neppy/tools/mod.rs` | Export hub. Declares submodules, re-exports built-in impls and the domain-owned tool sets (`agent`, `voice::audio_toolkit`, `codegraph`, `composio`, `cron`, `integrations`, `memory`, `wallet`, `whatsapp_data`), and the `all_tools_*` controller pair. |
...
| `src/neppy/tools/ops.rs` | Registry assembly: `default_tools`, `default_tools_with_runtime`, `all_tools`, `all_tools_with_runtime`, `browser_allowed_domains`. All config-gating logic lives here. |
| `src/neppy/tools/schemas.rs` | JSON-RPC `tools` namespace controllers + `handle_*` fns. `all_controller_schemas` / `all_registered_controllers` (re-exported as `all_tools_*`). |
| `src/neppy/tools/policy.rs` | `ToolPolicy` trait + `PolicyDecision` (`Allow`/`Deny`) + allow-all `DefaultToolPolicy`. Evaluated on the agent hot path before each `execute()`. |
| `src/neppy/tools/schema.rs` | `SchemaCleanr` / `CleaningStrategy` — per-provider JSON-Schema normalization (Gemini keyword stripping, `$ref` resolution, union flattening, cycle detection). |
| `src/neppy/tools/orchestrator_tools.rs` | Synthesizes named per-subagent tools from the orchestrator's `subagents = [...]` definition; collapses skill wildcards into `delegate_to_integrations_agent`. |
| `src/neppy/tools/generated.rs` | `GeneratedToolDefinition` + wrapper for runtime/profile-supplied generated capability tools (provider/capability/risk metadata for policy). |
| `src/neppy/tools/user_filter.rs` | `filter_tools_by_user_preference` + UI-toggle-ID → Rust-tool-name map. Unmapped tools are always retained. |
| `src/neppy/tools/impl/mod.rs` | Aggregates and re-exports the built-in tool families. |
| `src/neppy/tools/impl/filesystem/` | `file_read`, `file_write`, `edit_file`, `apply_patch`, `grep`, `glob_search`, `list_files`, `read_diff`, `csv_export`, `git_operations`, `run_linter`, `run_tests`, `update_memory_md`. |
| `src/neppy/tools/impl/browser/` | `browser` (DOM-snapshot automation, pluggable backend), `browser_open`, `image_info`, action parser, native backend, security. |
| `src/neppy/tools/impl/computer/` | `mouse`, `keyboard` (native control, default-off), human-path resolution. |
| `src/neppy/tools/impl/network/` | `http_request`, `web_fetch`, `curl`, `gitbooks` (search/get-page), `mcp` (list servers/tools, call), `mcp_setup` (5 setup-agent tools), `gmail_unsubscribe`, `url_guard`. |
| `src/neppy/search/` | Search engine registry and search-owned agent tools such as `web_search`. |
| `src/neppy/tools/impl/system/` | `shell`, `node_exec`, `npm_exec`, `install_tool`, `detect_tools`, `current_time`, `schedule`, `proxy_config`, `pushover`, `lsp`, `tool_stats`, `update_check`, `update_apply`, `insert_sql_record`, `workspace_state`. |
...
- All built-in tool structs (e.g. `ShellTool`, `FileReadTool`, `EditFileTool`, `GrepTool`, `BrowserTool`, `HttpRequestTool`, `CurlTool`, `WebSearchTool`, `LspTool`, …) via `pub use implementations::*`, plus re-exported domain tool sets such as `neppy::search::tools::*`.
...
- **Search**: `web_search` and provider-specific search families are registered by `neppy::search::registry`; `search.engine = "disabled"` suppresses this surface entirely.
...
None. This module has no `bus.rs` and registers no `EventHandler`. Approval coordination is via the `Tool::external_effect[_with_args]` hooks that the agent harness reads to route calls through the `ApprovalGate`; the gate itself lives in `neppy::security::approval`.
...
- `neppy::agent` — `host_runtime` (`RuntimeAdapter`/`NativeRuntime`), `tool_policy::GeneratedToolRuntimeContext`, harness definitions (`AgentDefinition`, `SubagentEntry`) for orchestrator tool synthesis, and the agent-owned dispatch tools re-exported here.
- `neppy::config` — `Config`, `BrowserConfig`, `HttpRequestConfig`, `SearchEngine`, `DelegateAgentConfig`; drives all registration gating and `config::rpc::load_config_with_timeout` in RPC handlers.
- `neppy::search` — active search engine registry and search-owned tool implementations.
- `neppy::security` — `SecurityPolicy` (host/path/command gating threaded into nearly every tool) + `AuditLogger`.
- `neppy::memory` — `Memory` trait, injected into memory/preference/stats tools.
- `neppy::integrations` — `build_client` backend HTTP client + the integration tool structs (apify, brave, parallel, stock, twilio, tinyfish, google_places, querit, seltz, searxng).
- `neppy::integrations::composio` — `all_composio_agent_tools`, mode-aware client for `tools.composio_execute`.
- `neppy::runtime::javascript` — `NodeBootstrap` shared by shell/node_exec/npm_exec.
- `neppy::mcp::config_servers` / `neppy::mcp::registry` — generic remote MCP server registry + bridge tools.
- `neppy::skills` — `skills::types::{ToolResult, ToolContent}` (the unified result type) + skill-run spawning.
- `neppy::agent::learning` — LinkedIn enrichment scrape/render for the Apify RPC handler.
- `neppy::web3::wallet`, `neppy::cron`, `neppy::codegraph`, `neppy::voice::audio_toolkit`, `neppy::channels::whatsapp_data` — domain-owned tools re-exported and registered.
- `neppy::security::approval`, `neppy::agent::context`, `neppy::security::credentials`, `neppy::platform::update`, `neppy::util` — supporting types used by individual tools.
...
- `neppy::agent` harness (`session/builder`, `dispatcher`, `subagent_runner`, `agent/tools/*`) and the `neppy::agent::tinyagents` seam (`SharedToolAdapter`, `ToolPolicyMiddleware`) — primary consumers; build the registry and execute/police tools on the tinyagents harness path.
- `neppy::channels`, `neppy::routing`, `neppy::inference::provider` — build tool sets / clean schemas per provider.
- `neppy::tools::agent_policy`, `neppy::security::approval` — read tool metadata (category, external-effect) for policy/approval decisions.
- `neppy::tools::registry`, `neppy::runtime::node`, `neppy::mcp::server` — registry/exposure consumers.
