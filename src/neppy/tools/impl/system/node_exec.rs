...
//! toolchain.
//!
//! Sibling to [`crate::neppy::tools::impl::system::shell::ShellTool`]: same
//! security gates, same env hygiene, but the command is pinned to the `node`
//! binary resolved by
//! [`crate::neppy::runtime::javascript::NodeBootstrap`].
//!
//! Two input modes:
//!
...
pub struct NodeExecTool {
    ...
    pool_cfg: crate::neppy::config::RuntimePoolConfig,
...
impl NodeExecTool {
    async fn execute_in_context(
        ...
        if matches!(
            ...
            Some(crate::neppy::agent::harness::definition::SandboxMode::Sandboxed)
        ) {
            return Ok(self
        ...
        match result {
            Ok(Ok(output)) => {
                ...
                if stdout.len() > MAX_OUTPUT_BYTES {
                    stdout.truncate(crate::neppy::util::floor_char_boundary(
                        &stdout,
                        MAX_OUTPUT_BYTES,
                    ));
                ...
                if stderr.len() > MAX_OUTPUT_BYTES {
                    stderr.truncate(crate::neppy::util::floor_char_boundary(
                        &stderr,
                        MAX_OUTPUT_BYTES,
                    ));
...
impl NodeExecTool {
    ...
    async fn try_pool_inline(
        ...
        if !crate::neppy::runtime::pool::node::enabled(&self.pool_cfg) {
            return None;
        }
        ...
        match crate::neppy::runtime::pool::node::run_inline(
        ...
        .await
        {
            ...
            Err(crate::neppy::runtime::pool::PoolRunError::PreDispatch(error)) => {
                tracing::warn!(
                    error = %error,
            ...
            Err(crate::neppy::runtime::pool::PoolRunError::Saturated) => {
            ...
            Err(crate::neppy::runtime::pool::PoolRunError::PostDispatch(error)) => {
                tracing::warn!(
                    error = %error,
...
impl NodeExecTool {
    ...
    async fn run_sandboxed(
        ...
        let effective = timeout.unwrap_or_else(|| {
            Duration::from_secs(crate::neppy::tools::timeout::SANDBOX_UNBOUNDED_CAP_SECS)
        });
        ...
        // finding on PR #3309.)
        let runtime_cfg = match crate::neppy::config::ops::load_config_with_timeout().await {
            Ok(cfg) => cfg.runtime,
            Err(err) => {
                tracing::warn!(
                    error = %err,
                ...
                crate::neppy::config::RuntimeConfig::default()
            }
        };
        ...
        let policy = sandbox::resolve_sandbox_policy(
            crate::neppy::agent::harness::definition::SandboxMode::Sandboxed,
            &security.action_dir,
            &runtime_cfg,
            false,
        );
...
fn pool_outcome_to_result(
    outcome: crate::neppy::runtime::pool::PoolExecOutcome,
    timeout: Option<Duration>,
) -> ToolResult {
...
mod tests {
    ...
    async fn inline_code_cannot_write_to_sibling_profile() {
        ...
        use crate::neppy::security::policy::ActiveProfileGuard;
