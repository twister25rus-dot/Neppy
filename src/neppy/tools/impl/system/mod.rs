...
use crate::neppy::security::policy::{TrustedAccess, TrustedRoot};
...
pub(super) fn check_cross_profile_command(
    ...
    let other_id = match crate::neppy::agent::profiles::classify_cross_profile_target(
        &guard.action_dir,
        &guard.profile_id,
        cwd,
    ) {
        crate::neppy::agent::profiles::CrossProfileDecision::Block { other_id } => Some(other_id),
        crate::neppy::agent::profiles::CrossProfileDecision::Allow => {
            crate::neppy::agent::profiles::scan_command_for_cross_profile(
                command,
                cwd,
                &guard.action_dir,
                &guard.profile_id,
    ...
    if other_id == crate::neppy::agent::profiles::PROFILES_ROOT_SENTINEL {
        Err(format!(
