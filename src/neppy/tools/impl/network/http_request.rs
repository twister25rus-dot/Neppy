...
fn network_egress_descriptor(
    ...
    has_body: bool,
    has_headers: bool,
) -> crate::neppy::security::egress::EgressDescriptor {
    use crate::neppy::security::egress::{DataKind, EgressDescriptor};
...
impl HttpRequestTool {
    ...
    async fn execute_request(
        ...
        let builder =
            crate::neppy::config::apply_runtime_proxy_to_builder(builder, "tool.http_request");
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // below stays the S2 disclosure point for permitted requests.
        {
            let host = reqwest::Url::parse(url)
                ...
                .and_then(|u| u.host_str().map(str::to_string))
                .unwrap_or_else(|| "unknown".to_string());
            if let Some(msg) = crate::neppy::security::egress::local_only_tool_block(
                &crate::neppy::security::egress::EgressDescriptor::network_fetch(host),
            ) {
        ...
        // round-trip.
        {
            let has_headers = headers_val
                ...
                .map(|h| !h.is_empty())
                .unwrap_or(false);
            crate::neppy::security::egress::emit_external_transfer(network_egress_descriptor(
                &url,
                body.is_some(),
                has_headers,
            ));
