...
use crate::neppy::tools::implementations::document::format::spec::ImageFormat;
...
use crate::neppy::agent::artifacts::{
...
impl Tool for PresentationTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        ...
        if let Err(err) = crate::neppy::agent::artifacts::store::save_artifact_args(
            &self.workspace_dir,
            &meta.id,
            &args,
        )
...
impl PresentationTool {
    ...
    async fn resolve_one_image(&self, image: &SlideImage) -> Result<ResolvedSlideImage, String> {
        ...
        // Identification and measurement live in `crate::neppy::tools::implementations::document::format::spec::image`, which
