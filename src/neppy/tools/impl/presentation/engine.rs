...
//! EMU geometry — lives in `crate::neppy::tools::implementations::document::format::pptx` and runs inside the loaded module.
...
use crate::neppy::tools::implementations::document::format::spec::{
...
use crate::neppy::modules::documents;
...
pub(super) async fn generate(
    ...
    let config = match config {
        Some(config) => config,
        None => {
            loaded_config = match crate::neppy::config::Config::load_or_init().await {
                Ok(config) => config,
                Err(error) => {
...
mod tests {
    ...
    //! `crate::neppy::tools::implementations::document::format::pptx`, where the code now lives — reproducing them here would
    ...
    use super::*;
    use crate::neppy::tools::implementations::presentation::types::SlideSpec;
    ...
    fn resolved(bytes: &[u8], caption: Option<&str>) -> ResolvedSlideImage {
        ResolvedSlideImage {
            bytes: bytes.to_vec(),
            format: crate::neppy::tools::implementations::document::format::spec::ImageFormat::Png,
            width_px: 4,
            height_px: 4,
