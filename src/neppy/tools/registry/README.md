...
| File | Role |
| --- | --- |
| `src/neppy/tools/registry/mod.rs` | Export-focused. Re-exports ops, providers, schemas (as `all_tool_registry_*`), and types. |
| `src/neppy/tools/registry/ops.rs` | Core logic: `registry_entries()`, `list_tools()`, `get_tool()`, `diagnostics()` / `diagnostics_for_config()`, plus schema→JSON-Schema conversion, tagging, write-capability heuristics, MCP write-audit health query. |
| `src/neppy/tools/registry/types.rs` | Serde response types: `ToolRegistryEntry`, `ToolRegistryList`, `ToolRegistryTransport`, `ToolRegistryHealth`, `ToolPolicyDiagnostics` + sub-structs, `RecentPolicyDenial`, `CapabilityProviderDiagnostics`. |
| `src/neppy/tools/registry/schemas.rs` | Controller schemas + `handle_list` / `handle_get` / `handle_diagnostics` handlers delegating to `ops.rs`. Inline tests. |
| `src/neppy/tools/registry/providers.rs` | `CapabilityProviderRegistry` over config: id normalization, dedupe, trust checks, redacted diagnostics. Inline tests. |
| `src/neppy/tools/registry/denials.rs` | Static `Mutex<VecDeque>` ring buffer (max 50) of recent policy denials; `record()` / `list()`; redacts secret markers, truncates reasons. Inline tests. |
| `src/neppy/tools/registry/ops_tests.rs` | Sibling test module for `ops.rs` (`#[path]`-included). |
...
- `crate::neppy::config` (`Config`, `config::schema::CapabilityProviderTrustState`) — autonomy posture, MCP client allowlists, capability-provider config.
- `crate::neppy::mcp::server` (`McpToolSpec`, `tool_specs()`) — MCP stdio tool source for registry entries.
- `crate::neppy::mcp::registry::connections` (`all_connected_tools()`) — live MCP client server tools, fetched via `block_in_place` only on the multi-thread runtime.
...
- `src/neppy/agent/tinyagents/middleware.rs` — calls `tool_registry::denials::record(...)` to log agent-tool policy denials.
- `src/neppy/platform/about_app/catalog.rs` — capability catalog references the registry surface.
- `src/neppy/mcp/registry/connections.rs` — provides `all_connected_tools()` for registry integration.
