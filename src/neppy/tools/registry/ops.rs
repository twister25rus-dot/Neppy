...
use crate::neppy::mcp::audit::McpWriteListQuery;
use crate::neppy::mcp::server::McpToolSpec;
...
fn mcp_write_audit_health(config: &Config) -> McpWriteAuditHealth {
    ...
    let result =
        crate::neppy::mcp::audit::list_writes(config, &query).map(|writes| writes.len() as u64);
...
fn build_registry_entries(config: Option<&Config>) -> Vec<ToolRegistryEntry> {
    ...
    for spec in crate::neppy::mcp::server::tool_specs() {
        let entry = mcp_tool_entry(spec);
        insert_registry_entry(&mut entries, entry, "mcp_stdio");
    }

    for schema in crate::neppy::tools::all_tools_controller_schemas() {
