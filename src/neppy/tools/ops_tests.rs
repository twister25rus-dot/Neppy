use super::*;
use crate::neppy::config::{BrowserConfig, Config, MemoryConfig};
use crate::neppy::security::credentials::{
    AuthService, APP_SESSION_PROVIDER, DEFAULT_AUTH_PROFILE_NAME,
};
use crate::neppy::security::AuditLogger;
...
fn integration_test_config(tmp: &TempDir, backend_url: &str) -> Config {
    ...
    cfg.search.engine = crate::neppy::config::SEARCH_ENGINE_PARALLEL.into();
...
fn all_tools_registers_generic_mcp_bridge_tools_when_servers_exist() {
    ...
    cfg.mcp_client
        .servers
        .push(crate::neppy::config::McpServerConfig {
            name: "docs".into(),
            ...
            timeout_secs: 30,
            auth: crate::neppy::config::McpAuthConfig::None,
        });
...
fn all_tools_registers_brave_engine_lsp_and_tool_stats_when_enabled() {
    ...
    unsafe {
        std::env::set_var(crate::neppy::tools::implementations::LSP_ENABLED_ENV, "1");
    }
...
fn all_tools_registers_querit_engine_when_enabled() {
    ...
    let http = crate::neppy::config::HttpRequestConfig::default();
    let mut cfg = test_config(&tmp);
    cfg.search.engine = crate::neppy::config::SEARCH_ENGINE_QUERIT.into();
...
fn all_tools_omits_search_surface_when_search_is_disabled() {
    ...
    let mut cfg = test_config(&tmp);
    cfg.api_url = Some("https://backend.example.test".to_string());
    cfg.search.engine = crate::neppy::config::SEARCH_ENGINE_DISABLED.into();
    cfg.search.brave.api_key = Some("test-brave-key".into());
    cfg.search.querit.api_key = Some("test-querit-key".into());
    cfg.integrations.tinyfish.enabled = true;
    store_test_session_token(&cfg);
...
fn null_driver_memory_cfg() -> crate::neppy::config::schema::MemorySubsystemConfig {
...
async fn memory_tools_all_present_under_the_module_driver() {
    ...
    let ctx = CoreContext::for_test(
        ...
        Some(crate::neppy::config::schema::MemorySubsystemConfig::default()),
    );
