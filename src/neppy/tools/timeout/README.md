...
| File | Role |
| --- | --- |
| `src/neppy/tools/timeout/mod.rs` | Entire module: constants, env parsing, pure resolver, atomic-backed runtime value, setter, public accessors, and inline unit tests. |
...
- `src/neppy/agent/tinyagents/tools.rs` — Neppy tools execute through `execute_with_options`, which applies each tool's [`Tool::timeout_policy`]: `Inherit` uses `tool_execution_timeout_secs()`; `Secs(n)` uses a clamped value plus grace; `Unbounded` runs with no deadline.
- `src/neppy/tools/impl/system/{shell,node_exec,npm_exec}.rs` — scripting tools: unbounded by default, explicit `timeout_secs` via `explicit_call_timeout_*`.
- `src/neppy/agent/tools/delegate.rs` — bounds the delegated provider chat call with `tool_execution_timeout_secs`.
- `src/neppy/config/ops.rs` — `apply_agent_settings` calls `set_tool_timeout_secs` after persisting; `get_agent_settings` reports `effective_timeout_secs` / `env_override`.
...
- `src/neppy/agent/harness/harness_gap_tests.rs` — pins `parse_tool_timeout_secs` default/boundary behaviour.
