...
use crate::neppy::agent::tool_policy::GeneratedToolRuntimeContext;

// Re-export the unified ToolResult from the lightweight skills types module so all tools use one type.
pub use crate::neppy::skills::types::{ToolContent, ToolResult};
...
/// steven@gmail.com" / `Read(src/neppy/web3/wallet/ops.rs)` without every tool
...
pub trait Tool: Send + Sync {
    ...
    fn pack_registry_handle(&self) -> Option<&crate::neppy::tools::toolpacks::PackRegistryHandle> {
        None
    }
    ...
    /// `crate::neppy::agent::tinyagents::tools`) currently executes tool
