...
use crate::neppy::config::schema::Config;
...
pub async fn evaluate(instruction: &str, model: Option<&str>) -> Result<String, String> {
    let mut config = crate::neppy::config::rpc::load_config_with_timeout()
        .await
        .map_err(|error| format!("loading config for prompt hook: {error}"))?;
    if let Some(model) = model {
        apply_model_override(&mut config, model);
    }
    let outcome = crate::neppy::inference::ops::inference_prompt(
        &config,
        instruction,
