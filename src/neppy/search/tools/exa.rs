...
impl ExaClient {
    ...
    fn egress_descriptor(&self) -> crate::neppy::security::egress::EgressDescriptor {
        crate::neppy::security::egress::EgressDescriptor::network_fetch(self.egress_host())
            .with_data_kind(crate::neppy::security::egress::DataKind::Prompt)
    }
    ...
    fn local_only_block(&self) -> Option<ToolResult> {
        crate::neppy::security::egress::local_only_tool_block(&self.egress_descriptor())
    ...
    async fn post(&self, path: &str, body: Value) -> anyhow::Result<Value> {
        ...
        crate::neppy::security::egress::emit_external_transfer(self.egress_descriptor());
    ...
    fn render_plain(&self, results: &[ExaResultItem], heading: &str, limit: usize) -> String {
        ...
        for (i, item) in results.iter().take(limit).enumerate() {
            ...
            if let Some(excerpt) = item.excerpt() {
                let truncated = crate::neppy::util::truncate_with_ellipsis(&excerpt, 500);
    ...
    fn render_markdown(&self, results: &[ExaResultItem], heading: &str, limit: usize) -> String {
        ...
        for item in results.iter().take(limit) {
            ...
            if let Some(excerpt) = item.excerpt() {
                let truncated = crate::neppy::util::truncate_with_suffix(&excerpt, 500, "...");
