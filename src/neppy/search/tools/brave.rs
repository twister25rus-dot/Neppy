...
async fn brave_get(
    ...
    if !status.is_success() {
        let detail = crate::neppy::util::utf8_safe_prefix_at_byte_boundary(&body, 500);
        tracing::warn!(
            path = %path,
            status = %status,
...
fn render_web_plain(results: &[WebResult], query: &str, max: usize) -> String {
    ...
    for (i, r) in results.iter().take(max).enumerate() {
        ...
        if !desc.is_empty() {
            lines.push(format!(
                "   {}",
                crate::neppy::util::truncate_with_ellipsis(desc, 500)
            ));
        }
    }
    lines.join("\n")
...
fn render_web_markdown(results: &[WebResult], query: &str, max: usize) -> String {
    ...
    for r in results.iter().take(max) {
        ...
        if !desc.is_empty() {
            out.push_str(&format!(
                "> {}\n",
                crate::neppy::util::truncate_with_suffix(desc, 500, "…")
            ));
        }
    }
    out
