use crate::neppy::config::{Config, SearchEngine};
...
fn build_backend_search_tools(root_config: &Config) -> Vec<Box<dyn Tool>> {
    let Some(client) = crate::neppy::integrations::build_client(root_config) else {
    ...
    if root_config.integrations.tinyfish.is_active() {
        tools.push(Box::new(
            crate::neppy::search::tools::TinyFishSearchTool::new(Arc::clone(&client)),
        ));
        tools.push(Box::new(
            crate::neppy::search::tools::TinyFishFetchTool::new(Arc::clone(&client)),
        ));
        tools.push(Box::new(
            crate::neppy::search::tools::TinyFishAgentRunTool::new(Arc::clone(&client)),
        ));
