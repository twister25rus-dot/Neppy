...
use crate::neppy::tinyplace::{acknowledge_message, decrypt_envelope};
...
async fn ingest_one(
    ...
    let linked =
        crate::neppy::agent::orchestration::pairing::linked_agent_ids(&workspace_dir).await;
...
pub async fn drain_mailbox_once(config: &Config) -> Result<usize, String> {
    ...
    let client = crate::neppy::tinyplace::ops::global_state()
        .client()
        .await?;
    let signer = client
        .http()
