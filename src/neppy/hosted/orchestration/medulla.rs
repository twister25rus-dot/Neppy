...
use crate::neppy::config::{Config, MedullaClientConfig};
...
pub async fn run(
    ...
    let token =
        crate::neppy::security::credentials::session_support::require_live_session_token(config)?;
    ...
    let config = json_object([
        ...
        (
            ...
            config.verification.map(|v| {
                json!(match v {
                    crate::neppy::config::MedullaVerification::Remind => "remind",
                    crate::neppy::config::MedullaVerification::Off => "off",
                })
            }),
        ),
    ]);
...
mod tests {
    ...
    use crate::neppy::config::{MedullaCycleConfig, MedullaPromptOverrides};
