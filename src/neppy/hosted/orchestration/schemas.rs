...
use crate::neppy::config::{rpc as config_rpc, Config};
...
fn handle_send_master_message(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        crate::neppy::tinyplace::handle_tinyplace_signal_send_message(send_params)
            .await
...
fn handle_attention(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let approvals = attention::approval_signals(
            crate::neppy::security::approval::rpc::approval_list_pending()
                .await
...
/// (`crate::neppy::tinyplace::handle_tinyplace_*`), so there is no new
...
fn handle_self_identity(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let key_status = crate::neppy::tinyplace::handle_tinyplace_signal_key_status(Map::new())
            .await
        ...
        let reverse = if agent_id.is_empty() {
            ...
            match tokio::time::timeout(
                ...
                crate::neppy::tinyplace::handle_tinyplace_directory_reverse(rev_params),
            )
            .await
            {
        ...
        let card_published = if agent_id.is_empty() {
            ...
            matches!(
                tokio::time::timeout(
                    std::time::Duration::from_secs(5),
                    crate::neppy::tinyplace::handle_tinyplace_directory_get_agent(card_params),
                )
                .await,
                Ok(Ok(_))
            )
...
fn handle_publish_identity(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        crate::neppy::tinyplace::handle_tinyplace_signal_register_encryption_key(Map::new())
            .await
        ...
        crate::neppy::tinyplace::handle_tinyplace_signal_provision(Map::new())
            .await
...
fn handle_relay_info(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let (base_url, network) = crate::neppy::tinyplace::ops::relay_endpoint();
