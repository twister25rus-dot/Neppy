...
impl Tool for ListContactsTool {
    ...
    async fn execute(&self, _args: Value) -> anyhow::Result<ToolResult> {
        match crate::neppy::tinyplace::handle_tinyplace_contacts_list(serde_json::Map::new()).await
        {
            Ok(v) => {
...
impl Tool for SendToAgentTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        ...
        // empty.
        let linked =
            crate::neppy::agent::orchestration::pairing::linked_agent_ids(&workspace).await;
        ...
        let accepted_contact = if linked.contains(&recipient) || known_session.is_some() {
            ...
            match crate::neppy::agent::orchestration::pairing::is_accepted_contact(&recipient).await
            {
        ...
        if let Err(e) =
            crate::neppy::tinyplace::handle_tinyplace_signal_send_message(send_params).await
        {
