...
fn resolve_model(model: &str) -> String {
    ...
    if trimmed.is_empty() {
        ...
        crate::neppy::config::MODEL_REASONING_V1.to_string()
    } else {
        trimmed.to_string()
    }
}
...
fn project_managed_usage(mut response: ModelResponse) -> ModelResponse {
    ...
    response.raw = crate::neppy::agent::tinyagents::model::merge_neppy_usage_meta(
        response.raw,
        charged_amount_usd,
        context_window,
    );
...
/// [`require_live_session_token`](crate::neppy::security::credentials::session_support::require_live_session_token)'s
...
fn maybe_publish_session_expired(err: &TinyAgentsError, operation: &str) {
    if let TinyAgentsError::Provider(pe) = err {
        if pe.provider.as_str() == "Neppy" && matches!(pe.status, Some(401 | 403)) {
            let reason = crate::neppy::inference::provider::ops::sanitize_api_error(&pe.message);
            crate::core::bus::BUS.publish(crate::core::events::DomainEvent::SessionExpired {
                source: format!(
                    "neppy_backend_model.{}({})",
                    operation,
    ...
    match err {
        TinyAgentsError::Provider(pe) => {
            log::warn!(
                ...
                pe.provider,
                pe.retryable,
                crate::neppy::inference::provider::ops::sanitize_api_error(&pe.message),
            );
        }
        other => {
            log::warn!(
...
mod tests {
    ...
    fn resolve_model_normalizes_blank_and_trims_non_empty_values() {
        assert_eq!(resolve_model(""), crate::neppy::config::MODEL_REASONING_V1);
        assert_eq!(
            resolve_model(" \t\n"),
            crate::neppy::config::MODEL_REASONING_V1
        );
    ...
    fn project_managed_usage_recovers_charged_and_cached() {
        use crate::neppy::agent::tinyagents::model::usage_info_from_response;
