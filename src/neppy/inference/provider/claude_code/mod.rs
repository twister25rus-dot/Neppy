...
/// parent.
pub fn workspace_dir_from_config(config: &crate::neppy::config::Config) -> PathBuf {
    config
        .config_path
        .parent()
        .map(PathBuf::from)
...
fn map_model_error(error: anyhow::Error) -> TinyAgentsError {
    ...
    if crate::neppy::inference::provider::error_classify::is_non_retryable(&error) {
...
impl ChatModel<()> for ClaudeCodeProvider {
    ...
    async fn invoke(
        ...
        let messages = crate::neppy::agent::tinyagents::model::native_chat_messages(&request);
        let response = self
            .run_chat(
                ChatRequest {
                    messages: &messages,
        ...
        Ok(crate::neppy::agent::tinyagents::model::native_model_response(&response))
    }
    ...
    async fn stream(&self, _state: &(), request: ModelRequest) -> tinyagents::Result<ModelStream> {
        ...
        let handle = tokio::spawn(async move {
                ...
                tokio::select! {
                    delta = delta_rx.recv() => {
                        if let Some(delta) = delta {
                            crate::neppy::agent::tinyagents::model::forward_provider_delta(
                                &item_tx,
                                delta,
                            );
                        }
            ...
            while let Ok(delta) = delta_rx.try_recv() {
                crate::neppy::agent::tinyagents::model::forward_provider_delta(&item_tx, delta);
            }

            let terminal = match response {
                Ok(response) => ModelStreamItem::Completed(
                    crate::neppy::agent::tinyagents::model::native_model_response(&response),
                ),
                Err(error) => ModelStreamItem::Failed(map_model_error(error).to_string()),
            };
            let _ = item_tx.send(terminal);
        });
        let guard = crate::neppy::agent::tinyagents::abort_guard::AbortOnDrop::new(handle, label);
        let stream =
