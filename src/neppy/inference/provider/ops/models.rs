...
pub async fn list_configured_models_from_config(
    ...
    let looked_up =
        crate::neppy::inference::provider::factory::lookup_key_for_slug(&entry.slug, config)
            .unwrap_or_default();
    let api_key = resolve_local_runtime_key(&entry.slug, looked_up, config);

    let bearer_is_oauth = entry.slug == "openai"
        && crate::neppy::inference::provider::factory::openai_bearer_is_oauth(config);
    let routing =
    ...
    let client = crate::neppy::config::build_runtime_proxy_client_with_timeouts(
    ...
    if !status.is_success() {
        ...
        let truncated = crate::neppy::util::truncate_with_ellipsis(&sanitized, 300);
    ...
    let body: serde_json::Value = serde_json::from_str(&raw_body).map_err(|e| {
        ...
        let snippet = crate::neppy::util::truncate_with_ellipsis(&sanitized, 300);
        format!(
...
/// [`crate::neppy::config::schema::cloud_providers::is_slug_reserved`]),
...
pub fn synthesize_local_runtime_entry(
    slug: &str,
    config: &crate::neppy::config::Config,
) -> Option<crate::neppy::config::schema::cloud_providers::CloudProviderCreds> {
    ...
    let endpoint = match slug {
        ...
        "ollama" => {
            let base = crate::neppy::inference::local::ollama_base_url_from_config(config);
            format!("{}/v1", base.trim_end_matches('/'))
        }
        // `lm_studio_base_url` already ends in `/v1`.
        "lmstudio" => crate::neppy::inference::local::lm_studio::lm_studio_base_url(config),
        _ => return None,
    };
