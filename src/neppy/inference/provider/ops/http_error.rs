...
use crate::neppy::inference::provider::neppy_backend_model;
...
pub fn is_budget_exhausted_http_400(status: reqwest::StatusCode, body: &str) -> bool {
    status == reqwest::StatusCode::BAD_REQUEST
        && crate::neppy::inference::provider::is_budget_exhausted_message(body)
}
...
pub fn is_backend_error_code_owned(provider: &str, body: &str) -> bool {
    provider == neppy_backend_model::PROVIDER_LABEL
        && crate::neppy::inference::provider::backend_error_code_skips_sentry(body)
}
...
pub fn log_backend_error_code_owned(
    ...
    let code = crate::neppy::inference::provider::extract_backend_error_code_token(body)
        .unwrap_or_default();
    tracing::info!(
        domain = "llm_provider",
        operation = operation,
...
/// [`crate::neppy::inference::provider::ChatRequest::max_tokens`]): a 402
...
pub fn is_provider_config_rejection_http(
    ...
    if !crate::neppy::inference::provider::is_provider_config_rejection_message(body) {
        return false;
    }
    ...
    if crate::neppy::inference::provider::is_openai_compatible_unknown_model_message(body) {
        return true;
    }
...
pub fn log_byo_provider_auth_failure(
    ...
    if crate::neppy::inference::auth_error_registry::record(provider, status_code) {
        crate::core::bus::BUS.publish(crate::core::events::DomainEvent::ProviderApiKeyRejected {
            provider: provider.to_string(),
            message: crate::neppy::inference::auth_error_registry::auth_error_message(
                provider,
                status_code,
            ),
        });
