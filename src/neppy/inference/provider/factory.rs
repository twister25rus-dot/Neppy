...
pub fn resolve_model_for_hint(hint_or_tier: &str, config: &Config) -> String {
    let hint_to_tier: &[(&str, &str)] = &[
        ("reasoning", crate::neppy::config::MODEL_REASONING_V1),
        ("chat", crate::neppy::config::MODEL_CHAT_V1),
        ("agentic", crate::neppy::config::MODEL_AGENTIC_V1),
        ("burst", crate::neppy::config::MODEL_BURST_V1),
        ("coding", crate::neppy::config::MODEL_CODING_V1),
        ("vision", crate::neppy::config::MODEL_VISION_V1),
        (
            "summarization",
            crate::neppy::config::MODEL_SUMMARIZATION_V1,
        ),
        ...
        ("subconscious", crate::neppy::config::MODEL_CHAT_V1),
    ];
    let tier_to_role: &[(&str, &str)] = &[
        (crate::neppy::config::MODEL_REASONING_V1, "reasoning"),
        (crate::neppy::config::MODEL_CHAT_V1, "chat"),
        (crate::neppy::config::MODEL_REASONING_QUICK_V1, "chat"),
        (crate::neppy::config::MODEL_AGENTIC_V1, "agentic"),
        (crate::neppy::config::MODEL_BURST_V1, "burst"),
        (crate::neppy::config::MODEL_CODING_V1, "coding"),
        (crate::neppy::config::MODEL_VISION_V1, "vision"),
        (
            crate::neppy::config::MODEL_SUMMARIZATION_V1,
            "summarization",
        ),
...
/// the constants in `crate::neppy::config`; the four `hint:*` strings that
...
/// ([`crate::neppy::inference::model_context::model_vision_enabled`]).
    ...
    if s.is_empty() || s == "cloud" {
        ...
        if super::fallback_diagnostics::role_falls_back_to_cloud(role) {
            if let Some(chat) = config.chat_provider.as_deref() {
                if crate::neppy::inference::local::profile::is_local_provider_string(chat) {
                    log::info!(
...
fn route_has_usable_credentials(resolved: &str, config: &Config) -> bool {
    ...
    if crate::neppy::inference::local::profile::is_local_provider_string(r)
        || r.starts_with(crate::neppy::inference::provider::claude_code::PROVIDER_PREFIX)
...
fn external_provider_label(provider: &str) -> String {
    ...
    if p.starts_with(crate::neppy::inference::provider::claude_code::PROVIDER_PREFIX) {
...
fn local_only_violation(mode: crate::neppy::config::PrivacyMode, provider: &str) -> Option<String> {
    ...
    if crate::neppy::inference::local::profile::is_local_provider_string(p) {
        return None;
    }
...
fn enforce_local_only_inference(role: &str, provider: &str) -> anyhow::Result<()> {
    let mode = crate::neppy::security::live_policy::current_privacy_mode();
...
/// [`emit_external_transfer`](crate::neppy::security::egress::emit_external_transfer)
...
fn emit_inference_egress(role: &str, provider: &str) {
    ...
    let is_local = crate::neppy::inference::local::profile::is_local_provider_string(p);
    ...
    crate::neppy::security::egress::emit_external_transfer(
        crate::neppy::security::egress::EgressDescriptor::inference(slug, service, !is_local),
    );
}
...
fn resolve_managed_backend_with_model_override(
    ...
    let model = match model.strip_prefix("hint:") {
        Some("reasoning") => crate::neppy::config::MODEL_REASONING_V1.to_string(),
        Some("chat") => crate::neppy::config::MODEL_CHAT_V1.to_string(),
        Some("agentic") => crate::neppy::config::MODEL_AGENTIC_V1.to_string(),
        Some("burst") => crate::neppy::config::MODEL_BURST_V1.to_string(),
        Some("coding") => crate::neppy::config::MODEL_CODING_V1.to_string(),
        Some("summarization") => crate::neppy::config::MODEL_SUMMARIZATION_V1.to_string(),
        Some("vision") => crate::neppy::config::MODEL_VISION_V1.to_string(),
        Some(_) => {
    ...
    crate::neppy::security::egress::emit_external_transfer(
        crate::neppy::security::egress::EgressDescriptor::inference("openhuman", &model, true),
    );
    Ok((
...
/// [`TurnModelSource`](crate::neppy::agent::tinyagents::TurnModelSource) to construct
...
fn try_create_claude_code_chat_model_from_string(
    ...
    let provider = provider.trim();
    let model_with_temp =
        provider.strip_prefix(crate::neppy::inference::provider::claude_code::PROVIDER_PREFIX)?;
    ...
    let workspace =
        crate::neppy::inference::provider::claude_code::workspace_dir_from_config(config);
    let effective_model = model_override.unwrap_or(&configured_model).to_string();
    emit_inference_egress(
        role,
        &format!(
            "{}{effective_model}",
            crate::neppy::inference::provider::claude_code::PROVIDER_PREFIX
        ),
    );
    let chat = match crate::neppy::inference::provider::claude_code::ClaudeCodeProvider::from_env(
        effective_model,
        workspace,
...
fn try_create_local_runtime_chat_model_from_string(
    ...
    if let Some(rest) = p.strip_prefix(LM_STUDIO_PROVIDER_PREFIX) {
        ...
        let endpoint = crate::neppy::inference::local::lm_studio::lm_studio_base_url(config);
...
/// author gate (issue B45, `neppy::flows::ops::evaluate_inference_readiness`)
...
fn cloud_entry_provider_string(
    ...
    let model = entry
        ...
        .unwrap_or(crate::neppy::config::DEFAULT_MODEL);
...
struct CloudSlugResolution<'a> {
    entry: &'a crate::neppy::config::schema::cloud_providers::CloudProviderCreds,
    effective_model: String,
    key: String,
    codex: crate::neppy::inference::provider::openai_codex::OpenAiCodexRouting,
}
...
fn resolve_cloud_slug<'a>(
    ...
    let local_chat = if implicit_fallback {
        config
            .chat_provider
            .as_deref()
            .filter(|chat| crate::neppy::inference::local::profile::is_local_provider_string(chat))
    } else {
        None
    };
...
fn try_create_cloud_slug_chat_model_from_string_with_native_tools(
    ...
    if p == PROVIDER_OPENHUMAN
        ...
        || p.starts_with(crate::neppy::inference::provider::claude_code::PROVIDER_PREFIX)
    {
        return None;
    }
    let colon = p.find(':')?;
    ...
    crate::neppy::security::egress::emit_external_transfer(
        crate::neppy::security::egress::EgressDescriptor::inference(&slug, &effective_model, true),
    );
...
pub(crate) fn openai_bearer_is_oauth(config: &Config) -> bool {
    use crate::neppy::security::credentials::profiles::AuthProfileKind;
    ...
    crate::neppy::inference::openai_oauth::lookup_openai_oauth_credentials(config)
        .ok()
        .flatten()
        .is_some()
}
...
pub fn lookup_key_for_slug(slug: &str, config: &Config) -> anyhow::Result<String> {
    ...
    if slug == crate::neppy::config::schema::EPHEMERAL_ROUTE_SLUG {
    ...
    if slug == "openai" {
        match crate::neppy::inference::openai_oauth::lookup_openai_bearer_token(config) {
