...
pub async fn install_from_config(config: &crate::neppy::config::Config) -> Result<(), String> {
...
#[cfg(feature = "modules")]
pub(super) async fn proxy(config: &crate::neppy::config::Config) -> Result<tinybus::Proxy, String> {
    let config = {
    ...
    crate::neppy::modules::ensure_loaded(config, "tinyjuice").await?;
    let record = crate::neppy::modules::registry::find("tinyjuice")
        .ok_or_else(|| "unknown module 'tinyjuice'".to_string())?;
    crate::neppy::modules::host::runtime()
        .await
        .map_err(|e| e.to_string())?
...
pub(super) async fn proxy(
    _config: &crate::neppy::config::Config,
