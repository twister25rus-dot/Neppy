...
use crate::neppy::inference::local as local_runtime;
use crate::neppy::inference::local::ops::ReactionDecision;
use crate::neppy::inference::provider as providers;
use crate::neppy::inference::{device, presets, sentiment, SentimentResult};
use crate::neppy::inference::{LocalAiEmbeddingResult, LocalAiStatus};
...
/// `src/neppy/inference/provider/ops.rs:54`
...
pub async fn inference_test_provider_model(
    ...
    let (chat_model, model) = if local {
        ...
        crate::neppy::inference::provider::create_chat_model_from_string_with_model_id(
            workload,
            provider,
            config,
...
pub async fn inference_provider_auth_errors() -> Result<RpcOutcome<Value>, String> {
    let errors = crate::neppy::inference::auth_error_registry::snapshot();
...
pub async fn inference_openai_oauth_start(config: &Config) -> Result<RpcOutcome<Value>, String> {
    ...
    let result = crate::neppy::inference::openai_oauth::start_openai_oauth(config).map(|start| {
        RpcOutcome::single_log(
            json!({
...
pub async fn inference_openai_oauth_complete(
    ...
    let result = crate::neppy::inference::openai_oauth::complete_openai_oauth(config, callback_url)
        .await
...
pub async fn inference_openai_oauth_import_codex_cli(
    ...
    let result = crate::neppy::inference::openai_oauth::import_openai_oauth_from_codex_cli(config)
...
pub async fn inference_openai_oauth_status(config: &Config) -> Result<RpcOutcome<Value>, String> {
    ...
    let result = crate::neppy::inference::openai_oauth::openai_oauth_status(config).map(|status| {
        RpcOutcome::single_log(
            json!({
...
pub async fn inference_openai_oauth_disconnect(
    ...
    let result = crate::neppy::inference::openai_oauth::disconnect_openai_oauth(config)
