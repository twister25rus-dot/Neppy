...
use crate::neppy::security::credentials::{AuthService, APP_SESSION_PROVIDER};
...
fn default_state_dir() -> PathBuf {
    ...
    let root = crate::neppy::config::default_root_neppy_dir().unwrap_or_else(|error| {
        log::warn!(
    ...
    let user_id = crate::neppy::config::read_active_user_id(&root);
    log::debug!(
...
fn env_workspace_state_dir(workspace: &std::path::Path) -> PathBuf {
    let (config_dir, _workspace_dir) =
        crate::neppy::config::resolve_config_dir_for_workspace(workspace);
    config_dir
}
...
fn user_scoped_state_dir(root: &std::path::Path, active_user_id: Option<&str>) -> PathBuf {
    crate::neppy::config::user_neppy_dir(
        root,
        active_user_id.unwrap_or(crate::neppy::config::PRE_LOGIN_USER_ID),
    )
}
...
impl EmbeddingProvider for NeppyCloudEmbedding {
    ...
    async fn embed(&self, texts: &[&str]) -> anyhow::Result<Vec<Vec<f32>>> {
        ...
        let egress = crate::neppy::security::egress::EgressDescriptor::embedding(
            "cloud",
            self.inner.model_id(),
        );
...
mod tests {
    ...
    fn default_scope_falls_back_to_the_pre_login_user_dir() {
        ...
        assert_eq!(
            resolved,
            root.join("users")
                .join(crate::neppy::config::PRE_LOGIN_USER_ID),
