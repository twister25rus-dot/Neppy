...
pub async fn update_settings(
    ...
    use crate::neppy::inference::embeddings::format_embedding_signature;
    ...
    if dims_changed {
        ...
        crate::neppy::memory::read_rpc::wipe_all_rpc(&config)
            .await
    ...
    if sig_changed {
        crate::neppy::memory::ops::maintenance::reembed_best_effort(&config, "embedding settings")
            .await;
    }
    ...
    let requeue_result = if is_embedding_remediation {
        crate::neppy::memory::ops::maintenance::retry_failed(&config).await
    } else {
        Ok(0)
    };
...
pub async fn set_api_key(
    ...
    let requeue_result = crate::neppy::memory::ops::maintenance::retry_failed(config).await;
    let requeued_count = *requeue_result.as_ref().unwrap_or(&0);
    let requeue_error = requeue_result.as_ref().err().cloned();
    let requeued_note = match &requeue_error {
        None => requeued_count.to_string(),
