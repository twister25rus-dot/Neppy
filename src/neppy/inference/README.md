...
| `voice/`                                                                          | Inference implementations imported by `crate::neppy::voice`.                                                                                                                                                                                   |
...
- `crate::neppy::config` — `Config`, `config::rpc` (load/save, `ModelSettingsPatch`, `LocalAiSettingsPatch`), cloud-provider schema (`AuthStyle`, slug reservation, id generation), abstract tier model constants. Heaviest dependency.
- `crate::neppy::security::credentials` — `AuthService`, `AuthProfilesStore`/`AuthProfile`/`TokenSet`, state dir — for OAuth token storage and provider auth resolution.
- `crate::neppy::tools` — tool schemas and product tool metadata projected into TinyAgents requests.
- `crate::neppy::agent::tinyagents` — native model, route, message, usage, and thread-context seams used by the agent harness.
- `crate::neppy::voice` — voice RPC/audio layer that imports these inference STT/TTS implementations (also a consumer).
- `crate::neppy::security::prompt_injection` — prompt-injection handling on the inference path.
- `crate::neppy::util` — small shared helpers.
