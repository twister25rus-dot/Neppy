...
use crate::neppy::inference::local::install::{
    find_system_ollama_binary, run_ollama_install_script,
};
use crate::neppy::inference::local::process_util::apply_no_window;
use crate::neppy::inference::paths::find_workspace_ollama_binary;

use super::super::LocalAiService;

impl LocalAiService {
    pub(in crate::neppy::inference::local::service) async fn resolve_or_install_ollama_binary(
        &self,
        config: &Config,
    ...
    pub(in crate::neppy::inference::local::service) async fn command_works(
        &self,
        command: &Path,
    ) -> bool {
    ...
    pub(in crate::neppy::inference::local::service) async fn download_and_install_ollama(
        &self,
        config: &Config,
    ) -> Result<(), String> {
        let install_dir = crate::neppy::inference::paths::workspace_ollama_dir(config);
        ...
        if crate::neppy::inference::local::install::is_ollama_installer_running() {
            log::info!(
            ...
            while crate::neppy::inference::local::install::is_ollama_installer_running() {
