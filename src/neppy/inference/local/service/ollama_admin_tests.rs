...
use crate::neppy::inference::local::service::LocalAiService;
...
async fn diagnostics_ok_when_expected_models_are_present() {
    ...
    let config = Config::default();
    let chat = crate::neppy::inference::model_ids::effective_chat_model_id(&config);
    let embedding = crate::neppy::inference::model_ids::effective_embedding_model_id(&config);
...
async fn kill_ollama_server_kills_owned_child() {
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    assert!(
        crate::neppy::inference::local::service::spawn_marker::pid_is_alive(pid),
    ...
    for _ in 0..40 {
        if !crate::neppy::inference::local::service::spawn_marker::pid_is_alive(pid) {
            still_alive = false;
            break;
        }
...
async fn shutdown_owned_ollama_clears_marker_and_kills_child() {
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    let marker_path = crate::neppy::inference::paths::ollama_spawn_marker_path(&config);
    let marker_writable = marker_path.starts_with(tmp.path());
    if marker_writable {
        crate::neppy::inference::local::service::spawn_marker::write_marker_at(
            &marker_path,
            &crate::neppy::inference::local::service::spawn_marker::OllamaSpawnMarker::new(
                pid,
