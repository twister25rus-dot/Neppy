...
use crate::neppy::inference::device::DeviceProfile;
use crate::neppy::inference::local::provider::{provider_from_config, LocalAiProvider};
use crate::neppy::inference::model_ids;
use crate::neppy::inference::presets::{self, VisionMode};
...
impl LocalAiService {
    ...
    pub async fn bootstrap(&self, config: &Config) {
        ...
        let device = crate::neppy::inference::device::detect_device_profile();
...
fn config_with_recommended_tier_if_unselected(config: &Config, device: &DeviceProfile) -> Config {
    let current_tier = crate::neppy::inference::presets::current_tier_from_config(&config.local_ai);
    ...
    if !config.local_ai.opt_in_confirmed {
        tracing::debug!(
            ...
            min_required_gb = crate::neppy::inference::presets::MIN_RAM_GB_FOR_LOCAL_AI,
            ?current_tier,
...
fn format_degraded_warning(err: &str, config: &Config) -> String {
    let current = crate::neppy::inference::presets::current_tier_from_config(&config.local_ai);
    match current {
        crate::neppy::inference::presets::ModelTier::Ram16PlusGb => {
            format!(
        ...
        crate::neppy::inference::presets::ModelTier::Ram8To16Gb => {
            format!(
        ...
        crate::neppy::inference::presets::ModelTier::Ram4To8Gb => format!(
...
mod tests {
    ...
    fn bootstrap_honors_opt_in_on_low_ram_device() {
        ...
        crate::neppy::inference::presets::apply_preset_to_config(
            &mut config.local_ai,
            crate::neppy::inference::presets::ModelTier::Ram2To4Gb,
        );
        let device = test_device(4);
