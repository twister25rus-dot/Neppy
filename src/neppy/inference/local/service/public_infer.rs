...
use crate::neppy::inference::parse::sanitize_inline_completion;
...
impl LocalAiService {
    ...
    pub(crate) async fn chat_with_history(
        &self,
        config: &Config,
        messages: Vec<crate::neppy::inference::local::ollama::OllamaChatMessage>,
        max_tokens: Option<u32>,
    ) -> Result<String, String> {
    ...
    async fn chat_with_history_internal(
        ...
        let _gate_permit = if gated {
            crate::neppy::cron::scheduler_gate::wait_for_capacity().await
        } else {
            None
        };
