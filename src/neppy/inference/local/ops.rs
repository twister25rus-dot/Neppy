...
use crate::neppy::inference::local as local_ai;
use crate::neppy::inference::provider as providers;
use crate::neppy::inference::{
...
fn grant_turn_cwd(config: &mut Config, root: &std::path::Path) {
    ...
    config
        .autonomy
        .trusted_roots
        .push(crate::neppy::security::TrustedRoot {
            path,
            access: crate::neppy::security::TrustedAccess::ReadWrite,
        });
}
...
fn effective_agent_chat_origin() -> crate::neppy::agent::turn_origin::AgentTurnOrigin {
    crate::neppy::agent::turn_origin::current()
        .unwrap_or(crate::neppy::agent::turn_origin::AgentTurnOrigin::Cli)
}
...
/// [`ProgressSink`](crate::neppy::agent::progress_sink::ProgressSink) around
...
pub async fn agent_chat(
    ...
    cwd: Option<String>,
    route: Option<crate::neppy::config::schema::EphemeralRoute>,
    ...
    if let Some(route) = route {
        crate::neppy::config::schema::ephemeral_route::apply(config, route);
    }
    ...
    if let Some(tx) = crate::neppy::agent::progress_sink::current_progress_sink() {
    ...
    let run = crate::neppy::agent::turn_origin::with_origin(
    ...
    let response = match thread_id.as_deref() {
        Some(id) if !id.trim().is_empty() => {
            ...
            crate::neppy::agent::tinyagents::thread_context::with_thread_id(id, run).await
        }
        _ => {
...
pub async fn agent_chat_simple(
    ...
    let default_model = effective
        .default_model
        .clone()
        .unwrap_or_else(|| crate::neppy::config::DEFAULT_MODEL.to_string());
...
pub async fn local_ai_chat(
    ...
    let mut ollama_messages: Vec<crate::neppy::inference::local::ollama::OllamaChatMessage> =
    ...
    for msg in messages.into_iter() {
        ...
        ollama_messages.push(crate::neppy::inference::local::ollama::OllamaChatMessage {
