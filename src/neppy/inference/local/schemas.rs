...
///
/// A near-copy of the params in [`crate::neppy::agent::schemas`], which
...
struct AgentChatParams {
    ...
    /// [`ephemeral_route`](crate::neppy::config::schema::ephemeral_route).
    #[serde(default)]
...
fn handle_agent_chat(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::inference::local::ops::agent_chat(
                &mut config,
                &p.message,
                p.model_override,
                p.temperature,
                p.thread_id,
                p.cwd,
                crate::neppy::config::schema::EphemeralRoute::from_params(
                    p.inference_url,
                    p.api_key,
                ),
            )
...
fn handle_agent_chat_simple(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::inference::local::ops::agent_chat_simple(
                &config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_local_ai_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::neppy::inference::local::ops::local_ai_transcribe(&config, p.audio_path.trim())
                .await?,
        )
    })
}
...
fn handle_local_ai_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::neppy::inference::local::ops::local_ai_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
            )
...
fn handle_local_ai_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::neppy::inference::local::ops::local_ai_tts(
                &config,
                &p.text,
...
fn handle_local_ai_assets_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::neppy::inference::local::ops::local_ai_assets_status(&config).await?)
    })
}
...
fn handle_local_ai_downloads_progress(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::neppy::inference::local::ops::local_ai_downloads_progress(&config).await?)
    })
}
...
fn handle_local_ai_download_asset(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::neppy::inference::local::ops::local_ai_download_asset(
                &config,
                p.capability.trim(),
            )
            .await?,
...
fn handle_local_ai_install_piper(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let slot =
            match crate::neppy::inference::local::voice_install_common::try_acquire_install_slot(
                crate::neppy::inference::local::voice_install_common::ENGINE_PIPER,
            ) {
                Some(slot) => slot,
                None => {
                    tracing::debug!(
                    ...
                    let current = crate::neppy::inference::local::voice_install_common::read_status(
                        crate::neppy::inference::local::voice_install_common::ENGINE_PIPER,
                    );
        ...
        crate::neppy::inference::local::voice_install_common::write_status(
            crate::neppy::inference::local::voice_install_common::VoiceInstallStatus {
                engine: crate::neppy::inference::local::voice_install_common::ENGINE_PIPER.to_string(),
                state:
                    crate::neppy::inference::local::voice_install_common::VoiceInstallState::Installing,
                progress: Some(0),
        ...
        tokio::spawn(async move {
            let _slot = slot;
            if let Err(e) = crate::neppy::inference::local::install_piper::install_piper(
        ...
        let status = crate::neppy::inference::local::voice_install_common::read_status(
            crate::neppy::inference::local::voice_install_common::ENGINE_PIPER,
        );
...
fn handle_local_ai_test_connection(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let result =
            crate::neppy::inference::local::service::ollama_admin::test_ollama_connection(&p.url)
                .await?;
...
fn handle_local_ai_piper_install_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let status = crate::neppy::inference::local::install_piper::status(&config);
        serde_json::to_value(status).map_err(|e| format!("serialize piper status: {e}"))
    })
}
