...
`src/neppy/tools/impl/system/` and consume this module's `NodeBootstrap`.
...
- `crate::neppy::modules::runtime` — the module client this delegates to.
- `crate::neppy::config` — the settings each request carries.
- `crate::neppy::tools`, `security`, `agent::host_runtime`, `memory` — the
...
- `src/neppy/runtime/javascript/mod.rs` — the public language slot.
- `src/neppy/tools/impl/system/{node_exec,npm_exec,shell}.rs` — hold an
  `Arc<NodeBootstrap>`; the exec tools call `resolve()`, `shell` uses the
  non-blocking `try_cached()`.
- `src/neppy/agent/harness_init/registry.rs` — the `node_runtime` init step
