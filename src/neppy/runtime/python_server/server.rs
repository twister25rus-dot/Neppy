...
use crate::neppy::runtime::python::process::PythonLaunchSpec;
...
async fn prepare_launch(config: &Config) -> Result<ServerLaunch> {
    ...
    let python_bin = if want_spacy {
        ...
        crate::neppy::runtime::python::PythonBootstrap::new(std::sync::Arc::new(config.clone()))
            .resolve()
            .await?
            .python_bin
    };
...
async fn spawn_inner(launch: &ServerLaunch) -> Result<ServerInner> {
    ...
    let resolved = crate::neppy::runtime::python::ResolvedPython {
        bin_dir: launch
            .python_bin
            .parent()
            .map(PathBuf::from)
        ...
        source: crate::neppy::runtime::python::PythonSource::Managed,
    };
    let mut spec = PythonLaunchSpec::new(launch.script_path.clone());
    for (key, value) in &launch.env {
        spec.env.insert(key.clone(), value.clone());
    }
    let mut child = crate::neppy::runtime::python::process::spawn_stdio_process(&resolved, &spec)
