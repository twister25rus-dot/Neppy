...
- `crate::neppy::inference::local::process_util` — the Windows no-console
...
- `src/neppy/runtime/python_server/` — resolves an interpreter, then spawns
  and supervises the long-lived model server with `spawn_stdio`.
- `src/neppy/tools/impl/system/{python_exec,shell}.rs` — hold an
  `Arc<PythonBootstrap>`; `python_exec` calls `resolve()`, `shell` uses the
  non-blocking `try_cached()` for `PATH` injection.
- `src/neppy/skills/runtime/ops.rs` — resolves an interpreter for
  Python-backed skills.
- `src/neppy/agent/harness_init/registry.rs` — the Python init step uses
