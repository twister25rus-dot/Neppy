...
| File | Role |
| --- | --- |
| `src/neppy/sandbox/cwd_jail/mod.rs` | Module docstring + the thin facade: `spawn` / `spawn_with` / `default_backend` (cached via `OnceLock`). Re-exports the public surface. Inline tests. |
| `src/neppy/sandbox/cwd_jail/jail.rs` | Core types: the `Jail` description struct (builder + `canonicalize`/`canonicalize_or_log`) and the `JailBackend` trait (`name`/`is_available`/`spawn`). |
| `src/neppy/sandbox/cwd_jail/detect.rs` | `pick_backend()` — cfg-gated platform selection; returns the first available backend or `NoopBackend`. |
| `src/neppy/sandbox/cwd_jail/noop.rs` | `NoopBackend` — no enforcement, plain `Command::spawn`. Always available. |
| `src/neppy/sandbox/cwd_jail/linux.rs` | `LandlockBackend` — kernel 5.13+ Landlock LSM applied in `pre_exec` (child-side, after fork, before exec). Gated on the `sandbox-landlock` cargo feature. |
| `src/neppy/sandbox/cwd_jail/macos.rs` | `SeatbeltBackend` — wraps the command in `/usr/bin/sandbox-exec -p '<profile>'`. Renders an allow-default-reads / deny-default-writes Seatbelt profile. |
| `src/neppy/sandbox/cwd_jail/windows.rs` | `AppContainerBackend` — `CreateAppContainerProfile` + DACL grant + `STARTUPINFOEX`/`CreateProcessW` via `windows-sys`. |
| `src/neppy/sandbox/cwd_jail/registry.rs` | `JailRegistry` + `JailRecord` — multi-jail manager persisted to `index.json`, with atomic-rename writes and containment checks. |
| `src/neppy/sandbox/cwd_jail/registry_tests.rs` | Sibling test suite for the registry (`#[path]`-included from `registry.rs`). |
...
This module is notably self-contained: its own files contain **no** `use crate::neppy::` or `use crate::core::` imports. Dependencies are external/std only:
...
The Linux backend's docstring references `crate::neppy::security::landlock` as conceptual prior art, but the implementation here is self-contained (it does not import that module).

## Used by

- Declared at `src/neppy/mod.rs:37` (`pub mod cwd_jail;`). No other `src/` Rust files currently reference `neppy::sandbox::cwd_jail` — it is a self-standing facade not yet wired into a calling domain.
