...
use super::*;
use crate::neppy::config::{Config, HttpHeader, McpAuthConfig, McpServerConfig as HostServer};
...
fn the_client_identity_survives_the_conversion() {
    ...
    let mut config = config_without_docs();
    config.mcp_client.client_identity.name = "neppy-core".into();
    ...
    assert_eq!(identity.name, "neppy-core");
...
fn a_credentialed_plaintext_non_loopback_endpoint_is_refused() {
    use crate::neppy::config::McpAuthConfig as Auth;
    let mut config = config_without_docs();
    config.mcp_client.servers.push(HostServer {
        name: "insecure".into(),
