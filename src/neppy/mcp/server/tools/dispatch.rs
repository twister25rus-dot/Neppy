...
use crate::neppy::inference::provider::types::build_tool_instructions_text;
...
async fn load_config_and_init_registry() -> Result<crate::neppy::config::Config, ToolCallError> {
...
async fn run_subagent_tool(params: &Map<String, Value>) -> Result<Value, ToolCallError> {
    ...
    let origin = crate::neppy::agent::turn_origin::AgentTurnOrigin::ExternalChannel {
    ...
    let response = Box::pin(subagent_depth::scope(
        child_depth,
        crate::neppy::agent::turn_origin::with_origin(origin, agent.run_single(&prompt)),
    ))
    .await
