...
| `src/neppy/mcp/server/stdio.rs` | CLI entry `run_stdio_from_cli` (arg parse: `--transport`/`--host`/`--port`/`--auth-token`/`-v`/`--help`), logging init, stdio read/write loop (`run_stdio`). |
| `src/neppy/mcp/server/tools_tests.rs` | Sibling `#[cfg(test)]` suite for `tools.rs` (via `#[path]`). |
...
It does **not** own any agent tools in the `tools.rs`/`src/neppy/tools` sense. The "tools" here are **MCP-protocol tools** advertised to external clients:
...
- `crate::neppy::config` (`Config`, `rpc::load_config_with_timeout`, `McpAuthConfig`/`McpClientIdentityConfig` in tests) — load config for policy/searxng gating and per-call config.
- `crate::neppy::security` (`SecurityPolicy`, `ToolOperation`) — enforce read/act autonomy policy per tool call.
- `crate::neppy::agent` (`Agent`, `agents::BUILTINS`, `harness::AgentDefinitionRegistry`) — build the orchestrator agent for `core.list_tools`/`core.tool_instructions`, list/run subagents, and cross-check the resource catalog.
- `crate::neppy::inference::provider::traits::build_tool_instructions_text` — render the markdown tool-use instructions block for `core.tool_instructions`.
- `crate::neppy::tools` (`SEARXNG_MAX_RESULTS`, `normalize_categories`) — SearXNG bounds + category normalization for `searxng_search`.
- `crate::neppy::mcp::audit` (`record_write`, `NewMcpWriteRecord`, list/query helpers in tests) — durable write-audit log.
- `crate::neppy::mcp::http_client::McpHttpClient` — round-trip test harness for the HTTP transport (test-only).
