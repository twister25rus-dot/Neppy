...
| File | Role |
| --- | --- |
| `src/neppy/mcp/registry/mod.rs` | Module docstring + exports; re-exports controller schema/registry pair and key types. |
| `src/neppy/mcp/registry/types.rs` | Domain types: `CommandKind`, `Transport` (Stdio / HttpRemote), `InstalledServer`, `McpTool`, `ServerStatus`, `ConnStatus`, Smithery DTOs, `ChatTurn`. |
| `src/neppy/mcp/registry/store.rs` | SQLite persistence (`mcp_clients/mcp_clients.db`): server CRUD, env values, registry cache; additive `transport`/`deployment_url` column migration. |
| `src/neppy/mcp/registry/registry.rs` | Multi-registry dispatch: `registry_search` (parallel fan-out + merge), `registry_get` (source-prefix routing or first-hit). |
| `src/neppy/mcp/registry/registries/mod.rs` | `Registry` trait + `enabled_registries` / `registry_for_source`; `SOURCE_*` constants. |
| `src/neppy/mcp/registry/registries/smithery.rs` | Smithery.ai adapter (`registry.smithery.ai`), 10-min SQLite cache, optional `SMITHERY_API_KEY` auth. |
| `src/neppy/mcp/registry/registries/mcp_official.rs` | Official modelcontextprotocol/registry adapter; cursor→page mapping with a bounded sequential cursor walk; optional `MCP_OFFICIAL_REGISTRY_*` env overrides. |
| `src/neppy/mcp/registry/connections.rs` | Global in-process connection registry; `connect`/`disconnect`/`call_tool`/`all_status`/`all_connected_tools`; dispatches stdio vs HTTP via `ActiveClient`. |
| `src/neppy/mcp/registry/boot.rs` | `spawn_installed_servers` — boot-time connect of all installs; per-server failures logged, never fatal. |
| `src/neppy/mcp/registry/supervisor.rs` | Background reconnect loop (#3312): every 60s probes each connected transport (`connections::probe_alive`) and reconnects dropped/never-connected enabled servers with per-server exponential backoff. Spawned once at startup after the boot pass; connectivity-only (publishes no health events). |
| `src/neppy/mcp/registry/ops.rs` | `mcp_clients_*` RPC handler implementations + `resolve_command` + `config_assist` inference call. |
| `src/neppy/mcp/registry/setup.rs` | Opaque secret-ref machinery (`SecretRef`, mint/fulfill/await/resolve/consume/gc) for the setup agent. |
| `src/neppy/mcp/registry/setup_ops.rs` | `mcp_setup_*` RPC handlers (search/get/request_secret/submit_secret/test_connection/install_and_connect) + connection `pick_connection`. |
| `src/neppy/mcp/registry/schemas.rs` | Controller schemas + `handle_*` dispatch for both `mcp_clients` and `mcp_setup` namespaces. |
| `src/neppy/mcp/registry/bus.rs` | `McpClientEventSubscriber` — logs lifecycle events; `init()` registers it. |
...
This module does not define `Tool` impls directly. The setup-agent tools live in `src/neppy/tools/impl/network/mcp_setup.rs` as thin wrappers calling `mcp::registry::setup_ops`. The MCP browse/list/call tools (`McpListServersTool`, `McpListToolsTool`, etc.) are wired in `src/neppy/tools/ops.rs` against the connected-tools surface, and `tool_registry` pulls live tools via `connections::all_connected_tools()`.
...
- `crate::neppy::config::Config` — workspace dir, `mcp_client.client_identity`, inference api_url/api_key/default_model.
- `crate::neppy::config::rpc` (`load_config_with_timeout`) — config load inside RPC handlers.
- `crate::neppy::mcp::config_servers` (`McpStdioClient`) and `crate::neppy::mcp::http_client` (`McpHttpClient`, `McpRemoteTool`, `McpServerToolResult`) — actual transport clients used by `connections.rs` and `setup_ops.rs`.
...
- `src/neppy/mcp/mod.rs` — declares `pub mod registry`.
...
- `src/neppy/tools/registry/ops.rs` — uses `connections` to surface MCP tools to agents.
- `src/neppy/tools/ops.rs` + `src/neppy/tools/impl/network/mcp_setup.rs` — agent tool wrappers.
- `src/neppy/platform/about_app/catalog.rs` — capability entry `channels.mcp_registry_browse`.
