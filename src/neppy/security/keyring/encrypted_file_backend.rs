...
use crate::neppy::security::keyring::backend::KeyringBackend;
use crate::neppy::security::keyring::crypto::{self, KEY_LEN};
use crate::neppy::security::keyring::error::KeyringError;
use crate::neppy::security::keyring::file_store;
use crate::neppy::security::keyring::store::BackendKind;
...
pub fn init_master_key() {
    ...
    let dir = crate::neppy::security::keyring::store::workspace_dir_for_file_backend();
    log::info!(
        "[keyring] init_master_key: resolved workspace_dir={}",
        dir.display()
    );
    crate::neppy::security::keyring::init_workspace(&dir);

    MASTER_KEY.get_or_init(|| {
        let backend_kind = crate::neppy::security::keyring::store::effective_backend_kind();
        ...
        match try_load_master_key() {
            ...
            Err(e) => {
                ...
                crate::neppy::security::keyring_consent::policy::notify_master_key_unavailable(&e);
                None
            }
        }
    });
