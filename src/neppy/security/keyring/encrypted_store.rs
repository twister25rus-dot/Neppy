...
impl SecretStore {
    ...
    fn load_or_create_key_from_keyring(&self) -> Result<Zeroizing<Vec<u8>>> {
        ...
        match crate::neppy::security::keyring::migrate_from_file(
            &user_id,
            KEYCHAIN_MASTER_KEY,
            &self.key_path,
        ) {
            Ok(crate::neppy::security::keyring::MigrationOutcome::MigratedAndDeleted) => {
                log::info!(
                    "[security] migrated legacy secret-store key into keychain from {}",
                    self.key_path.display()
                );
            }
            Ok(crate::neppy::security::keyring::MigrationOutcome::AlreadyMigrated)
            | Ok(crate::neppy::security::keyring::MigrationOutcome::NoSourceFile) => {}
            Err(error) => {
                log::warn!(
        ...
        let hex_key = crate::neppy::security::keyring::get_or_create_random(
            &user_id,
            KEYCHAIN_MASTER_KEY,
            KEY_LEN,
        )
