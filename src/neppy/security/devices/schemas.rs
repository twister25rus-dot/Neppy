...
fn handle_create_pairing(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::neppy::security::devices::rpc::devices_create_pairing(&config, label).await?)
    })
}
...
fn handle_list(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::neppy::security::devices::rpc::devices_list(&config).await?)
    })
}
...
fn handle_revoke(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::neppy::security::devices::rpc::devices_revoke(&config, channel_id).await?)
    })
}
