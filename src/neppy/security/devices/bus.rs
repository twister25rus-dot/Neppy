...
use crate::neppy::security::devices::crypto::{
    base64url_decode, base64url_encode, derive_session_keys, TunnelCipher, TunnelRole,
};
use crate::neppy::security::devices::rpc::{
    ACTIVE_CIPHERS, PEER_STATUS, PENDING_KEYPAIRS, PENDING_SESSIONS,
};
use crate::neppy::security::devices::store;
use crate::neppy::security::devices::tunnel_client::emit_frame;
...
async fn handle_tunnel_frame(channel_id: &str, payload_b64: &str) {
    ...
    let frame_bytes = match crate::neppy::security::devices::crypto::base64url_decode(payload_b64) {
        Ok(b) => b,
        Err(e) => {
            log::warn!(
    ...
    if frame_bytes.first() == Some(&crate::neppy::security::devices::crypto::FRAME_VERSION) {
    ...
    let handshake_payload = if frame_bytes.first() == Some(&0x01) {
        ...
        let dh_key = match core_keypair.derive_shared_secret(
            &crate::neppy::security::devices::crypto::base64url_encode(&eph_pub_bytes),
        ) {
            Ok(k) => k,
            Err(e) => {
                log::warn!(
    ...
    if let Ok(config) = crate::neppy::config::rpc::load_config_with_timeout().await {
