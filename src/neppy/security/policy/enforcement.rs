...
impl SecurityPolicy {
    ...
    pub fn from_config(
        autonomy_config: &crate::neppy::config::AutonomyConfig,
        workspace_dir: &Path,
        action_dir: &Path,
    ) -> Self {
        log::info!(
        ...
        let projects_path = crate::neppy::config::default_projects_dir()
            .to_string_lossy()
            .to_string();
        ...
        match ensure_neppy_scratch_dir() {
            Some(scratch) => {
        ...
        Self {
            ...
            privacy_mode: crate::neppy::config::PrivacyMode::default(),
    ...
    #[must_use]
    pub fn with_privacy_mode(mut self, privacy_mode: crate::neppy::config::PrivacyMode) -> Self {
        log::debug!(
    ...
    pub fn privacy_mode(&self) -> crate::neppy::config::PrivacyMode {
        self.privacy_mode
    }
}
...
pub fn neppy_scratch_dir() -> std::path::PathBuf {
    #[cfg(windows)]
    {
...
/// Create [`neppy_scratch_dir`] with restrictive perms, best-effort.
...
pub fn ensure_neppy_scratch_dir() -> Option<std::path::PathBuf> {
    let dir = neppy_scratch_dir();
...
mod scratch_dir_tests {
    use super::{ensure_neppy_scratch_dir, neppy_scratch_dir};
    ...
    fn ensure_scratch_dir_creates_and_returns_it() {
        ...
        let ensured = ensure_neppy_scratch_dir();
        let expected = neppy_scratch_dir();
