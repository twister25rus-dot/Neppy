...
fn config_default_policy_includes_windows_read_equivalents() {
    let cfg = crate::neppy::config::AutonomyConfig::default();
...
fn from_config_maps_all_fields() {
    let autonomy_config = crate::neppy::config::AutonomyConfig {
        ...
        ..crate::neppy::config::AutonomyConfig::default()
    };
...
fn policy_from_config_carries_auto_approve_all() {
    ...
    let enabled_config = crate::neppy::config::AutonomyConfig {
    ...
    let disabled_config = crate::neppy::config::AutonomyConfig {
...
fn expand_tilde_delegates_to_config_single_source_of_truth() {
    ...
    assert_eq!(
        ...
        crate::neppy::config::expand_tilde(input)
    );
...
fn validate_command_truncates_secrets_in_allowlist_miss_error() {
    ...
    assert!(
        err.starts_with(crate::neppy::security::POLICY_BLOCKED_MARKER),
...
fn from_config_grants_default_projects_dir_as_readwrite_root() {
    ...
    let projects = crate::neppy::config::default_projects_dir()
        .to_string_lossy()
        .to_string();
    assert!(
        policy
...
fn from_config_does_not_duplicate_user_granted_projects_root() {
    let projects = crate::neppy::config::default_projects_dir()
        .to_string_lossy()
        .to_string();
    let cfg = crate::neppy::config::AutonomyConfig {
...
async fn write_into_the_scoped_turn_root_is_allowed() {
    let (_root, checkout, policy) = turn_workspace_policy();
    let target = checkout.join("notes.md");
    let resolved = crate::neppy::agent::turn_workspace::with_workspace(checkout.clone(), {
        let policy = &policy;
...
async fn the_turn_root_grant_does_not_reach_a_sibling_directory() {
    ...
    let err = crate::neppy::agent::turn_workspace::with_workspace(checkout, {
        let policy = &policy;
        let target = target.clone();
        async move { policy.validate_parent_path(target.to_str().unwrap()).await }
    })
