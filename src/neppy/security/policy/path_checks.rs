...
impl SecurityPolicy {
    ...
    /// [`crate::neppy::config::expand_tilde`] — the single source of truth —
    ...
    pub(super) fn expand_tilde(&self, path: &str) -> String {
        crate::neppy::config::expand_tilde(path)
    }
    ...
    pub(super) fn check_cross_profile(&self, resolved: &Path) -> Result<(), String> {
        ...
        if let crate::neppy::agent::profiles::CrossProfileDecision::Block { other_id } =
            crate::neppy::agent::profiles::classify_cross_profile_target(
                &guard.action_dir,
                &guard.profile_id,
                resolved,
            )
    ...
    pub fn is_within_trusted_root(&self, path: &Path, require_write: bool) -> bool {
        ...
        if let Some(turn_root) = crate::neppy::agent::turn_workspace::current() {
