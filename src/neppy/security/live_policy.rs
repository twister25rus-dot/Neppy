...
/// [`config_set_action_dir`](crate::neppy::config::ops::set_action_dir)
...
pub fn reload_from(autonomy_config: &crate::neppy::config::AutonomyConfig) {
    let Some(state) = STATE.get() else {
        return;
    };
    let workspace = state
...
mod tests {
    use super::*;
    use crate::neppy::config::AutonomyConfig;
