...
use crate::neppy::util::retry_with_backoff;
...
pub struct AuthProfilesStore {
    ...
    /// retryable (`src/neppy/util.rs:618`). Production binaries never
    /// see this field.
    #[cfg(test)]
...
impl AuthProfilesStore {
    pub fn new(state_dir: &Path, encrypt_secrets: bool) -> Self {
        ...
        let use_keychain = policy
            == crate::neppy::security::keyring_consent::PolicyDecision::Proceed
        ...
        match policy {
            ...
            crate::neppy::security::keyring_consent::PolicyDecision::ConsentRequired => {
                log::warn!(
            ...
            crate::neppy::security::keyring_consent::PolicyDecision::Declined => {
                log::warn!(
    ...
    fn keychain_store_secrets(&self, profile: &AuthProfile) -> anyhow::Result<()> {
        ...
        crate::neppy::security::keyring::set(&self.user_id, &key, &payload).map_err(|e| {
            anyhow::anyhow!(
    ...
    fn keychain_load_secrets(&self, profile_id: &str) -> anyhow::Result<Option<KeychainSecrets>> {
        ...
        let payload = match crate::neppy::security::keyring::get(&self.user_id, &key) {
            Ok(Some(p)) => p,
            Ok(None) => {
                log::debug!(
    ...
    fn keychain_delete_secrets(&self, profile_id: &str) {
        let key = self.keychain_key_for_profile(profile_id);
        if let Err(e) = crate::neppy::security::keyring::delete(&self.user_id, &key) {
            log::warn!(
    ...
    fn acquire_lock(&self) -> Result<AuthProfileLockGuard> {
        ...
        loop {
            let open_result =
                crate::neppy::util::retry_with_backoff("create auth profile lock", 6, 100, || {
                    OpenOptions::new()
                        .create_new(true)
                        .write(true)
                        .open(&self.lock_path)
