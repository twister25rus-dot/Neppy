...
use crate::neppy::security::keyring::SecretStore;
...
pub async fn start_login_gated_services(config: &Config) {
        ...
        tasks.push((
            ...
            tokio::spawn(async move {
                if config.local_ai.runtime_enabled {
                    ...
                    crate::neppy::inference::local::global(&config)
                        .bootstrap(&config)
                        .await;
                    log::debug!(
    ...
    //      with everything else).
    {
        ...
        tasks.push((
            ...
            tokio::spawn(async move {
                let step = std::time::Instant::now();
                crate::neppy::voice::server::start_if_enabled(&config).await;
                if !config.voice_server.auto_start {
                    crate::neppy::voice::dictation_listener::start_if_enabled(&config).await;
                }
                log::debug!(
    ...
    //     command registration above.
    {
        ...
        tasks.push((
            ...
            tokio::spawn(async move {
                let step = std::time::Instant::now();
                crate::neppy::voice::always_on::start_if_enabled(&config).await;
                log::debug!(
    ...
    //    the previous behaviour exactly.
    {
        ...
        if hosted_enabled {
            ...
            tasks.push((
                ...
                tokio::spawn(async move {
                    let step = std::time::Instant::now();
                    crate::neppy::hosted::orchestration::start_hosted_client_services(&config)
                        .await;
                    log::debug!(
...
pub async fn stop_login_gated_services(config: &Config) {
    // 2. Voice server
    if let Some(server) = crate::neppy::voice::server::try_global_server() {
        server.stop().await;
    ...
    if config.local_ai.runtime_enabled {
        let service = crate::neppy::inference::local::global(config);
    ...
    crate::neppy::voice::dictation_listener::stop();
    ...
    crate::neppy::voice::always_on::stop();

    // 7. Orchestration hosted-client loops (read-sync + world-diff uploader).
    crate::neppy::hosted::orchestration::stop_hosted_client_services();
...
async fn store_session_inner(
    ...
    if !local_session {
        match decode_jwt_exp(trimmed_token) {
            Some(exp) => {
                metadata.insert(
                    crate::neppy::security::credentials::session_support::SESSION_EXPIRES_AT_META
                        .to_string(),
                    exp.to_rfc3339(),
                );
                tracing::info!(
    ...
    let effective_config = if resolved_user_id.is_some() {
        match crate::neppy::config::load_config_with_timeout().await {
            Ok(c) => c,
            Err(_) => config.clone(),
        }
    } else {
    ...
    if local_session {
        match crate::neppy::config::ops::set_onboarding_completed(false).await {
    ...
    crate::neppy::memory::ops::maintenance::reembed_best_effort(
        &effective_config,
        "session stored",
    )
    .await;
...
pub async fn clear_session(config: &Config) -> Result<RpcOutcome<serde_json::Value>, String> {
    ...
    if let Some(manager) = crate::neppy::platform::socket::global_socket_manager() {
...
fn clear_provider_auth_error(provider: &str) {
    ...
    crate::neppy::inference::auth_error_registry::clear(slug);
}
...
///
/// Used by [`crate::neppy::integrations::composio::client::create_composio_client`]
