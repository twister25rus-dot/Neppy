...
- **OS keychain** when available (`crate::neppy::security::keyring::is_available`): all token fields stored under key `auth:{profile_id}` namespaced by a per-user id derived from the state dir; JSON keeps no secret fields.
...
- `crate::neppy::memory` — bind memory client to the active workspace after login.
- `crate::neppy::subconscious` — post-login bootstrap / user-switch reset.
- `crate::neppy::inference`, `::voice`, `::autocomplete` — login-gated services started/stopped.
