...
pub fn check_secret_access() -> PolicyDecision {
    if crate::neppy::security::keyring::is_available() {
...
pub fn current_status() -> KeyringStatus {
    let available = crate::neppy::security::keyring::is_available();
    let backend_name = crate::neppy::security::keyring::backend_name();
...
pub fn retry_probe() -> KeyringStatus {
    ...
    crate::neppy::security::keyring::reset_availability_cache();
    CONSENT_EVENT_PUBLISHED.store(false, Ordering::SeqCst);
    current_status()
}
