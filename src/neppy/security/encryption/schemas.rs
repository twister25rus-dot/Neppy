...
fn handle_encrypt_secret(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::neppy::security::encryption::rpc::encrypt_secret(&config, &plaintext).await?)
    })
}
...
fn handle_decrypt_secret(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::security::encryption::rpc::decrypt_secret(&config, &ciphertext).await?,
        )
    })
}
