...
- `src/neppy/config/` — `Config` for workspace-relative data directory.
- `src/neppy/security/credentials/` — `credentials::rpc::{encrypt_secret, decrypt_secret}` carry the actual key-management responsibility.

## Called by

- `src/neppy/security/credentials/` — uses the same `EncryptedPayload` / `EncryptionKey` primitives directly when storing per-channel secrets.
- `src/core/all.rs` — registers `all_encryption_*` controllers so the shell + CLI can encrypt configuration secrets.
- Indirect: `src/neppy/memory/`, `src/neppy/channels/`, and `src/neppy/inference/local/` rely on the credentials domain (which in turn uses this layer) for secrets at rest.

## Tests

- This domain has no `*_tests.rs` siblings; the underlying crypto round-trips are exercised by `src/neppy/security/keyring/encrypted_store_tests.rs` and the credentials tests, which both cover encrypt/decrypt happy paths and tampered-ciphertext rejection.
