...
pub(crate) fn handle_voice_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::neppy::voice::voice_status(&config).await?)
    })
}
...
pub(crate) fn handle_voice_agent_signed_url(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::neppy::voice::realtime::mint_voice_agent_signed_url(&config).await?)
    })
}
...
pub(crate) fn handle_voice_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::voice::voice_transcribe(
                &config,
                &p.audio_path,
                p.context.as_deref(),
                p.skip_cleanup,
...
pub(crate) fn handle_voice_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::neppy::voice::voice_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
                p.context.as_deref(),
...
pub(crate) fn handle_voice_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::neppy::voice::voice_tts(&config, &p.text, p.output_path.as_deref()).await?)
    })
}
...
pub(crate) fn handle_voice_reply_synthesize(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let voice = p
            ...
            .unwrap_or_else(|| {
                if provider_name == "piper" {
                    crate::neppy::voice::DEFAULT_PIPER_VOICE.to_string()
                } else {
                    String::new()
                }
            });
        ...
        let provider = crate::neppy::voice::create_tts_provider(&provider_name, &voice, &config)
...
pub(crate) fn handle_voice_cloud_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let opts = crate::neppy::voice::cloud_transcribe::CloudTranscribeOptions {
            model: p.model,
            language: p.language,
        ...
        to_json(
            crate::neppy::voice::cloud_transcribe::transcribe_cloud(
                &config,
                &p.audio_base64,
                &opts,
            )
...
pub(crate) fn handle_voice_stt_dispatch(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let provider = crate::neppy::voice::create_stt_provider(&provider_name, &model, &config)
            .map_err(|e| e.to_string())?;
