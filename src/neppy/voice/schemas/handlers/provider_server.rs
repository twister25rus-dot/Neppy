        ...
        if let Some(providers) = p.voice_providers {
            ...
            for update in providers {
                ...
                let auth_style = match update.auth_style.as_deref() {
                    Some("bearer") | None => crate::neppy::config::schema::AuthStyle::Bearer,
                    Some("none") => crate::neppy::config::schema::AuthStyle::None,
                    Some(other) => {
                        return Err(format!(
...
pub(crate) fn handle_voice_test_provider(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        match p.workload.as_str() {
            "stt" => {
                let provider = crate::neppy::voice::create_stt_provider(&p.provider, "", &config)
            ...
            "tts" => {
                ...
                if p.validate_only && !matches!(trimmed, "cloud" | "openhuman" | "piper" | "") {
                    ...
                    let provider = crate::neppy::voice::create_tts_provider(trimmed, "", &config)
                        .map_err(|e| e.to_string())?;
...
pub(crate) fn handle_voice_server_start(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        use crate::neppy::voice::server::{global_server, VoiceServerConfig};
        ...
        let activation_mode = match params.get("activation_mode").and_then(|v| v.as_str()) {
            ...
            None => match config.voice_server.activation_mode {
                crate::neppy::config::VoiceActivationMode::Push => ActivationMode::Push,
                crate::neppy::config::VoiceActivationMode::Tap => ActivationMode::Tap,
            },
        };
        ...
        if let Some(existing) = crate::neppy::voice::server::try_global_server() {
            let existing_status = existing.status().await;
            if existing_status.state != crate::neppy::voice::server::ServerState::Stopped {
        ...
        if let Some(s) = crate::neppy::voice::server::try_global_server() {
...
pub(crate) fn handle_voice_server_stop(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        if let Some(server) = crate::neppy::voice::server::try_global_server() {
            ...
            let status = crate::neppy::voice::server::VoiceServerStatus {
                state: crate::neppy::voice::server::ServerState::Stopped,
                hotkey: String::new(),
                activation_mode: crate::neppy::voice::hotkey::ActivationMode::Push,
...
pub(crate) fn handle_overlay_stt_notify(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        use crate::neppy::voice::dictation_listener::{
