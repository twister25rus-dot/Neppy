...
use crate::neppy::modules::voice as tinyvoice;
use crate::neppy::voice::audio_capture::TARGET_SAMPLE_RATE;
...
fn notch_status(status: &str, ttl_ms: u32) {
    let _ = crate::neppy::desktop::overlay::publish_attention(
        crate::neppy::desktop::overlay::OverlayAttentionEvent::new(status)
            .with_source("voice")
            .with_ttl_ms(ttl_ms),
    );
}
...
async fn transcribe_and_deliver(config: &Config, samples_16k: Vec<f32>) {
    ...
    let provider_name = crate::neppy::voice::effective_stt_provider(config);
    ...
    let provider = match crate::neppy::voice::create_stt_provider(&provider_name, "", config) {
        Ok(p) => p,
        Err(e) => {
...
async fn deliver_command(config: &Config, cmd: String) {
    use crate::neppy::modules::voice::{route, VoiceIntent};
    ...
    if matches!(intent, VoiceIntent::Unknown) {
        ...
        crate::neppy::voice::dictation_listener::publish_transcription(cmd);
        return;
    }
    log::info!(
...
async fn execute_intent(
    _config: &Config,
    intent: crate::neppy::modules::voice::VoiceIntent,
) -> Result<String, String> {
    use crate::neppy::modules::voice::VoiceIntent as VI;
    match intent {
...
mod tests {
    ...
    fn vad_config_maps_persisted_seconds_to_milliseconds() {
        let mut c = crate::neppy::config::VoiceServerConfig::default();
