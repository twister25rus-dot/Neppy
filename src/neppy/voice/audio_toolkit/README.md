...
| File | Role |
| --- | --- |
| `src/neppy/voice/audio_toolkit/mod.rs` | Export-only module surface: re-exports ops fns, schema registries, and types. |
| `src/neppy/voice/audio_toolkit/types.rs` | Serde domain types: `AudioFormat`, request/result structs. |
| `src/neppy/voice/audio_toolkit/ops.rs` | Business logic: `generate_podcast`, `email_podcast`, `generate_and_email_podcast`, `resolve_email_capture_dir`, plus private helpers (path/format/voice resolution, base64 decode, MIME enforcement, slugify, email build/capture). Contains the unit-test suite. |
| `src/neppy/voice/audio_toolkit/schemas.rs` | RPC controller schemas + `handle_*` handlers delegating to `ops.rs`; loads `Config` via `config::rpc`. |
| `src/neppy/voice/audio_toolkit/tools.rs` | Re-exports the three agent tool structs from `tools/podcast.rs`. |
| `src/neppy/voice/audio_toolkit/tools/podcast.rs` | `Tool` impls: `AudioGeneratePodcastTool`, `AudioEmailPodcastTool`, `AudioGenerateAndEmailPodcastTool`; security gating + arg parsing. |
...
Tools are constructed with `Arc<Config>` + `Arc<SecurityPolicy>` and registered in `src/neppy/tools/ops.rs`; re-exported via `src/neppy/tools/mod.rs`.
...
- `crate::neppy::voice` — `create_tts_provider`, `DEFAULT_PIPER_VOICE`; actual speech synthesis.
- `crate::neppy::channels::email_channel::EmailChannel` — SMTP delivery of the built message.
- `crate::neppy::config::Config` / `config::rpc` — workspace dir, `local_ai.tts_provider`, `channels_config.email`; `load_config_with_timeout` in RPC handlers.
- `crate::neppy::security` — `SecurityPolicy` / `ToolOperation` to gate the agent tools.
- `crate::neppy::tools::traits` — `Tool`, `ToolResult`, `PermissionLevel`.
...
- `src/neppy/tools/ops.rs` — instantiates the three tools into the agent tool registry; `src/neppy/tools/mod.rs` re-exports them.
