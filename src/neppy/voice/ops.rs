...
use crate::neppy::inference::local::model_ids;
use crate::neppy::inference::local::paths::{resolve_piper_binary, resolve_tts_voice_path};
...
use crate::neppy::modules::voice::{is_hallucinated, HallucinationMode};
...
pub async fn voice_status(config: &Config) -> Result<RpcOutcome<VoiceStatus>, String> {
    ...
    let stt_error = create_stt_provider(&stt_engine, "", config)
        .and_then(|_| {
            ...
            let key = crate::neppy::inference::provider::factory::lookup_key_for_slug(
                slug.trim(),
                config,
            )?;
...
mod tests {
    ...
    async fn voice_status_reports_unconfigured_engine_as_unavailable() {
        ...
        config.workspace_dir = tmp.path().join("workspace");
        config.config_path = tmp.path().join("config.toml");
        config.voice_server.stt_engine = crate::neppy::config::schema::SttEngine::Elevenlabs;
    ...
    async fn voice_status_reports_external_engine_without_credentials_as_unavailable() {
        ...
        config.voice_providers.push(
            crate::neppy::config::schema::voice_providers::VoiceProviderCreds {
                slug: "elevenlabs".into(),
                endpoint: "https://api.elevenlabs.io/v1".into(),
                capability: crate::neppy::config::schema::voice_providers::VoiceCapability::Both,
                ..Default::default()
            },
        );
