...
impl EmbeddingCallbacks {
    async fn embed(
        ...
        let api_key = crate::neppy::inference::embeddings::resolve_api_key(&self.0, &provider);
        let endpoint = self
            .0
            .cloud_providers
            .iter()
        ...
        let embedder = crate::neppy::inference::embeddings::create_embedding_provider_with_config(
...
impl ChatCallbacks {
    async fn complete(
        ...
        let (model, _) = crate::neppy::inference::provider::create_chat_model_with_model_id(
            &role,
            &self.0,
...
impl ComposioCallbacks {
    ...
    async fn api_key(&self) -> tinybus::Result<Option<String>> {
        ...
        let stored = crate::neppy::security::credentials::get_composio_api_key(config.as_ref())
            .ok()
            .flatten();
...
impl RuntimeCallbacks {
    async fn publish_event(&self, event: MemoryEvent) -> tinybus::Result<()> {
        ...
        if matches!(event, MemoryEvent::StoreCorruptQuarantined { .. }) {
            ...
            tokio::task::spawn_blocking(move || {
                crate::neppy::memory::host_impls::reset_in_process_chunk_store(&config);
            })
            .await
            .ok();
        }
    ...
    async fn extract_spacy(&self, text: String) -> tinybus::Result<SpacyResponse> {
        let response = crate::neppy::runtime::python_server::extract_spacy(&self.0, &text)
            .await
            .map_err(method_error)?;
...
fn into_domain_event(event: MemoryEvent) -> Option<crate::core::events::DomainEvent> {
    ...
    Some(match event {
        ...
        MemoryEvent::LocalModelUnavailable { origin } => {
            crate::neppy::memory::tree::health::user_error::publish_local_model_unavailable_user_error(&origin);
            return None;
        }
        ...
        MemoryEvent::StoreCorruptQuarantined {
            ...
            crate::neppy::memory::tree::health::user_error::publish_store_corrupt_user_error(
                &origin,
