...
fn the_registry_entry_matches_the_interface_this_client_calls() {
    ...
    let record =
        crate::neppy::modules::registry::find("tinyvoice").expect("tinyvoice is registered");
...
///   neppy::modules::voice::tests::the_published_module_answers_through_this_client
/// ```
///
    ...
    assert!(matches!(
        super::VadSession::open(
            ...
            super::vad_config_from_server_config(
                &crate::neppy::config::VoiceServerConfig::default()
            )
        )
        .await,
        Err(VoiceCallError::Unavailable(_))
