...
use crate::neppy::memory::ops::guard::active_memory_guard;
...
/// (`src/neppy/memory/store/`) is internally inconsistent about
...
/// tinyflows `memory` node's `scope: "flows"` (`NeppyMemory::recall` in
/// `crate::neppy::flows::tinyflows::memory_adapter`) — both surfaces must see
...
/// Renders recall hits the same way [`crate::neppy::memory::tools::recall`]
...
mod tests {
    ...
    fn test_mem() -> (TempDir, Arc<dyn crate::neppy::memory::Memory>) {
