...
//! ([`crate::neppy::platform::socket::medulla::workflows::WorkflowBridge`]) with this
...
use crate::neppy::platform::socket::medulla::payloads::{
    CopilotOutcome, WorkflowDescriptor, WorkflowInputDescriptor,
};
use crate::neppy::platform::socket::medulla::workflows::{set_workflow_bridge, WorkflowBridge};
...
impl FlowsWorkflowBridge {
    ...
    async fn config(&self) -> Result<Arc<Config>, String> {
        match &self.config {
            ...
            None => crate::neppy::config::rpc::load_config_with_timeout()
                .await
                .map(Arc::new)
...
fn node_kind_json(contract: &crate::neppy::flows::node_contracts::NodeKindContract) -> Value {
    json!({
        "kind": contract.kind,
