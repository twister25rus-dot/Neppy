...
async fn flows_create_binds_schedule_cron_job_for_an_enabled_flow() {
    ...
    assert!(
        crate::neppy::cron::find_flow_schedule_job(&config, &created.value.id)
            .unwrap()
            .is_none(),
    ...
    let job = crate::neppy::cron::find_flow_schedule_job(&config, &created.value.id).unwrap();
    assert!(
        job.is_some(),
...
async fn reconcile_schedule_triggers_on_boot_survives_a_corrupt_row() {
    ...
    let good_job = crate::neppy::cron::find_flow_schedule_job(&config, &good.value.id)
        .unwrap()
        .expect("precondition: good flow's cron job bound on enable");
    crate::neppy::cron::remove_job(&config, &good_job.id).unwrap();
    assert!(
        crate::neppy::cron::find_flow_schedule_job(&config, &good.value.id)
            .unwrap()
            .is_none(),
...
async fn flows_delete_clears_flow_memory_namespace() {
    use crate::neppy::memory::{MemoryCategory, MemoryTaint};
    ...
    let memory = crate::neppy::memory::binding::for_config(&config)
...
async fn flows_update_rebinds_schedule_cron_job_when_trigger_schedule_changes() {
    ...
    flows_set_enabled(&config, &created.value.id, true)
        .await
        .unwrap();
    let old_job = crate::neppy::cron::find_flow_schedule_job(&config, &created.value.id)
        .unwrap()
    ...
    let new_job = crate::neppy::cron::find_flow_schedule_job(&config, &created.value.id)
        .unwrap()
    ...
    let flow_jobs: Vec<_> = crate::neppy::cron::list_jobs(&config)
        .unwrap()
        .into_iter()
...
async fn flows_update_does_not_rebind_when_graph_is_not_supplied() {
    ...
    let job = crate::neppy::cron::find_flow_schedule_job(&config, &created.value.id)
        .unwrap()
...
async fn flows_run_emits_pending_approval_notification() {
    let tmp = TempDir::new().unwrap();
    let config = test_config(&tmp);
    let mut rx = crate::neppy::desktop::notifications::bus::subscribe_core_notifications();
    ...
    assert_eq!(
        ...
        crate::neppy::desktop::notifications::types::CoreNotificationCategory::Agents
    );
...
use crate::neppy::flows::tinyflows::observability::FlowRunObserver;
...
use crate::neppy::integrations::composio::ComposioConnection;
...
fn build_flow_connections_emits_parseable_refs_for_both_kinds() {
    ...
    assert_eq!(
        crate::neppy::flows::tinyflows::caps::composio_connection_id(&gmail.connection_ref),
        Some("ca_abc")
    );
    ...
    assert_eq!(
        crate::neppy::flows::tinyflows::caps::http_cred_name(&stripe.connection_ref),
        Some("stripe")
    );
...
fn build_flow_connections_attaches_platform_user_id_from_a_seeded_identity() {
    use crate::neppy::integrations::composio::providers::profile::ConnectedIdentity;
...
async fn flows_list_connections_aggregates_http_creds_and_tolerates_composio() {
    ...
    config.composio.mode = crate::neppy::config::schema::COMPOSIO_MODE_DIRECT.to_string();
...
fn seed_suggestion(config: &Config, id: &str) {
    let s = crate::neppy::flows::FlowSuggestion {
        id: id.to_string(),
        ...
        confidence: 0.5,
        status: crate::neppy::flows::SuggestionStatus::New,
        created_at: "2026-07-05T00:00:00Z".to_string(),
        source_run_id: None,
    };
    crate::neppy::flows::store::upsert_suggestions(config, &[s]).unwrap();
}
...
async fn list_suggestions_filters_by_status() {
    ...
    let active = flows_list_suggestions(&config, Some(crate::neppy::flows::SuggestionStatus::New))
        .await
        .unwrap();
...
async fn run_builder_gates_does_not_reject_when_signed_out() {
    ...
    let _signed_out = crate::neppy::cron::scheduler_gate::SignedOutTestGuard::set(true);
...
fn agent_node_role_prefers_custom_registry_entry_model_pin_over_default() {
    ...
    use crate::neppy::agent::registry::types::{AgentRegistryEntry, AgentRegistrySource};
...
async fn flows_build_hides_the_live_run_tool_from_the_builder_belt() {
    ...
    use crate::neppy::tools::Tool as _;
    let live_runner =
        crate::neppy::flows::tools::RunFlowTool::new(std::sync::Arc::new(config.clone()));
    assert!(
        live_runner.external_effect(),
        "the flows live-runner must be external-effect for the #4593 concern to apply"
    );
    let resumer = crate::neppy::flows::builder_tools::ResumeFlowRunTool::new(std::sync::Arc::new(
        config.clone(),
    ));
    assert!(
    ...
    let canceller = crate::neppy::flows::builder_tools::CancelFlowRunTool::new(
    ...
    let mut agent = crate::neppy::agent::Agent::from_config_for_agent(&config, "workflow_builder")
...
async fn flows_build_applies_the_builder_definitions_effective_iteration_cap() {
    ...
    crate::neppy::agent::harness::AgentDefinitionRegistry::init_global(&config.workspace_dir)
        .expect("agent registry init");
    let def = crate::neppy::agent::harness::AgentDefinitionRegistry::global()
    ...
    let agent = crate::neppy::agent::Agent::from_config_for_agent(&config, "workflow_builder")
        .expect("build workflow_builder agent");
...
async fn flows_discover_applies_the_flow_discovery_definitions_effective_iteration_cap() {
    ...
    let agent = crate::neppy::agent::Agent::from_config_for_agent(&config, "flow_discovery")
...
async fn draft_promote_creates_a_new_flow_and_removes_the_draft() {
    use crate::neppy::flows::DraftOrigin;
    let tmp = TempDir::new().unwrap();
    let config = test_config(&tmp);
...
fn extract_workflow_proposal_survives_large_graph() {
    use crate::neppy::agent::messages::{ConversationMessage, ToolResultMessage};
...
fn builder_tool_call(id: &str, name: &str) -> crate::neppy::agent::messages::ConversationMessage {
    use crate::neppy::agent::messages::ConversationMessage;
    use crate::neppy::inference::provider::ToolCall;
...
fn builder_tool_result(
    call_id: &str,
    content: &str,
) -> crate::neppy::agent::messages::ConversationMessage {
    use crate::neppy::agent::messages::{ConversationMessage, ToolResultMessage};
    ConversationMessage::ToolResults(vec![ToolResultMessage {
...
async fn approval_manifest_marks_blocked_classes_under_readonly_tier() {
    let tmp = TempDir::new().unwrap();
    let mut config = test_config(&tmp);
    config.autonomy.level = crate::neppy::security::AutonomyLevel::ReadOnly;
    let entries = compute_approval_manifest(&config, &manifest_graph()).await;
