...
impl FlowTriggerSubscriber {
    ...
    async fn handle_schedule_tick(&self, flow_id: &str) {
        ...
        self.spawn_run(
            ...
            Value::Null,
            inputs,
            crate::neppy::flows::FlowRunTrigger::Schedule,
        );
    }
        ...
        for flow in flows {
            if matches_app_event(&flow, toolkit, trigger_slug) {
                ...
                self.spawn_run(
                    ...
                    payload.clone(),
                    inputs,
                    crate::neppy::flows::FlowRunTrigger::AppEvent,
                );
            }
        }
    ...
    fn spawn_run(
        ...
        trigger: crate::neppy::flows::FlowRunTrigger,
    ) {
        ...
        tokio::spawn(async move {
            ...
            match crate::neppy::flows::ops::flows_run(&config, &flow_id, input, inputs, trigger)
                .await
            {
                Ok(_) => {
...
impl FlowRunDigestSubscriber {
    ...
    fn with_memory(
        config: Arc<Config>,
        memory: Arc<crate::neppy::memory::guard::MemoryGuard>,
    ) -> Self {
        Self {
            config,
    ...
    async fn resolve_memory(&self) -> Option<Arc<crate::neppy::memory::guard::MemoryGuard>> {
        ...
        match crate::neppy::memory::ops::guard::active_memory_guard().await {
            Ok(guard) => Some(guard),
            Err(e) => {
...
/// `src/neppy/flows/tinyflows/caps.rs`) the engine's `FlowStateStore` hands the
...
mod tests {
    ...
    ) -> Arc<crate::neppy::memory::guard::MemoryGuard> {
        crate::neppy::memory::guard::in_memory::guarded_in_memory().1
    }
    ...
    async fn handle_app_event_ignores_disabled_flows() {
        ...
        crate::neppy::flows::store::upsert_flow(&config, &flow).unwrap();
        ...
        let (enabled, skipped) = crate::neppy::flows::store::list_enabled_flows(&config).unwrap();
    ...
    async fn digest_writes_run_digest_entry_for_completed_run() {
        ...
        .unwrap();
        let step = crate::neppy::flows::FlowRunStep {
    ...
    fn render_run_digest_is_bounded_and_includes_key_fields() {
        let run = FlowRun {
            ...
            steps: vec![crate::neppy::flows::FlowRunStep {
                node_id: "n1".to_string(),
                output: json!({ "ok": true }),
                port: None,
                status: Some("success".to_string()),
    ...
    fn dedup_state_namespace(flow_id: &str) -> String {
        ...
        // (`src/neppy/flows/tinyflows/caps.rs`) — this test asserts the
