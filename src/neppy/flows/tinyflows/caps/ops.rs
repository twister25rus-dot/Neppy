...
use crate::neppy::flows::tinyflows::checkpoint_sqlite::SqliteCheckpointer;
use anyhow::Context;
...
#[cfg(test)]
use crate::neppy::config::HttpRequestConfig;
#[cfg(test)]
use crate::neppy::flows;
#[cfg(test)]
use crate::neppy::security::credentials::HttpCredential;
use crate::neppy::security::credentials::HttpCredentialsStore;
use crate::neppy::security::{CommandClass, SecurityPolicy};
#[cfg(test)]
use crate::neppy::security::{GateDecision, POLICY_BLOCKED_MARKER};

// The JSON Schema walkers moved to `neppy::json_schema`, a domain owned by
// neither this seam nor `composio` — see that module's docs for why neutral
// ownership is load-bearing rather than tidiness.
//
// Re-exported so `crate::neppy::flows::tinyflows::caps::<fn>` keeps resolving for
...
#[cfg(test)]
pub(crate) use crate::neppy::integrations::composio::catalog::ProbedOutputSample;
pub(crate) use crate::neppy::integrations::composio::catalog::{
...
#[cfg(test)]
pub(crate) use crate::neppy::json_schema::{
    compute_primary_array_path, response_fields_from_schema,
};
pub(crate) use crate::neppy::json_schema::{missing_required_args, unsupported_arg_names};
...
async fn flow_tool_allowed(
    ...
    use crate::neppy::memory::sync::composio::providers::{
...
async fn connected_toolkit_slugs(config: &Config) -> Option<Vec<String>> {
    use crate::neppy::integrations::composio::{
...
/// evaluates it under.
///
/// Reuses [`curated_scope_for`](crate::neppy::memory::sync::composio::providers::curated_scope_for),
...
/// [`classify_unknown`](crate::neppy::memory::sync::composio::providers::classify_unknown)
...
pub(crate) async fn classify_composio_action_for_tier(slug: &str) -> CommandClass {
    use crate::neppy::memory::sync::composio::providers::{curated_scope_for, ToolScope};
...
fn resolve_account<'a>(
    integrations: &'a [crate::neppy::integrations::composio::ConnectedIntegration],
    connection_id: &str,
...
pub(crate) async fn resolve_composio_account(
    ...
    let integrations =
        crate::neppy::integrations::composio::fetch_connected_integrations(config).await;
    resolve_account(&integrations, connection_id)
        .map(|(toolkit, label)| (toolkit.to_string(), label.map(str::to_string)))
}

/// [`ToolInvoker`] adapter over Composio (`src/neppy/integrations/composio/client.rs`).
///
...
pub(crate) fn reject_unsuccessful_composio_response(
    slug: &str,
    resp: crate::neppy::integrations::composio::ComposioExecuteResponse,
) -> Result<crate::neppy::integrations::composio::ComposioExecuteResponse> {
    if resp.successful {
        return Ok(resp);
    }
    let detail = resp
...
pub(crate) fn reject_failed_native_tool_result(
    slug: &str,
    result: &crate::neppy::skills::types::ToolResult,
) -> Result<()> {
    if !result.is_error {
        return Ok(());
    }
...
pub(crate) fn native_tool_payload(result: &crate::neppy::skills::types::ToolResult) -> Value {
...
pub fn build_capabilities(config: Arc<Config>, state_namespace: impl Into<String>) -> Capabilities {
    ...
    Capabilities {
        ...
        approvals: None,
        memory: Some(Arc::new(
            crate::neppy::flows::tinyflows::memory_adapter::NeppyMemory {
                config: config.clone(),
                security,
            },
        )),
...
mod tests {
    use super::*;
    use crate::neppy::agent::prompts::types::IntegrationConnection;
    use crate::neppy::integrations::composio::{ComposioExecuteResponse, ConnectedIntegration};
    use crate::neppy::skills::types::{ToolContent, ToolResult};
    ...
    fn injected_secret_never_reaches_the_audit_redaction() {
        ...
        let redacted = crate::neppy::security::approval::redact_args(&request);
    ...
    fn policy(level: crate::neppy::security::AutonomyLevel) -> SecurityPolicy {
        SecurityPolicy {
            autonomy: level,
    ...
    fn agent_node_runtime_resolves_to_the_definitions_effective_iteration_cap() {
        ...
        crate::neppy::agent::harness::definition::AgentDefinitionRegistry::init_global(
            &config.workspace_dir,
        )
        .expect("agent registry init");
        let def = crate::neppy::agent::harness::definition::AgentDefinitionRegistry::global()
        ...
        let agent = crate::neppy::agent::Agent::from_config_for_agent(&config, "code_executor")
    ...
    // `neppy::flows::ops_tests` for the end-to-end
    ...
    fn crate_model_response_preserves_flow_completion_contract() {
        ...
        let response = ModelResponse {
            ...
            raw: crate::neppy::agent::tinyagents::model::merge_neppy_usage_meta(
                None, 0.125, 128_000,
            ),
            resolved_model: None,
            continue_turn: None,
