...
impl ToolBackend for NativeToolBackend {
    ...
    async fn invoke(
        ...
        let class =
            crate::neppy::runtime::node::ops::classify_tool_call(ctx.config, tool_name, &args)
                .map_err(EngineError::Capability)?;
        let tier_decision = super::super::enforce_node_tier_gate(ctx.security, class, "tool_call")?;
        let summary = crate::neppy::security::approval::summarize_action(tool_name, &args);
        ...
        let exec_result =
            crate::neppy::runtime::node::ops::execute_tool(ctx.config, tool_name, args, false)
                .await
                .map_err(EngineError::Capability)
                .and_then(|outcome| {
