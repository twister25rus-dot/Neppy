...
use super::*;
use crate::neppy::config::{Config, HttpRequestConfig};
use crate::neppy::security::credentials::{HttpCredential, HttpCredentialsStore};
use crate::neppy::security::{CommandClass, SecurityPolicy};
use crate::neppy::tools::traits::Tool as _;
use crate::neppy::tools::HttpRequestTool;

/// [`HttpClient`] adapter over `HttpRequestTool`
/// (`src/neppy/tools/impl/network/http_request.rs`). Allowlist + DNS-rebind
...
impl HttpClient for NeppyHttp {
    async fn request(&self, mut request: Value, conn: Option<&str>) -> Result<Value> {
        ...
        let summary = crate::neppy::security::approval::summarize_action(TOOL_NAME, &request);
        ...
        if let crate::neppy::security::approval::GateOutcome::Deny { reason } = outcome {
