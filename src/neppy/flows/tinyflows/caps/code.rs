...
use crate::neppy::sandbox::{execute_in_sandbox, resolve_sandbox_policy};
use crate::neppy::security::{CommandClass, SecurityPolicy};

/// [`CodeRunner`] adapter running sandboxed user code via
/// `src/neppy/sandbox/ops.rs` (`resolve_sandbox_policy` +
/// `execute_in_sandbox`), modeled on
/// `src/neppy/tools/impl/system/node_exec.rs::run_sandboxed`.
///
...
impl CodeRunner for NeppyCode {
    async fn run(&self, language: CodeLanguage, source: &str, input: Value) -> Result<Value> {
        ...
        let summary = crate::neppy::security::approval::summarize_action("flows_code", &action);
        let redacted = crate::neppy::security::approval::redact_args(&action);
        let (gate_outcome, audit_id) =
            gate_call_for_tier(tier_decision, "flows_code", &summary, redacted).await;
        if let crate::neppy::security::approval::GateOutcome::Deny { reason } = gate_outcome {
