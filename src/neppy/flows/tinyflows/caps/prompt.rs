...
use crate::neppy::inference::provider::{is_raw_passthrough_model, UsageInfo};
...
pub(crate) fn model_response_to_completion_value(
    ...
    json!({
        ...
        "tool_calls": response
            .tool_calls()
            .iter()
            .map(crate::neppy::agent::tinyagents::ta_call_to_oh_call)
            .collect::<Vec<_>>(),
        "usage": usage_to_json(
            &crate::neppy::agent::tinyagents::model::usage_info_from_response(response)
        ),
        "reasoning_content": crate::neppy::agent::tinyagents::reasoning_from_content(
