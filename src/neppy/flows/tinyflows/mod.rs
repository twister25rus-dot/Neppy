...
//! in-crate state-graph runtime) to real Neppy services.
//!
...
//! `memory` node's `MemoryProvider` adapter (`NeppyMemory`) lives in its
...
//! (`src/neppy/flows/ops.rs`) calls [`build_capabilities`] /
...
/// `NeppyMemory` adapter + real store — see the module doc there for why
...
pub use memory_adapter::NeppyMemory;
