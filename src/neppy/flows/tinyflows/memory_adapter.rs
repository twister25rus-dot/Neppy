...
//! [`crate::neppy::flows::flow_namespace`], and `scope: "flows"` recall
//! delegates to the exact same [`crate::neppy::flows::cross_flow_recall`]
...
use crate::neppy::agent::harness::memory_context_safety::{
...
use crate::neppy::flows::{cross_flow_recall, flow_namespace};
use crate::neppy::memory::tools::flavour::{lookup_flavour, FlavourLookup};
...
impl NeppyMemory {
    ...
    async fn memory(&self) -> Result<Arc<crate::neppy::memory::guard::MemoryGuard>> {
        crate::neppy::memory::ops::guard::active_memory_guard()
            .await
...
impl MemoryProvider for NeppyMemory {
    ...
    async fn people(&self, query: Option<&str>) -> Result<Value> {
        ...
        let guard = crate::neppy::memory::ops::guard::active_memory_guard()
            .await
            .map_err(|e| {
        ...
        let outcome = crate::neppy::memory::people::rpc::handle_list(people, DEFAULT_PEOPLE_LIMIT)
            .await
            .map_err(EngineError::Capability)?;
    ...
    async fn remember(&self, scope: &str, key: &str, value: Value) -> Result<()> {
        ...
        if crate::neppy::memory::safety::has_likely_secret(&content) {
            tracing::warn!(
                target: "flows",
