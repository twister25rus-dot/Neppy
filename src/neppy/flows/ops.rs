...
use crate::neppy::flows::build_registry;
use crate::neppy::flows::bus;
use crate::neppy::flows::draft_store;
use crate::neppy::flows::run_registry;
use crate::neppy::flows::store;
use crate::neppy::flows::types::{
    FlowConnection, FlowRunStep, FlowRunTrigger, FlowSuggestion, SuggestionStatus,
};
use crate::neppy::flows::{flow_namespace, Flow, FlowRun};
...
/// 10-minute fail-closed TTL (`src/neppy/security/approval/`), so a flow HITL gate a
...
// (`src/neppy/security/`) the agent tool-loop honors, built via
...
// (`src/neppy/flows/tinyflows/caps.rs::enforce_node_tier_gate`) maps the node to a
...
pub(crate) fn engine_compatibility_errors(
    graph: &WorkflowGraph,
) -> Vec<crate::neppy::flows::FlowValidationError> {
...
fn collect_engine_compatibility_errors(
    ...
    depth: u64,
    max_depth: u64,
    errors: &mut Vec<crate::neppy::flows::FlowValidationError>,
) {
...
fn graph_engine_compatibility_errors(
    ...
    for fan_in in &graph.nodes {
        ...
        for predecessor in incoming {
            ...
            errors.push(crate::neppy::flows::FlowValidationError {
                code: code.to_string(),
                message: format!(
...
) -> crate::neppy::flows::FlowValidationError {
    crate::neppy::flows::FlowValidationError {
...
pub(crate) async fn build_builder_proposal(
    ...
    let summary = crate::neppy::flows::tools::build_summary(graph);
...
pub(crate) async fn graph_wiring_warnings(config: &Config, graph: &WorkflowGraph) -> Vec<String> {
    use crate::neppy::flows::tinyflows::caps::{composio_required_args, missing_required_args};
...
/// [`crate::neppy::flows::tinyflows::caps::ToolContract::output_fields`]'s doc),
...
async fn graph_output_field_warnings(config: &Config, graph: &WorkflowGraph) -> Vec<String> {
    use crate::neppy::flows::tinyflows::caps::fetch_live_toolkit_catalog;
    ...
    for node in &graph.nodes {
        for (location, expr) in collect_expressions(&node.config) {
            ...
            let contract =
                crate::neppy::flows::tinyflows::caps::apply_probe_override(contract.clone());
...
///    [`crate::neppy::flows::tinyflows::caps::compute_composio_array_path`] —
...
///    [`crate::neppy::flows::tinyflows::caps::apply_probe_override`], a real
...
async fn graph_split_out_path_warnings(config: &Config, graph: &WorkflowGraph) -> Vec<String> {
    use crate::neppy::flows::tinyflows::caps::{apply_probe_override, fetch_live_toolkit_catalog};
    use tinymemory_api::composio::toolkit_from_slug;
...
/// the stable `{ json, text, raw }` envelope (`src/neppy/flows/tinyflows/caps.rs`):
...
/// wraps the field in `data` (see [`crate::neppy::flows::tinyflows::caps::ToolContract::output_fields`]'s
...
/// that resolves via [`crate::neppy::flows::tinyflows::caps::route_for_agent_ref`]
/// to a harness [`AgentDefinition`](crate::neppy::agent::harness::definition::AgentDefinition)
/// (`AgentRoute::Harness`), OR — when it routes to `AgentRoute::RegistryFallback`
/// — resolves to an *enabled*
/// [`AgentRegistryEntry`](crate::neppy::agent::registry::AgentRegistryEntry)
/// via [`crate::neppy::agent::registry::get_agent`]. Both are exactly the
...
pub(crate) async fn validate_agent_refs(config: &Config, graph: &WorkflowGraph) -> Vec<String> {
    ...
    use crate::neppy::agent::registry::AgentRegistryEntry;
    use crate::neppy::flows::tinyflows::caps::{route_for_agent_ref, AgentRoute};
        ...
        match route_for_agent_ref(agent_ref) {
            ...
            AgentRoute::RegistryFallback => {
                if custom_registry.is_none() {
                    custom_registry = Some(crate::neppy::agent::registry::list_agents(true).await);
                }
...
fn invalidate_inference_probe_cache_if_signed_out() {
    if crate::neppy::cron::scheduler_gate::is_signed_out() {
...
async fn cached_probe_inference_readiness(role: &str, config: &Config) -> Result<(), String> {
    ...
    let result = crate::neppy::inference::provider::probe_inference_readiness(role, config).await;
    INFERENCE_PROBE_CACHE
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner)
...
///    [`role_for_model_tier`](crate::neppy::inference::provider::role_for_model_tier).
/// 2. A static (non-`=`) `agent_ref` whose custom
///    [`AgentRegistryEntry`](crate::neppy::agent::registry::AgentRegistryEntry)
///    itself pins a `model` (e.g. `hint:reasoning`) — resolved the same way
///    [`NeppyAgentRunner::run_via_harness`](crate::neppy::flows::tinyflows::caps::NeppyAgentRunner)
///    does via `resolve_node_model(&request, entry_model)`, using the same
///    sync, config-only accessor
///    ([`find_custom_in_config`](crate::neppy::agent::registry::find_custom_in_config))
///    it calls.
...
fn agent_node_role(config: &Config, node: &tinyflows::model::Node) -> &'static str {
    ...
    if let Some(model) = pinned_model {
        return crate::neppy::inference::provider::role_for_model_tier(model);
    }
    ...
    if let Some(agent_ref) = static_agent_ref {
        if let Some(entry_model) =
            crate::neppy::agent::registry::find_custom_in_config(config, agent_ref)
                .and_then(|entry| entry.model)
        {
            let entry_model = entry_model.trim();
            if !entry_model.is_empty() {
                return crate::neppy::inference::provider::role_for_model_tier(entry_model);
            }
        }
    }
...
async fn evaluate_inference_readiness(
    ...
    #[cfg(not(test))]
    if let Err(e) = crate::neppy::inference::provider::factory::verify_session_active(config) {
        tracing::debug!(
            target: "flows",
            node = %first_node.id,
            error = %e,
...
// [`crate::neppy::flows::tinyflows::caps::fetch_live_toolkit_catalog`] — the
...
pub(crate) fn toolkit_has_curated_catalog(toolkit: &str) -> bool {
    ...
    use crate::neppy::memory::sync::composio::providers::{catalog_for_toolkit, get_provider};
...
pub(crate) async fn validate_tool_contracts(config: &Config, graph: &WorkflowGraph) -> Vec<String> {
    use crate::neppy::flows::tinyflows::caps::{
...
/// segments. Mirrors [`crate::neppy::flows::tinyflows::caps::composio_connection_id`]'s
...
pub(crate) async fn validate_connection_refs(
    ...
    let connections: Option<Vec<FlowConnection>> =
        match crate::neppy::integrations::composio::ops::composio_list_connections(config).await {
...
//
// `crate::neppy::flows::builder_tools::DryRunWorkflowTool` already
...
/// [`crate::neppy::flows::tinyflows::caps::PreflightToolInvoker`] the way
...
pub(crate) async fn validate_required_arg_resolvability(graph: &WorkflowGraph) -> Vec<String> {
    use crate::neppy::flows::builder_tools::CapturingObserver;
    use crate::neppy::flows::tinyflows::caps::{SchemaAwareMockAgentRunner, SchemaAwareMockLlm};
...
///   (`crate::neppy::flows::tinyflows`'s `expr_scope` docs) — always
///   trigger-scoped.
...
pub fn flows_validate(graph_json: Value) -> RpcOutcome<crate::neppy::flows::FlowValidation> {
    use crate::neppy::flows::FlowValidation;
        ...
        Err(error) => {
            ...
            return RpcOutcome::single_log(
                FlowValidation {
                    ...
                    error_details: vec![crate::neppy::flows::FlowValidationError {
...
///   [`crate::neppy::flows::n8n_import`] into a `WorkflowGraph` (unmapped
...
) -> Result<RpcOutcome<crate::neppy::flows::FlowImport>, String> {
    use crate::neppy::flows::{n8n_import, FlowImport};
...
/// `src/neppy/flows/tinyflows/caps.rs`).
///
...
pub async fn flows_list_connections(
    ...
    let http_creds =
        match crate::neppy::security::credentials::HttpCredentialsStore::from_config(config).list()
        {
            Ok(list) => {
                tracing::debug!(
                    count = list.len(),
    ...
    let identities =
        crate::neppy::integrations::composio::providers::profile::load_connected_identities();
    tracing::debug!(
...
fn build_flow_connections(
    composio: Vec<crate::neppy::integrations::composio::ComposioConnection>,
    http: Vec<crate::neppy::security::credentials::HttpCredentialSummary>,
    identities: &[crate::neppy::integrations::composio::providers::profile::ConnectedIdentity],
) -> Vec<FlowConnection> {
    use crate::neppy::integrations::composio::providers::profile::normalize_connection_identifier;
...
fn composio_connection_display(
    toolkit: &str,
    conn: &crate::neppy::integrations::composio::ComposioConnection,
) -> String {
...
fn http_credential_display(
    cred: &crate::neppy::security::credentials::HttpCredentialSummary,
) -> String {
...
fn publish_flow_changed(flow_id: &str, kind: &str, actor: &str) {
    ...
    crate::neppy::platform::socket::medulla::workflows::emit_register_workflows();
}
...
pub fn flows_get_history(
    ...
    id: &str,
    limit: usize,
) -> Result<RpcOutcome<Vec<crate::neppy::flows::FlowRevision>>, String> {
...
async fn flows_delete_impl(
    config: &Config,
    id: &str,
    memory_override: Option<Arc<crate::neppy::memory::guard::MemoryGuard>>,
    ...
    let guard = match memory_override {
        Some(guard) => Ok(guard),
        None => crate::neppy::memory::ops::guard::active_memory_guard().await,
    };
...
fn bind_schedule_trigger(config: &Config, flow: &Flow) {
    ...
    let schedule: crate::neppy::cron::Schedule = match serde_json::from_value(schedule_raw) {
        Ok(s) => s,
        Err(e) => {
    ...
    match crate::neppy::cron::find_flow_schedule_job(config, &flow.id) {
        Ok(Some(existing)) => {
            let patch = crate::neppy::cron::CronJobPatch {
                enabled: Some(true),
                schedule: Some(schedule),
                ..Default::default()
            };
            if let Err(e) = crate::neppy::cron::update_job(config, &existing.id, patch) {
        ...
        Ok(None) => match crate::neppy::cron::add_flow_schedule_job(config, &flow.id, schedule) {
            Ok(job) => {
...
fn unbind_schedule_trigger(config: &Config, flow_id: &str) {
    match crate::neppy::cron::find_flow_schedule_job(config, flow_id) {
        Ok(Some(job)) => {
            if let Err(e) = crate::neppy::cron::remove_job(config, &job.id) {
...
async fn export_run_to_langfuse(
    ...
    crate::neppy::flows::tinyflows::langfuse_export::export_flow_run_trace(
        config,
        flow_name,
        flow_id,
        thread_id,
...
async fn run_flow_body(
    ...
    let caps = crate::neppy::flows::tinyflows::build_capabilities(
        config_arc.clone(),
        format!("flow:{flow_id}"),
    );
    let checkpointer = match crate::neppy::flows::tinyflows::open_flow_checkpointer(config) {
    ...
    let observer: Arc<dyn tinyflows::observability::RunObserver> = Arc::new(
        crate::neppy::flows::tinyflows::observability::FlowRunObserver::new(
            Arc::new(config.clone()),
            flow_id,
            thread_id.clone(),
        ),
...
pub async fn flows_resume(
    ...
    let checkpointer = crate::neppy::flows::tinyflows::open_flow_checkpointer(config)
        .map_err(|e| e.to_string())?;
...
async fn drop_checkpoint(config: &Config, thread_id: &str) {
    match crate::neppy::flows::tinyflows::open_flow_checkpointer(config) {
...
/// run's settle path.
///
/// [`FlowRunObserver`]: crate::neppy::flows::tinyflows::observability::FlowRunObserver
...
fn notify_pending_approval(flow: &Flow, thread_id: &str, pending_approvals: &[String]) {
    ...
    use crate::neppy::desktop::notifications::bus::publish_core_notification;
    use crate::neppy::desktop::notifications::types::{
...
fn attach_flow_progress_bridge(
    agent: &mut crate::neppy::agent::Agent,
    ...
    crate::neppy::web_chat::spawn_progress_bridge(
        progress_rx,
        "system".to_string(),
        target.thread_id.clone(),
        target.request_id.clone(),
        crate::neppy::threads::turn_state::TurnStateStore::new(config.workspace_dir.clone()),
        crate::neppy::web_chat::ChatRequestMetadata {
...
async fn finalize_flow_stream(
    ...
    match result {
        Ok(text) => {
            crate::neppy::web_chat::presentation::deliver_response(
                "system",
                &target.thread_id,
                &target.request_id,
                text,
        ...
        Err(err) => {
            crate::neppy::web_chat::publish_web_channel_event(
...
pub async fn flows_discover(
    ...
    use crate::neppy::agent::turn_origin::{with_origin, AgentTurnOrigin};
    ...
    crate::neppy::agent::harness::AgentDefinitionRegistry::init_global(&config.workspace_dir)
...
/// makes [`crate::neppy::security::approval::ApprovalGate`] **auto-allow** every
/// `external_effect` tool. The flows live-runner (`run_flow`,
/// [`crate::neppy::flows::tools`]'s `RunFlowTool`) executes a *live* saved
...
///
/// Delegates to [`crate::neppy::agent::Agent::hide_tools`], which removes
...
fn restrict_builder_toolset(agent: &mut crate::neppy::agent::Agent) {
    tracing::debug!(
        target: "flows",
...
/// the [`crate::neppy::security::approval::ApprovalGate`] no longer auto-allows
...
fn restrict_builder_toolset_for_copilot(agent: &mut crate::neppy::agent::Agent) {
    tracing::info!(
        target: "flows",
...
/// structured [`BuilderRequest`](crate::neppy::flows::agents::workflow_builder::builder_prompt::BuilderRequest)
...
pub async fn flows_build(
    config: &Config,
    req: crate::neppy::flows::agents::workflow_builder::builder_prompt::BuilderRequest,
    stream: Option<FlowStreamTarget>,
) -> Result<RpcOutcome<Value>, String> {
...
pub(crate) async fn flows_build_with_extra_hidden_tools(
    ...
    use crate::neppy::agent::Agent;
    use crate::neppy::flows::agents::workflow_builder::builder_prompt::render_prompt;
    ...
    let approval_gate_active =
        crate::neppy::security::approval::ApprovalGate::try_global().is_some();
    ...
    let timed = match &stream {
        Some(target) => {
            ...
            let run = crate::neppy::agent::tinyagents::thread_context::with_thread_id(
                target.thread_id.clone(),
                run,
            );
...
fn build_trail_off_fallback(
    history: &[crate::neppy::agent::messages::ConversationMessage],
) -> String {
...
fn last_builder_tool_blocker(
    ...
    use crate::neppy::agent::messages::ConversationMessage;
    ...
    for message in history.iter().rev() {
        ...
        for result in results.iter().rev() {
            ...
            return describe_tool_result_blocker(&result.content)
                .map(|desc| crate::neppy::util::truncate_with_ellipsis(&desc, 500));
        }
    }
    None
}
...
/// `crate::neppy::flows::tinyflows::caps` (`NeppyTools::invoke` /
...
pub async fn compute_approval_manifest(config: &Config, graph: &WorkflowGraph) -> Vec<Value> {
    use crate::neppy::flows::tinyflows::caps::classify_composio_action_for_tier;
    use crate::neppy::security::{CommandClass, GateDecision, SecurityPolicy};
        ...
        match node.kind {
            ...
            NodeKind::ToolCall => {
                ...
                match slug {
                    ...
                    Some(s)
                        if s.starts_with(
                            crate::neppy::flows::tinyflows::caps::NATIVE_TOOL_PREFIX,
                        ) =>
                    {
                        let tool_name = s
                            .trim_start_matches(
                        ...
                        let class = crate::neppy::runtime::node::ops::classify_tool_call(
...
pub async fn flows_approval_manifest(
    ...
    let gate = crate::neppy::security::approval::ApprovalGate::try_global();
...
pub async fn flows_search_tool_catalog(
    ...
    let tools =
        crate::neppy::flows::builder_tools::search_live_catalog(config, query, toolkit, limit)
            .await;
    Ok(RpcOutcome::single_log(
        json!({ "tools": tools }),
...
pub async fn flows_get_tool_contract(
    ...
    let Some(catalog) =
        crate::neppy::flows::tinyflows::caps::fetch_live_toolkit_catalog(config, &toolkit).await
    else {
        return Err(format!(
...
pub fn flows_draft_create(
    ...
    name: String,
    graph: Value,
    origin: crate::neppy::flows::DraftOrigin,
) -> Result<RpcOutcome<crate::neppy::flows::FlowDraft>, String> {
...
pub fn flows_draft_list(
    config: &Config,
) -> Result<RpcOutcome<Vec<crate::neppy::flows::FlowDraft>>, String> {
