...
| File | Role |
| --- | --- |
| `src/neppy/test_support/mod.rs` | Export-only module: declares `introspect`, `rpc`, `schemas`; re-exports `all_test_support_controller_schemas` / `all_test_support_registered_controllers`. |
| `src/neppy/test_support/rpc.rs` | `openhuman.test_reset` implementation — `reset()` wipes cron, memory tree, config fields, and active user; returns a `ResetSummary`. Includes the `OPENHUMAN_E2E_MODE` guard and inline tests. |
| `src/neppy/test_support/introspect.rs` | Read-only introspection RPCs: `workspace_root`, `list_workspace_files`, `read_workspace_file`, `in_flight_chats`, `wallet_prepared_quotes`, plus the `resolve_workspace_relative` path guard and BFS `walk_dir`. |
| `src/neppy/test_support/schemas.rs` | `ControllerSchema` definitions, the registered-controller list, and `handle_*` dispatchers that delegate to `rpc`/`introspect` and serialize via `RpcOutcome::into_cli_compatible_json`. |
...
- `crate::neppy::config` — `Config::load_or_init`/`save`, `workspace_dir`, `clear_active_user`, `default_root_openhuman_dir`; config is the source of truth for the workspace root and the fields reset wipes.
- `crate::neppy::cron` — `clear_all_jobs` to wipe scheduled jobs during reset.
- `crate::neppy::memory::read_rpc` — `wipe_all_rpc` to clear memory-tree rows, content dirs, and sync state.
- `crate::neppy::web_chat` — `in_flight_entries_for_test` to snapshot the live `IN_FLIGHT` chat map.
- `crate::neppy::web3::wallet` — `prepared_quotes_for_test` and `PreparedTransaction` to snapshot prepared quotes.
...
- `src/neppy/mod.rs` — declares `pub mod test_support`.
