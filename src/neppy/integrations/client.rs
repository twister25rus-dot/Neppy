...
pub(crate) fn extract_error_detail(body: &str, max_bytes: usize) -> String {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(body) {
        if let Some(msg) = v.get("error").and_then(|e| e.as_str()) {
            ...
            if !trimmed.is_empty() {
                return crate::neppy::util::truncate_at_byte_boundary(trimmed, max_bytes);
            }
        }
    }
    crate::neppy::util::truncate_at_byte_boundary(body, max_bytes)
}
...
/// `neppy::platform::socket::ws_loop`. A *third-party* integration's auth failure
...
fn handle_session_jwt_unauthorized(method: &str, path: &str, url: &str, detail: &str) -> String {
    ...
    crate::core::bus::BUS.publish(crate::core::events::DomainEvent::SessionExpired {
        ...
        reason: crate::neppy::inference::provider::ops::sanitize_api_error(&message),
    });
...
fn backend_egress_descriptor(path: &str) -> crate::neppy::security::egress::EgressDescriptor {
    let endpoint = path.split('?').next().unwrap_or(path);
    crate::neppy::security::egress::EgressDescriptor::integration(endpoint)
}
...
fn emit_backend_egress(path: &str) {
    crate::neppy::security::egress::emit_external_transfer(backend_egress_descriptor(path));
}
...
/// [`is_control_plane`](crate::neppy::security::egress). Call before
/// [`emit_backend_egress`] so a blocked call is neither disclosed nor sent.
fn enforce_backend_egress(path: &str) -> anyhow::Result<()> {
    crate::neppy::security::egress::enforce_egress(&backend_egress_descriptor(path))
}
...
pub struct IntegrationClient {
    ...
    budget_config: Option<Arc<crate::neppy::config::Config>>,
...
impl IntegrationClient {
    ...
    fn new_inner(
        ...
        // are tagged.
        let download_client = crate::neppy::util::tls::tls_client_builder()
            .http1_only()
    ...
    async fn ensure_budget_available(&self, path: &str) -> anyhow::Result<()> {
        ...
        if let Some(config) = &self.budget_config {
            if crate::neppy::hosted::team::managed_tool_budget_exhausted(config).await {
                anyhow::bail!(
...
pub async fn pricing_for_config(
    ...
    use crate::neppy::config::schema::COMPOSIO_MODE_DIRECT;
...
pub fn build_client(config: &crate::neppy::config::Config) -> Option<Arc<IntegrationClient>> {
