...
//! `Tool` trait plumbing in [`crate::neppy::tools`].
//!
...
use crate::neppy::agent::harness::current_sandbox_mode;
use crate::neppy::agent::harness::current_task_recency_window;
...
pub struct ComposioListToolkitsTool {
    ...
    /// [`crate::neppy::config::ComposioConfig::mode`] toggle is
...
impl Tool for ComposioConnectTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        ...
        if crate::neppy::security::approval::APPROVAL_CHAT_CONTEXT
            .try_with(|_| ())
            .is_err()
        {
            return Ok(ToolResult::error(format!(
        ...
        let gate = match crate::neppy::security::approval::ApprovalGate::try_global() {
            Some(g) => g,
            None => {
                return Ok(ToolResult::error(
...
pub struct ComposioListToolsTool {
    ...
    /// [`crate::neppy::integrations::composio::ops::composio_execute`] and avoids
...
pub struct ComposioExecuteTool {
    ...
    /// [`crate::neppy::integrations::composio::ops::composio_execute`]. See
    /// issue #1710.
    config: Arc<Config>,
}
...
pub fn all_composio_agent_tools(config: &crate::neppy::config::Config) -> Vec<Box<dyn Tool>> {
    ...
    if !crate::neppy::agent::harness::subagent_runner::user_is_signed_in_to_composio(config) {
        tracing::debug!(
