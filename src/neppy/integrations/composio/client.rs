...
//! [`crate::neppy::integrations::IntegrationClient`] so they inherit
...
use crate::neppy::integrations::IntegrationClient;
...
impl ComposioClient {
    ...
    pub async fn execute_tool(
        ...
        let egress = crate::neppy::security::egress::EgressDescriptor::composio(tool);
        // Local-only enforcement (privacy epic S7, #4441): refuse the external
        // tool call under LocalOnly BEFORE disclosing or sending it.
        crate::neppy::security::egress::enforce_egress(&egress)?;
        crate::neppy::security::egress::emit_external_transfer(egress);
    ...
    async fn raw_delete<T: serde::de::DeserializeOwned>(&self, path: &str) -> Result<T> {
        ...
        let http_client = crate::neppy::util::tls::tls_client_builder()
            .http1_only()
        ...
        if !status.is_success() {
            ...
            let detail = crate::neppy::integrations::client::extract_error_detail(
                &body_text,
                crate::neppy::integrations::client::MAX_ERROR_BODY_LEN,
            );
            ...
            let logged_body =
                crate::neppy::integrations::client::extract_error_detail(&body_text, 300);
            tracing::debug!(
...
/// [`crate::neppy::agent::harness::subagent_runner::user_is_signed_in_to_composio`]
/// (probe) instead.
///
...
/// [`crate::neppy::integrations::build_client`]. The only reason
...
pub(super) fn build_composio_client(
    ...
    let inner = crate::neppy::integrations::build_client(config)?;
    Some(ComposioClient::new(inner))
}

// ── Direct-mode factory ─────────────────────────────────────────────
//
// Mirrors `src/neppy/inference/embeddings/factory.rs` so anyone reading both
...
use crate::neppy::config::schema::{COMPOSIO_MODE_BACKEND, COMPOSIO_MODE_DIRECT};
...
pub enum ComposioClientKind {
    ...
    Direct(Arc<crate::neppy::tools::ComposioTool>),
}

pub(crate) fn create_direct_composio_tool_for_api_key(
    config: &crate::neppy::config::Config,
    api_key: &str,
) -> anyhow::Result<Arc<crate::neppy::tools::ComposioTool>> {
    ...
    let tool = match (
        ...
        _ => crate::neppy::tools::ComposioTool::new(
            api_key,
            Some(config.composio.entity_id.as_str()),
            security,
        ),
    ...
    let tool = crate::neppy::tools::ComposioTool::new(
        api_key,
        Some(config.composio.entity_id.as_str()),
        security,
    );
...
///   [`crate::neppy::security::credentials::COMPOSIO_DIRECT_PROVIDER`]
...
pub fn create_composio_client(
    ...
    match mode {
        ...
        MODE_DIRECT_PAT => {
            ...
            let stored = crate::neppy::security::credentials::get_composio_api_key(config)
...
/// [`crate::neppy::tools::ComposioTool::get_connection_url`] and
...
/// [`crate::neppy::tools::ComposioTool::list_connected_accounts`])
...
/// [`crate::neppy::tools::ComposioTool::list_tool_schemas_v3`] and
