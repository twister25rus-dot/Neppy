use super::*;
use crate::neppy::integrations::composio::providers::tool_scope::{CuratedTool, ToolScope};
use crate::neppy::integrations::composio::providers::{
...
fn fake_config_arc() -> Arc<crate::neppy::config::Config> {
...
async fn authorize_tool_execute_rejects_missing_toolkit() {
    ...
    let txt = result
        ...
        .filter_map(|c| match c {
            crate::neppy::tools::traits::ToolContent::Text { text } => Some(text.clone()),
            _ => None,
        })
        .collect::<Vec<_>>()
        .join(" ");
...
fn tool_result_text(result: &crate::neppy::tools::traits::ToolResult) -> String {
    result
        .content
        .iter()
        .filter_map(|c| match c {
...
async fn connect_tool_validates_before_gating_in_chat_context() {
    use crate::neppy::security::approval::{ApprovalChatContext, APPROVAL_CHAT_CONTEXT};
...
async fn connection_is_active_errs_without_a_client() {
    ...
    let cfg = crate::neppy::config::Config::default();
...
async fn sandbox_read_only_blocks_write_scope_action() {
    let t = ComposioExecuteTool::new(fake_config_arc());
    let result =
        crate::neppy::agent::harness::with_current_sandbox_mode(SandboxMode::ReadOnly, async {
...
async fn sandbox_sandboxed_mode_does_not_trigger_readonly_gate() {
    ...
    let t = ComposioExecuteTool::new(Arc::new(config));
    let result =
        crate::neppy::agent::harness::with_current_sandbox_mode(SandboxMode::Sandboxed, async {
            t.execute(serde_json::json!({ "tool": "GMAIL_SEND_EMAIL" }))
                .await
                .unwrap()
        })
...
fn render_tools_markdown_groups_by_toolkit_and_drops_schemas() {
    use crate::neppy::integrations::composio::types::{
...
fn render_tools_markdown_handles_empty_response() {
    use crate::neppy::integrations::composio::types::ComposioToolsResponse;
...
fn direct_mode_config() -> crate::neppy::config::Config {
...
fn execute_tool_resolves_to_direct_kind_when_mode_is_direct() {
    ...
    let kind = crate::neppy::integrations::composio::client::create_composio_client(&config)
        .expect("direct mode with inline api_key must resolve");
    assert_eq!(
        kind.mode(),
        crate::neppy::config::schema::COMPOSIO_MODE_DIRECT,
...
fn execute_tool_resolves_to_backend_kind_when_mode_is_backend() {
    ...
    assert_eq!(
        kind.mode(),
        crate::neppy::config::schema::COMPOSIO_MODE_BACKEND,
...
async fn authorize_in_direct_mode_refuses_with_app_composio_dev_hint() {
    ...
    let _cache_guard =
        crate::neppy::integrations::composio::connected_integrations::composio_cache_test_lock();
    let _env_guard = TEST_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
