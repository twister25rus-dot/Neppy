...
| File | Role |
| --- | --- |
| `src/neppy/integrations/composio/mod.rs` | Export-focused module root; re-exports types, ops, schemas, agent tools, trigger-history, and (via `memory_sync::composio`) bus/periodic/providers. |
| `src/neppy/integrations/composio/types.rs` | Serde domain types mirroring backend response envelopes (toolkits, connections, tools, execute, triggers, trigger events/history). Includes drift-tolerant `de_string_or_object` deserializers. |
| `src/neppy/integrations/composio/ops.rs` | RPC-facing `composio_*` operations returning `RpcOutcome<T>`; mode-aware client routing, memory-cleanup targets on delete, provider dispatch for profile/sync, mode + api-key ops, Sentry funnel `report_composio_op_error`. |
| `src/neppy/integrations/composio/schemas.rs` | Controller schemas + `handle_*` handlers; `all_controller_schemas` / `all_registered_controllers`. |
| `src/neppy/integrations/composio/client.rs` | `ComposioClient` (thin HTTP wrapper over `IntegrationClient` for backend routes) + `ComposioClientKind` (Backend/Direct), `create_composio_client`, and direct-mode v3 helpers (`direct_list_connections`, `direct_list_tools`, `direct_execute`, `direct_authorize`). |
| `src/neppy/integrations/composio/tools.rs` | Agent tools (`ComposioListToolkitsTool`, `ComposioListConnectionsTool`, `ComposioAuthorizeTool`, `ComposioListToolsTool`, `ComposioExecuteTool`) + `all_composio_agent_tools`; scope/visibility gating (`resolve_action_scope`, `evaluate_tool_visibility`). |
| `src/neppy/integrations/composio/tools/direct.rs` | Direct-mode Composio tool provider hitting Composio v2/v3 APIs with the user's key (`ComposioTool`, `ComposioAction`, `ComposioConnectedAccount`). |
| `src/neppy/integrations/composio/action_tool.rs` | `ComposioActionTool` — a `Tool` wrapping exactly one Composio action, constructed dynamically when `integrations_agent` is spawned with a toolkit. |
| `src/neppy/integrations/composio/execute_dispatch.rs` | Centralized execute path: prepare args → retry policy (auth/rate-limit) → error mapping; mode-aware over backend/direct. |
| `src/neppy/integrations/composio/execute_prepare.rs` | Local pre-flight argument validation/preparation for action calls. |
| `src/neppy/integrations/composio/auth_retry.rs` | Single-shot retry for the post-OAuth token-propagation gap ("Connection error, try to authenticate"). |
| `src/neppy/integrations/composio/error_mapping.rs` | `ComposioErrorClass` + classifier/formatter so tool failures aren't bucketed as generic gateway 502s (#1797). |
| `src/neppy/integrations/composio/oauth_handoff.rs` | OAuth handoff helpers + Meta (Instagram/Facebook) rate-limit mitigations (#1952); rate-limit error wrapping. |
| `src/neppy/integrations/composio/googlecalendar_args.rs` | Default-args transformer for Google Calendar list/find slugs (timezone/`singleEvents` defaults, #1714). |
| `src/neppy/integrations/composio/identity.rs` | Resolves the connected account username for a toolkit via the provider `fetch_user_profile` path (used by skill preflight identity gate). |
| `src/neppy/integrations/composio/trigger_history.rs` | Persistent JSONL trigger-event archive partitioned by UTC day; global `OnceLock` store (`init_global`/`global`). |
| `src/neppy/integrations/composio/bus.rs` | Compatibility shim re-exporting `memory_sync::composio::bus`. |
| `src/neppy/integrations/composio/periodic.rs` | Compatibility shim re-exporting `memory_sync::composio::periodic` (periodic sync loop). |
| `src/neppy/integrations/composio/providers/mod.rs` | Compatibility shim re-exporting `memory_sync::composio::providers` (native per-toolkit provider registry, curated catalogs, user-scope prefs). |
...
- `crate::neppy::integrations` — shared `IntegrationClient` (Bearer JWT, timeouts, envelope parsing, proxy) backing backend-mode calls.
- `crate::neppy::config` — `Config` / `ComposioConfig` (`mode`, `entity_id`), `config::rpc` config loading.
- `crate::neppy::memory` — the memory client used for ingestion and for reading a provider's sync state during cleanup-target discovery.
- `crate::neppy::memory::binding` — the workspace's bound memory driver. Connection-scoped cleanup deletes through `MemorySourceSink::forget_matching` (the `Source` / `SourcePrefix` / `Owner` selectors), not through the engine's chunk store (#5560). A driver that does not serve `Sources` is refused per target, and the refusal is reported beside `memory_chunks_deleted` rather than read as a delete of nothing.
- `crate::neppy::memory::sync::composio` — the actual home of providers, periodic sync, and bus subscribers (this module re-exports them via shims).
- `crate::neppy::agent::harness` — sandbox mode (`current_sandbox_mode` / `SandboxMode`) for tool gating.
- `crate::neppy::tools::traits` — `Tool`, `ToolResult`, `ToolCategory`, `PermissionLevel`, `ToolCallOptions`.
- `crate::neppy::security` — `SecurityPolicy` / `ToolOperation` for direct-tool gating.
- `crate::neppy::security::credentials` — encrypted store for the direct-mode API key.
- `crate::neppy::agent::context::prompt`, `agent::prompts` — prompt/profile injection of connected identities.
...
- `src/neppy/tools/{mod,ops,schemas}.rs` — wires agent tools into the tool registry.
- `src/neppy/agent/**` — harness/session/subagent spawning (`integrations_agent`), triage escalation, debug.
- `src/neppy/platform/socket/event_handlers.rs` — parses `composio:trigger` and publishes `ComposioTriggerReceived`.
- `src/neppy/heartbeat/planner/collectors.rs`, `subconscious/situation_report` — read connected integrations / calendar.
- `src/neppy/agent/learning/{linkedin_enrichment,profile_md_renderer}.rs`, `memory/read_rpc.rs`, `memory_tree/score/store.rs` — profile/identity + memory consumers.
- `src/neppy/skills/preflight.rs` — identity gate via `connection_identity`.
- `src/neppy/security/credentials/ops.rs`, `channels/runtime/dispatch.rs`.

## Notes / gotchas

- **`bus.rs`, `periodic.rs`, `providers/mod.rs` are compatibility shims** — the real implementations live under `src/neppy/memory/sync/composio/`. Edit there.
