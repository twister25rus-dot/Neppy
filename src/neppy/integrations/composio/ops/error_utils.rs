...
pub(crate) fn direct_mode_without_key(config: &Config) -> OpResult<bool> {
    if config.composio.mode.trim() != crate::neppy::config::schema::COMPOSIO_MODE_DIRECT {
        return Ok(false);
    }
    let has_key = crate::neppy::security::credentials::get_composio_api_key(config)
...
///
/// The shared [`crate::neppy::integrations::IntegrationClient`]
