...
fn build_composio_client_some_with_auth_token() {
    ...
    let mut config = Config::default();
    config.config_path = tmp.path().join("config.toml");
    crate::neppy::security::credentials::AuthService::from_config(&config)
...
async fn authorize_rejects_empty_toolkit() {
    let inner = Arc::new(crate::neppy::integrations::IntegrationClient::new(
...
async fn execute_tool_blocked_under_local_only() {
    ...
    let _mode = crate::neppy::security::live_policy::test_privacy_scope(
        crate::neppy::config::PrivacyMode::LocalOnly,
    );
    let inner = Arc::new(crate::neppy::integrations::IntegrationClient::new(
        "http://127.0.0.1:0".into(),
        "test".into(),
...
fn config_with_session_token(tmp: &tempfile::TempDir) -> crate::neppy::config::Config {
...
fn store_get_clear_composio_api_key_roundtrip() {
    use crate::neppy::security::credentials::{get_composio_api_key, COMPOSIO_DIRECT_PROVIDER};
...
fn direct_tool_for_test() -> std::sync::Arc<crate::neppy::tools::ComposioTool> {
    std::sync::Arc::new(crate::neppy::tools::ComposioTool::new(
        "ck_test_direct",
        Some("default"),
        std::sync::Arc::new(crate::neppy::security::SecurityPolicy::default()),
    ))
}
...
fn direct_tool_for_mock(base_v3: String) -> std::sync::Arc<crate::neppy::tools::ComposioTool> {
...
fn direct_tool_for_mock_with_key(
    base_v3: String,
    api_key: &str,
) -> std::sync::Arc<crate::neppy::tools::ComposioTool> {
    std::sync::Arc::new(crate::neppy::tools::ComposioTool::new_with_v3_base(
        api_key,
        Some("default"),
        std::sync::Arc::new(crate::neppy::security::SecurityPolicy::default()),
        base_v3,
...
impl DirectAuthFailureGuard {
    fn for_tool(tool: &std::sync::Arc<crate::neppy::tools::ComposioTool>) -> Self {
        let key_id = tool.auth_key_fingerprint();
        crate::neppy::integrations::composio::direct_auth::reset_direct_auth_failure(key_id);
        Self { key_id }
    }
}
...
impl Drop for DirectAuthFailureGuard {
    fn drop(&mut self) {
        crate::neppy::integrations::composio::direct_auth::reset_direct_auth_failure(self.key_id);
    }
}
...
async fn pricing_for_config_short_circuits_in_direct_mode() {
    ...
    let client = crate::neppy::integrations::IntegrationClient::new(
        "http://127.0.0.1:0".into(),
        "test".into(),
    );
    let mut config = crate::neppy::config::Config::default();
    ...
    let pricing = crate::neppy::integrations::pricing_for_config(&client, &config).await;
