use super::*;
use crate::neppy::agent::context::prompt::IntegrationConnection;
...
impl DirectAuthFailureGuard {
    fn new(api_key: &str) -> Self {
        let key_id =
            crate::neppy::integrations::composio::direct_auth::fingerprint_api_key(api_key);
...
fn config_with_backend(tmp: &tempfile::TempDir, base: String) -> Config {
    ...
    c.config_path = tmp.path().join("config.toml");
    c.api_url = Some(base);
    crate::neppy::security::credentials::AuthService::from_config(&c)
...
async fn composio_delete_connection_clear_memory_cascades_live_sealed_tree_and_file() {
    use crate::neppy::memory::tree::tree::bucket_seal::{seal_one_level, LabelStrategy};
...
async fn composio_get_user_profile_via_mock_returns_provider_profile() {
    ...
    crate::neppy::integrations::composio::providers::init_default_providers();
...
async fn composio_sync_gmail_via_mock_stores_skill_document_and_updates_outcome() {
    ...
    // earlier test in the same binary happened to run first.
    crate::neppy::memory::host_impls::install_for_tests();
    let _serial = crate::neppy::memory::ops::GLOBAL_MEMORY_TEST_LOCK
        .lock()
        .await;
    use crate::neppy::config::TEST_ENV_LOCK;
    let _cache_guard = cache_guard();
...
fn including_expired_serves_stale_snapshot_for_transient_fallback() {
    ...
    let config = test_config(&tmp);
    let key = crate::neppy::integrations::composio::connected_integrations::cache_key(&config);
    clear_cache_key(&key);
    seed_cache(&key, vec![integration("gmail", true)]);
...
fn direct_mode_config(tmp: &tempfile::TempDir) -> Config {
    ...
    c.workspace_dir = tmp.path().join("workspace");
    c.config_path = tmp.path().join("config.toml");
    c.composio.mode = crate::neppy::config::schema::COMPOSIO_MODE_DIRECT.into();
    crate::neppy::security::credentials::AuthService::from_config(&c)
        .store_provider_token(
            crate::neppy::security::credentials::ops::COMPOSIO_DIRECT_PROVIDER,
            crate::neppy::security::credentials::DEFAULT_AUTH_PROFILE_NAME,
            "ck_test_direct_key",
            std::collections::HashMap::new(),
            true,
...
async fn composio_set_api_key_rejects_invalid_direct_key_before_persisting() {
    use crate::neppy::config::TEST_ENV_LOCK;
    use crate::neppy::security::credentials::get_composio_api_key;
...
async fn composio_set_api_key_validates_candidate_key_even_when_stored_key_exists() {
    use crate::neppy::config::TEST_ENV_LOCK;
    use crate::neppy::security::credentials::{get_composio_api_key, store_composio_api_key};
...
async fn init_memory_client(workspace: &std::path::Path) -> tokio::sync::MutexGuard<'static, ()> {
    let guard = crate::neppy::memory::ops::GLOBAL_MEMORY_TEST_LOCK
        .lock()
        .await;
