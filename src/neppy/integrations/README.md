...
Search provider implementations live under `src/neppy/search/`. This module
...
| File | Role |
| --- | --- |
| `src/neppy/integrations/mod.rs` | Export-only module root. Re-exports `build_client`, `pricing_for_config`, `IntegrationClient`, pricing/envelope types, and `ToolScope`. |
| `src/neppy/integrations/client.rs` | `IntegrationClient` + `post`/`get`/`pricing`, backend URL sanitization, and client construction. |
| `src/neppy/integrations/types.rs` | Shared serde types for backend envelopes and pricing. |
| `src/neppy/integrations/tools.rs` | Aggregates and re-exports non-search integration tool modules. |
| `src/neppy/integrations/tools/apify.rs` | Apify actor run + status + dataset results. |
| `src/neppy/integrations/tools/google_places.rs` | Google Places search + details. |
| `src/neppy/integrations/tools/stock_prices.rs` | Market data via backend financial APIs. |
| `src/neppy/integrations/tools/twilio.rs` | Outbound phone calls via backend Twilio. |

## Search Boundary

Search-owned tools are in `src/neppy/search/tools/`, including Parallel,
Brave, Querit, SearXNG, Seltz, TinyFish, and the managed `WebSearchTool`.
The search registry in `src/neppy/search/registry.rs` decides which search
tool surface is active for `search.engine`.

## Public Surface

From `src/neppy/integrations/mod.rs`:
...
Tools are constructed and registered by `src/neppy/tools/ops.rs`, gated by
...
- Direct search APIs such as SearXNG, Brave, Querit, and Seltz are intentionally
  outside this module in `src/neppy/search/`.
