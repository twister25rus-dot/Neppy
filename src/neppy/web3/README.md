...
`web3_dapp_execute` (registered in `src/neppy/tools/ops.rs`). They call the
...
- [`crate::neppy::web3::wallet`] — `sign_and_broadcast_evm` / `sign_and_broadcast_solana` (crate-internal), `status` for address resolution, `EvmNetwork` / `WalletChain`.
- [`crate::neppy::integrations`] (`IntegrationClient`, `build_client`) — backend auth + transport.
- `crate::neppy::security::approval::APPROVAL_CHAT_CONTEXT` — quote-owner binding.
