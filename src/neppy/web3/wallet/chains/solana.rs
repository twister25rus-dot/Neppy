...
async fn solana_signer(
    ...
    let mnemonic =
        crate::neppy::security::encryption::rpc::decrypt_secret(config, &secret.encrypted_mnemonic)
            .await?
            .value;
...
pub(crate) async fn tinyplace_signer_seed() -> Result<[u8; 32], String> {
    ...
    let exported = crate::neppy::modules::wallet::export_key(&config, &signing_secret)
        .await
...
mod tests {
    ...
    async fn tx_status_reads_signature_status() {
        ...
        assert_eq!(
            info.state,
            crate::neppy::web3::wallet::execution::TxState::Confirmed
        );
