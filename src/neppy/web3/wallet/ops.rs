...
fn keychain_load_mnemonic(config: &Config) -> Option<String> {
    let policy = crate::neppy::security::keyring_consent::policy::check_secret_access();
    if policy != crate::neppy::security::keyring_consent::PolicyDecision::Proceed
        || !crate::neppy::security::keyring::is_available()
    {
    ...
    match crate::neppy::security::keyring::get(&user_id, KEYCHAIN_MNEMONIC_KEY) {
        Ok(Some(val)) => {
...
fn keychain_save_mnemonic(config: &Config, encrypted_mnemonic: &str) -> bool {
    ...
    let user_id = wallet_user_id(config);
    match crate::neppy::security::keyring::set(&user_id, KEYCHAIN_MNEMONIC_KEY, encrypted_mnemonic)
    {
        Ok(()) => {
...
fn keychain_has_mnemonic(config: &Config) -> bool {
    ...
    let user_id = wallet_user_id(config);
    matches!(
        crate::neppy::security::keyring::get(&user_id, KEYCHAIN_MNEMONIC_KEY),
        Ok(Some(_))
    )
}
...
pub async fn reveal_recovery_phrase() -> Result<RpcOutcome<RevealRecoveryPhraseResult>, String> {
    ...
    let phrase = crate::neppy::security::credentials::ops::decrypt_secret(&config, &ciphertext)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    async fn reveal_recovery_phrase_returns_error_when_no_wallet() {
        ...
        let _wallet_lock = crate::neppy::web3::wallet::test_support::TEST_LOCK.lock();
        let _workspace_guard =
            crate::neppy::web3::wallet::test_support::set_workspace_env_for_test(&temp);
    ...
    async fn reveal_recovery_phrase_returns_phrase_for_existing_wallet() {
        let temp = tempfile::tempdir().expect("temp dir");
        let _wallet_lock = crate::neppy::web3::wallet::test_support::TEST_LOCK.lock();
        let _workspace_guard = crate::neppy::web3::wallet::test_support::setup_wallet_in(&temp)
            .await
            .expect("setup wallet");
        ...
        assert_eq!(
            result.value.phrase,
            crate::neppy::web3::wallet::test_support::TEST_MNEMONIC
        );
