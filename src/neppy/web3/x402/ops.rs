...
async fn wallet_signer() -> Result<
    ...
    X402Error,
> {
    use crate::neppy::web3::wallet::WalletChain;

    let secret = crate::neppy::web3::wallet::secret_material(WalletChain::Solana)
        .await
        .map_err(|e| X402Error::Wallet(format!("wallet secret: {e}")))?;

    let config = crate::neppy::config::rpc::load_config_with_timeout()
        .await
        .map_err(|e| X402Error::Wallet(format!("load config: {e}")))?;

    let mnemonic = crate::neppy::security::encryption::rpc::decrypt_secret(
        &config,
    ...
    let account = crate::neppy::modules::wallet::derive_account(&config, &signing_secret)
        .await
...
async fn build_solana_payment(
    ...
    let signature = crate::neppy::modules::wallet::sign_message(
        config,
        signing_secret,
        &message,
...
> {
    ...
    let secret = crate::neppy::web3::wallet::secret_material(WalletChain::Evm)
        .await
        .map_err(|e| X402Error::Wallet(format!("wallet secret: {e}")))?;
...
async fn fetch_recent_blockhash_for_x402() -> Result<[u8; 32], X402Error> {
    ...
    let result: BlockhashResponse = crate::neppy::web3::wallet::rpc::rpc_call(
        WalletChain::Solana,
        "getLatestBlockhash",
