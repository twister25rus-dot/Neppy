...
- `pub fn build_system_prompt` — re-exported from `crate::neppy::agent::context::channels_prompt`.
...
- `src/neppy/agent/` — inbound messages spawn or resume agent runs through `runtime/dispatch.rs`.
- `src/neppy/security/credentials/` — per-channel auth tokens, refresh flow.
- `src/neppy/config/schema/channels.rs` — runtime channel configuration.
- `src/neppy/threads/` — thread state for platforms with native threading (Slack `thread_ts`).
- `src/neppy/desktop/notifications/` — surface inbound deliveries to the UI.
- `src/neppy/security/encryption/` — at-rest secret protection.
- `src/core/event_bus/` — emits `DomainEvent::Channel(*)`; `channels/bus.rs` registers `ChannelInboundSubscriber`.

## Called by

- `src/neppy/threads/ops.rs` — thread lifecycle uses channel send paths.
- `src/neppy/memory/conversations/bus.rs` — persists incoming channel messages as conversation memories.
- `src/neppy/cron/bus.rs` — scheduled triggers can post via channels.
- `src/neppy/config/schema/channels.rs` — config layer references channel types for validation.
