...
use crate::neppy::agent::context::channels_prompt::build_system_prompt;
use crate::neppy::agent::harness::build_tool_instructions_filtered;
use crate::neppy::agent::host_runtime;
use crate::neppy::channels::context::{
...
use crate::neppy::channels::dingtalk::DingTalkChannel;
use crate::neppy::channels::discord::DiscordChannel;
use crate::neppy::channels::email_channel::EmailChannel;
use crate::neppy::channels::imessage::IMessageChannel;
use crate::neppy::channels::irc;
use crate::neppy::channels::irc::IrcChannel;
use crate::neppy::channels::lark::LarkChannel;
use crate::neppy::channels::linq::LinqChannel;
use crate::neppy::channels::mattermost::MattermostChannel;
use crate::neppy::channels::qq::QQChannel;
use crate::neppy::channels::signal::SignalChannel;
use crate::neppy::channels::slack::SlackChannel;
use crate::neppy::channels::telegram::TelegramChannel;
use crate::neppy::channels::traits;
use crate::neppy::channels::whatsapp::WhatsAppChannel;
#[cfg(feature = "whatsapp-web")]
use crate::neppy::channels::whatsapp_web::WhatsAppWebChannel;
use crate::neppy::channels::yuanbao::YuanbaoChannel;
use crate::neppy::channels::Channel;
use crate::neppy::config::Config;
use crate::neppy::inference::provider;
use crate::neppy::security::SecurityPolicy;
use crate::neppy::tools;
use anyhow::Result;
...
async fn start_relay_runtime(
    ...
    crate::neppy::channels::relay_runtime::register_relay_transport(transport.clone());
...
pub async fn start_channels(mut config: Config) -> Result<()> {
    ...
    // no connections.
    crate::neppy::integrations::composio::start_periodic_sync();
    ...
    crate::neppy::config::ensure_agent_dirs(&mut config).await;
    ...
    let security = crate::neppy::security::live_policy::install(
        Arc::new(
    ...
    let skills = crate::neppy::skills::load_workflow_metadata(&workspace);
    ...
    crate::neppy::skills::bus::ensure_triggered_workflow_subscriber(&workspace);
    ...
    // subagent via category_filter = "skill".
    let non_skill_tools: Vec<&Box<dyn crate::neppy::tools::Tool>> = tools_registry
        .iter()
        .filter(|t| t.category() != crate::neppy::tools::traits::ToolCategory::Workflow)
        .collect();
    let non_skill_refs: Vec<&dyn crate::neppy::tools::Tool> =
    ...
    let channel_host = crate::neppy::channels::host::build_channel_host(Arc::new(config.clone()));
    ...
    if let Some(ref tg) = config.channels_config.telegram {
        ...
        .with_http_client(crate::neppy::config::build_runtime_proxy_client(
            "channel.telegram",
        ));
    ...
    if let Some(ref dc) = config.channels_config.discord {
        channels.push(Arc::new(DiscordChannel::with_http_client(
            ...
            dc.listen_to_bots,
            dc.mention_only,
            crate::neppy::config::build_runtime_proxy_client("channel.discord"),
        )));
    }
    ...
    if let Some(ref sl) = config.channels_config.slack {
        channels.push(Arc::new(SlackChannel::with_http_client(
            ...
            crate::neppy::config::build_runtime_proxy_client("channel.slack"),
        )));
    ...
    if let Some(ref mm) = config.channels_config.mattermost {
        channels.push(Arc::new(MattermostChannel::with_http_client(
            ...
            crate::neppy::config::build_runtime_proxy_client("channel.mattermost"),
        )));
    }
    ...
    if let Some(ref sig) = config.channels_config.signal {
        channels.push(Arc::new(SignalChannel::with_http_client(
            ...
            sig.ignore_stories,
            crate::neppy::config::apply_runtime_proxy_to_builder(
            ...
            "cloud" => {
                ...
                if wa.is_cloud_config() {
                    channels.push(Arc::new(WhatsAppChannel::with_http_client(
                        ...
                        crate::neppy::config::build_runtime_proxy_client("channel.whatsapp"),
                    )));
                } else {
    ...
    if let Some(ref dt) = config.channels_config.dingtalk {
        channels.push(Arc::new(DingTalkChannel::with_http_client(
            ...
            crate::neppy::config::build_runtime_proxy_client("channel.dingtalk"),
        )));
    }
    ...
    if let Some(ref qq) = config.channels_config.qq {
        channels.push(Arc::new(QQChannel::with_http_client(
            ...
            crate::neppy::config::build_runtime_proxy_client("channel.qq"),
        )));
    }
    ...
    let _cron_delivery_handle = bus.subscribe(Arc::new(
        crate::neppy::cron::bus::CronDeliverySubscriber::new(Arc::clone(&channels_by_name)),
    ));
    ...
    let proactive_sub = crate::neppy::channels::proactive::ProactiveMessageSubscriber::new(
    ...
    crate::neppy::channels::proactive::register_active_channel_handle(
    ...
    let _telegram_remote_handle = if channels_by_name.contains_key("telegram") {
        let handle = bus.subscribe(Arc::new(
            crate::neppy::channels::providers::telegram::TelegramRemoteSubscriber::new(
                config.workspace_dir.clone(),
            ),
        ));
    ...
    let _telegram_approval_surface_handle = if channels_by_name.contains_key("telegram") {
        let handle = bus.subscribe(Arc::new(
            crate::neppy::channels::providers::telegram::TelegramApprovalSurfaceSubscriber::new(
                Arc::clone(&channels_by_name),
            ),
        ));
    ...
    let _tree_summarizer_handle = bus.subscribe(Arc::new(
        crate::neppy::memory::tree::tree_runtime::bus::TreeSummarizerEventSubscriber::new(),
    ));
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        memory: crate::neppy::memory::ops::guard::active_memory_guard()
            .await
...
fn format_access_context(security: &SecurityPolicy) -> String {
    use crate::neppy::security::{AutonomyLevel, TrustedAccess};
...
fn resolve_yuanbao_app_secret(
    mut yb_cfg: crate::neppy::channels::providers::yuanbao::YuanbaoConfig,
    config: &Config,
) -> crate::neppy::channels::providers::yuanbao::YuanbaoConfig {
    if !yb_cfg.app_secret.is_empty() {
        return yb_cfg;
    }
    let auth = crate::neppy::security::credentials::AuthService::from_config(config);
...
fn resolve_email_password(
    mut email_cfg: crate::neppy::channels::email_channel::EmailConfig,
    config: &Config,
) -> crate::neppy::channels::email_channel::EmailConfig {
...
pub mod test_support {
    ...
    pub fn resolve_yuanbao_app_secret_for_test(
        yb_cfg: crate::neppy::channels::providers::yuanbao::YuanbaoConfig,
        config: &Config,
    ) -> crate::neppy::channels::providers::yuanbao::YuanbaoConfig {
...
mod yuanbao_secret_tests {
    use super::*;
    use crate::neppy::channels::providers::yuanbao::YuanbaoConfig;
...
mod email_secret_tests {
    use super::*;
    use crate::neppy::channels::email_channel::EmailConfig;
    use crate::neppy::security::credentials::AuthService;
    use std::collections::HashMap;
    use tempfile::tempdir;
