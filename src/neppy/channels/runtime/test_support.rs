...
use crate::neppy::channels::context::{ChannelRuntimeContext, CHANNEL_MESSAGE_TIMEOUT_SECS};
use crate::neppy::channels::traits::{ChannelMessage, SendMessage};
use crate::neppy::channels::Channel;
use crate::neppy::config::{MultimodalConfig, MultimodalFileConfig, ReliabilityConfig};
...
/// query.
///
/// Deliberately not [`InMemoryProvider`](crate::neppy::memory::guard::in_memory::InMemoryProvider):
...
pub async fn run_dispatch_harness(options: DispatchHarnessOptions) -> DispatchHarnessObservation {
    ...
    provider_cache.insert(
        ...
        crate::neppy::agent::tinyagents::TurnModelSource::from_model(Arc::clone(&model)),
    );
    ...
    let ctx = Arc::new(ChannelRuntimeContext {
        ...
        memory: crate::neppy::memory::guard::in_memory::guard_over(Arc::new(HarnessMemory {
            entries: options
                .memory_entries
                .into_iter()
                .map(memory_entry)
