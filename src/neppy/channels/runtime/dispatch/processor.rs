...
use crate::neppy::agent::bus::{AgentTurnRequest, AgentTurnResponse, AGENT_RUN_TURN_METHOD};
...
use crate::neppy::channels::providers::telegram::TELEGRAM_APPROVAL_CLIENT_ID;
use crate::neppy::channels::routes::{
    get_or_create_turn_model_source, get_route_selection, handle_runtime_command_if_needed,
};
use crate::neppy::channels::traits;
use crate::neppy::channels::{ChannelSendExt, SendMessage};
...
///
/// [`ApprovalChatContext`]: crate::neppy::security::approval::ApprovalChatContext
/// [`ApprovalGate`]: crate::neppy::security::approval::ApprovalGate
...
/// `web_chat/`.
///
/// [`ApprovalGate::decide`]: crate::neppy::security::approval::ApprovalGate::decide
async fn try_route_approval_reply(msg: &traits::ChannelMessage) -> bool {
    ...
    let Some(decision) = crate::neppy::security::approval::parse_approval_reply(&msg.content)
    else {
        return false;
    };
...
pub(crate) async fn process_channel_runtime_message(
    ...
    // `crate::neppy::agent::bus::register_agent_handlers`); this keeps
    ...
    let turn_origin = crate::neppy::agent::turn_origin::AgentTurnOrigin::ExternalChannel {
        channel: msg.channel.clone(),
    ...
    let turn_request = AgentTurnRequest {
        ...
        multimodal_files: crate::neppy::config::MultimodalFileConfig::for_untrusted_channel_input(),
    ...
    let llm_result = tokio::time::timeout(Duration::from_secs(ctx.message_timeout_secs), async {
        if channel_has_approval_surface(&msg.channel) {
            ...
            crate::neppy::security::approval::APPROVAL_CHAT_CONTEXT
    ...
    let (success, response_text, response_provider, response_model) = match llm_result {
        Ok(Ok(response)) => {
            ...
            // Save user + assistant turn to per-sender history
            {
                ...
                while turns.len() > crate::neppy::channels::context::MAX_CHANNEL_HISTORY {
                    turns.remove(0);
                }
            }
            println!(
        ...
        Ok(Err(e)) => {
            ...
            // arm is unchanged.
            if crate::neppy::agent::error::is_max_iterations_error(&e.to_string()) {
                log::info!(
                    target: "channels",
