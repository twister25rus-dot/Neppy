...
impl Transcriber for VoiceTranscriber {
    ...
    async fn transcribe(
        ...
        let provider = crate::neppy::voice::effective_stt_provider(&self.config);
        tracing::debug!(
        ...
        let stt = crate::neppy::voice::create_stt_provider(&provider, "", &self.config)?;
        let outcome = stt
            .transcribe(
                &self.config,
...
impl SpeechSynthesizer for VoiceSynthesizer {
    async fn synthesize(&self, request: SpeechRequest) -> anyhow::Result<SpeechResult> {
        ...
        let opts = crate::neppy::voice::reply_speech::ReplySpeechOptions {
        ...
        let outcome =
            crate::neppy::voice::reply_speech::synthesize_reply(&self.config, &request.text, &opts)
                .await
                .map_err(|e| anyhow::anyhow!(e))?;
...
impl ReactionGate for InferenceReactionGate {
    async fn should_react(&self, query: ReactionQuery) -> anyhow::Result<ReactionDecision> {
        ...
        let outcome = crate::neppy::inference::ops::inference_should_react(
            &self.config,
            &query.message,
            &query.channel_type,
        )
...
impl ApprovalGate for CoreApprovalGate {
    fn parse_reply(&self, message: &str) -> Option<ApprovalDecision> {
        crate::neppy::security::approval::parse_approval_reply(message).map(|decision| {
            use crate::neppy::security::approval::ApprovalDecision as Core;
            match decision {
    ...
    async fn publish(
        ...
        match domain {
            "web" => {
                ...
                crate::neppy::web_chat::publish_web_channel_event(event);
            }
            "channel" => {
