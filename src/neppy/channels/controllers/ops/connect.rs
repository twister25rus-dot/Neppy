...
use crate::neppy::channels::email_channel::{EmailChannel, EmailConfig};
use crate::neppy::channels::providers::yuanbao::YuanbaoConfig;
use crate::neppy::channels::traits::Channel;
use crate::neppy::config::{Config, DiscordConfig, IMessageConfig, TelegramConfig};
    ...
    if channel_id == "telegram" && auth_mode == ChannelAuthMode::BotToken {
        ...
        let (stream_mode, draft_update_interval_ms, silent_streaming, mention_only) =
            if let Some(existing) = persisted.channels_config.telegram.as_ref() {
                ...
                (
                    crate::neppy::config::StreamMode::default(),
                    1000,
                    true,
                    false,
                )
...
/// ([`crate::neppy::channels::proactive::set_runtime_active_channel`]) so the
...
pub async fn set_default_channel(
    ...
    crate::neppy::channels::proactive::set_runtime_active_channel(Some(canonical.clone()));
