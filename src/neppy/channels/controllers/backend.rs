...
use crate::neppy::config::{ChannelsConfig, Config};
...
impl ChannelBackend for NeppyChannelBackend {
    ...
    async fn send_outbound_intent(
        ...
        if crate::neppy::channels::relay_runtime::relay_runtime_fronts_channel(
            channels_config,
            &intent.channel_id,
        ) {
            if let Some(result) =
                crate::neppy::channels::relay_runtime::send_outbound_intent(&intent).await?
            {
                return Ok(result);
            }
        }
