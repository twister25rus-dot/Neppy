...
use crate::neppy::agent::context::channels_prompt::{build_system_prompt, BOOTSTRAP_MAX_CHARS};
...
fn prompt_skills_compact_list() {
    let ws = make_workspace();
    let skills = vec![crate::neppy::skills::Workflow {
...
fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
    ...
    let result = std::panic::catch_unwind(|| crate::neppy::util::truncate_with_ellipsis(msg, 80));
    assert!(
        result.is_ok(),
