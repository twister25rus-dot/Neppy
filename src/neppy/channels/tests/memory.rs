...
use crate::neppy::memory::{Memory, MemoryCategory};
...
async fn autosave_keys_preserve_multiple_conversation_facts() {
    ...
    let recalled = mem
        .recall("45", 5, crate::neppy::memory::RecallOpts::default())
        .await
        .unwrap();
...
async fn build_memory_context_includes_recalled_entries() {
    let (_provider, mem) = crate::neppy::memory::guard::in_memory::guarded_in_memory();
    mem.store(
        "",
        "age_fact",
        "Age is 45",
...
async fn process_channel_message_restores_per_sender_history_on_follow_ups() {
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        turn_model_source: Some(
            crate::neppy::agent::tinyagents::TurnModelSource::from_model(provider_impl.clone()),
        ),
...
async fn process_channel_message_uses_autosaved_memory_after_history_is_cleared() {
    ...
    let provider_impl = Arc::new(HistoryCaptureModel::default());
    let (_memory_provider, memory) = crate::neppy::memory::guard::in_memory::guarded_in_memory();
