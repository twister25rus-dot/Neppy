...
async fn process_channel_message_handles_models_command_without_llm_call() {
    ...
    provider_cache_seed.insert(
        ...
        crate::neppy::agent::tinyagents::TurnModelSource::from_model(Arc::clone(&default_provider)),
    );
    provider_cache_seed.insert(
        "openrouter".to_string(),
        crate::neppy::agent::tinyagents::TurnModelSource::from_model(fallback_provider),
    );

    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        channels_by_name: Arc::new(channels_by_name),
        turn_model_source: Some(
            crate::neppy::agent::tinyagents::TurnModelSource::from_model(Arc::clone(
                &default_provider,
            )),
        ),
        default_provider: Arc::new("test-provider".to_string()),
...
async fn process_channel_message_uses_route_override_provider_and_model() {
    ...
    provider_cache_seed.insert(
        "openrouter".to_string(),
        crate::neppy::agent::tinyagents::TurnModelSource::from_model(routed_provider),
    );
