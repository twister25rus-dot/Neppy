...
impl TurnStateMirror {
    ...
    fn persist_interrupted_partial(&self) {
        ...
        let Some(path) =
            crate::neppy::agent::harness::session::transcript::find_root_transcript_for_thread(
                workspace_dir,
                thread_id,
            )
        else {
        ...
        match crate::neppy::agent::harness::session::transcript::append_interrupted_partial(
            &path,
            partial,
            request_id,
