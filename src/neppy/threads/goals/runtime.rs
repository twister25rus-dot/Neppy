    ...
    matches!(
        ...
        Some(
            crate::neppy::agent::turn_origin::AgentTurnOrigin::TrustedAutomation {
                source: crate::neppy::agent::turn_origin::TrustedAutomationSource::GoalContinuation,
                ..
            }
        )
    )
...
mod tests {
    use super::*;
    use crate::neppy::agent::cost::TurnCost;
    ...
    async fn account_turn_charges_active_goal_and_trips_budget() {
        ...
        crate::neppy::agent::tinyagents::thread_context::with_thread_id("t-acct", async {
    ...
    async fn account_turn_skips_non_active_goal() {
        let tmp = tempfile::tempdir().unwrap();
        let dir = tmp.path().to_path_buf();
        crate::neppy::agent::tinyagents::thread_context::with_thread_id("t-paused", async {
    ...
    async fn account_turn_clears_suppression_without_losing_usage() {
        let tmp = tempfile::tempdir().unwrap();
        let dir = tmp.path().to_path_buf();
        crate::neppy::agent::tinyagents::thread_context::with_thread_id("t-suppressed", async {
