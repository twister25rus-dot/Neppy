...
fn host(config: &Config) -> std::sync::Arc<neppy_core::neppy::mcp::host::McpHost> {
    neppy_core::neppy::mcp::host::for_config(config).expect("the mcp host opens")
}
...
async fn supervise_once(host: &neppy_core::neppy::mcp::host::McpHost) {
...
async fn boot_skips_disabled_servers_and_records_errors() {
    use neppy_core::neppy::mcp::registry::boot;
...
async fn set_enabled_false_disconnects_running_server() {
    use neppy_core::neppy::mcp::registry::ops;
