...
use neppy_core::neppy::memory::sources::readers::reader_for;
use neppy_core::neppy::memory::sources::registry;
use neppy_core::neppy::memory::sources::rpc as memory_sources_rpc;
...
use neppy_core::neppy::memory::sources::types::{
    ContentType, MemorySourceEntry, SourceContent, SourceItem, SourceKind,
};
use neppy_core::neppy::memory::sources::{
...
use neppy_core::neppy::memory::sync::composio;
...
use neppy_core::neppy::memory::sync::composio::providers::profile_md::{
...
use neppy_core::neppy::memory::sync::composio::providers::sync_state::{
    extract_item_id, DailyBudget, PersistedSyncState, SyncState, DEFAULT_DAILY_REQUEST_LIMIT,
};
use neppy_core::neppy::memory::sync::composio::providers::user_scopes;
...
use neppy_core::neppy::memory::tree::score::resolver::CanonicalEntity;
use neppy_core::neppy::memory::tree::score::signals::{
...
use neppy_core::neppy::memory::tree::score::store as score_store;
use neppy_core::neppy::memory::tree::score::{resolver, ScoringConfig};
use neppy_core::neppy::memory::tree::summarise::{
    fallback_summary, SummaryContext, SummaryInput,
};
use neppy_core::neppy::memory::tree::tree::bucket_seal::LeafRef;
use neppy_core::neppy::memory::tree::tree_runtime::store as tree_runtime_store;
...
use neppy_core::neppy::memory::tree::{retrieval, score::embed};
...
use neppy_core::neppy::memory::api::provider::MemoryCore;
use neppy_core::neppy::memory::api::types::MemoryTaint;
// These request/record types are consumed directly by `neppy_core::neppy::memory::ops`
// and `neppy::threads::ops` handlers below, which take the host's own `rpc_models` types,
// not the engine crate's same-named ones — so they must come from the host, not `tinymemory_core`.
use neppy_core::neppy::memory::rpc_models::{
...
use neppy_core::neppy::threads::title::{
...
use neppy_core::neppy::threads::ThreadsError;
use neppy_core::neppy::threads::{
    all_threads_controller_schemas, all_threads_registered_controllers,
};
use neppy_core::neppy::tools::traits::{PermissionLevel, Tool, ToolCategory};
...
fn module_workspace() -> &'static Path {
    ...
    WORKSPACE.get_or_init(|| {
        ...
        ensure_memory_seams();
        #[cfg(feature = "modules")]
        neppy_core::neppy::modules::memory::set_modules_policy(Arc::new(shared_config_at(
            &path,
        )));
        path
    })
    ...
    let extract_again = memory
        .extract_graph(
            ...
            &MemoryIngestionConfig {
                extraction_mode: neppy_core::neppy::memory::ExtractionMode::Chunk,
                ..Default::default()
            },
        )
        .await
...
async fn memory_thread_tree_and_sync_controller_schemas_execute_public_handlers() {
    ...
    assert_eq!(
        neppy_core::neppy::threads::schemas::schemas("missing").function,
        "unknown"
    );
...
fn memory_schema_registries_and_query_tool_metadata_cover_public_surfaces() {
    ...
    for function in [
        ...
        let schema = neppy_core::neppy::memory::schemas::schemas(function);
    ...
    assert_eq!(
        neppy_core::neppy::memory::schemas::schemas("missing").function,
        "unknown"
    );

    let legacy_tree_schemas = neppy_core::neppy::memory::schema::all_controller_schemas();
    let legacy_tree_controllers =
        neppy_core::neppy::memory::schema::all_registered_controllers();
    assert!(
    ...
    for function in [
        ...
        let schema = neppy_core::neppy::memory::schema::schemas(function);
    ...
    assert_eq!(
        neppy_core::neppy::memory::schema::schemas("missing").function,
        "unknown"
    );
...
fn memory_tree_scoring_signal_helpers_cover_boundaries_and_serialization() {
    ...
    let regex_entities = neppy_core::neppy::memory::tree::score::extract::regex::extract(
...
fn memory_tree_runtime_store_buffers_and_retrieval_wire_helpers() {
    ...
    let source_factory = neppy_core::neppy::memory::tree::tree::TreeFactory::source(
        "gmail:alice@example.com|bob@example.com",
    );
    assert_eq!(
        source_factory.profile(),
        neppy_core::neppy::memory::tree::tree::TreeProfile::Source
    );
    assert_eq!(
    ...
    assert_eq!(
        neppy_core::neppy::memory::tree::tree::TreeFactory::from_tree(&source_tree).kind(),
        TreeKind::Source
    );
    let topic_factory = neppy_core::neppy::memory::tree::tree::TreeFactory::topic(
    ...
    assert!(
        neppy_core::neppy::memory::tree::tree::new_tree_id(TreeKind::Global)
            .starts_with("global:")
    );
    assert!(neppy_core::neppy::memory::tree::tree::new_summary_id(2).contains(":L2-"));
    assert!(
        neppy_core::neppy::memory::tree::tree::registry::is_unique_violation(
    ...
    assert_eq!(
        neppy_core::neppy::memory::tree::tree::store::get_tree_by_scope(
            &config,
            TreeKind::Source,
            "gmail:alice@example.com|bob@example.com"
        )
...
async fn memory_read_rpc_score_index_and_summary_helpers_cover_dashboard_paths() {
    ...
    let empty =
        neppy_core::neppy::memory::tree::summarise::summarise(&config, &[], &empty_ctx)
            .await
            .expect("empty summarise avoids provider");
    assert_eq!(empty.token_count, 0);

    let embedder =
        neppy_core::neppy::memory::tree::score::embed::factory::build_embedder_from_config(
            &config,
        )
...
async fn memory_preferences_remember_redaction_and_pipeline_traits_cover_public_edges() {
    ...
    let (_provider, memory) = neppy_core::neppy::memory::guard::in_memory::guarded_in_memory();
    ...
    memory
        .store(
            ...
            neppy_core::neppy::memory::api::types::MemoryCategory::Core,
            None,
...
fn memory_tree_io_contract_types_round_trip_leaf_read_and_write_shapes() {
    ...
    let payload = neppy_core::neppy::memory::tree::TreeLeafPayload {
    ...
    let round_trip =
        neppy_core::neppy::memory::tree::TreeLeafPayload::from(leaf_ref.clone());
    assert_eq!(round_trip.content, payload.content);
    assert_eq!(round_trip.score, payload.score);

    let write_default_json =
        serde_json::to_value(neppy_core::neppy::memory::tree::TreeWriteRequest {
    ...
    let decoded_write: neppy_core::neppy::memory::tree::TreeWriteRequest =
    ...
    assert_eq!(
        ...
        neppy_core::neppy::memory::tree::TreeLabelStrategy::Empty
    );
    assert!(decoded_write.leaf.entities.is_empty());
    assert!(decoded_write.deferred);

    let outcome = neppy_core::neppy::memory::tree::TreeWriteOutcome {
    ...
    let read_request: neppy_core::neppy::memory::tree::TreeReadRequest =
        serde_json::from_value(json!({
            "tree_id": "tree-contract",
            "max_depth": 2,
            "query": "coverage",
    ...
    let hit = neppy_core::neppy::memory::tree::TreeReadHit {
    ...
    let result = neppy_core::neppy::memory::tree::TreeReadResult {
        hits: vec![hit],
        total: 4,
        tree_id: "tree-contract".into(),
    };
    ...
    let empty = neppy_core::neppy::memory::tree::TreeReadResult::empty(&tree);
...
fn gmail_post_processor_and_provider_registry_cover_public_edges() {
    let gmail_provider =
        neppy_core::neppy::memory::sync::composio::providers::gmail::GmailProvider::new();
...
fn memory_sync_profile_markdown_and_status_helpers_are_idempotent() {
    ...
    let now = 1_700_000_000_000_i64;
    assert_eq!(
        neppy_core::neppy::memory::sync::sync_status::FreshnessLabel::from_age_ms(
            Some(now - 30_000),
            now
        ),
        neppy_core::neppy::memory::sync::sync_status::FreshnessLabel::Active
    );
    assert_eq!(
        neppy_core::neppy::memory::sync::sync_status::FreshnessLabel::from_age_ms(
            Some(now - 30_001),
        ...
        neppy_core::neppy::memory::sync::sync_status::FreshnessLabel::Recent
    );
    assert_eq!(
        neppy_core::neppy::memory::sync::sync_status::FreshnessLabel::from_age_ms(
            None, now
        ),
        neppy_core::neppy::memory::sync::sync_status::FreshnessLabel::Idle
    );
}
...
async fn memory_sources_registry_rpc_and_schema_handlers_cover_crud_edges() {
    ...
    assert_eq!(
        neppy_core::neppy::memory::sources::schemas::schemas("read_item").function,
        "read_item"
    );
...
async fn memory_ops_public_handlers_cover_document_file_kv_graph_and_envelopes_body() {
    ...
    let sync_channel = neppy_core::neppy::memory::ops::memory_sync_channel(
        neppy_core::neppy::memory::ops::SyncChannelParams {
    ...
    let sync_all = neppy_core::neppy::memory::ops::memory_sync_all()
        .await
        .expect("sync all request")
        .value;
    assert!(sync_all.requested);
    let ingestion = neppy_core::neppy::memory::ops::memory_ingestion_status()
        .await
        .expect("ingestion status")
        .value;
    assert_eq!(ingestion.queue_depth, 0);
    let learn_none = neppy_core::neppy::memory::ops::memory_learn_all(
        neppy_core::neppy::memory::ops::LearnAllParams {
    ...
    let write =
        neppy_core::neppy::memory::ops::ai_write_memory_file(WriteMemoryFileRequest {
    ...
    let read = neppy_core::neppy::memory::ops::ai_read_memory_file(ReadMemoryFileRequest {
        relative_path: "notes/raw.md".into(),
    })
    .await
    ...
    let root_files =
        neppy_core::neppy::memory::ops::ai_list_memory_files(ListMemoryFilesRequest {
            relative_dir: "".into(),
        })
        .await
    ...
    let document_id = neppy_core::neppy::memory::ops::doc_put(
        neppy_core::neppy::memory::ops::PutDocParams {
    ...
    let namespaces = neppy_core::neppy::memory::ops::namespace_list()
        .await
        .expect("namespace list")
        .value;
    assert!(namespaces.iter().any(|candidate| candidate == namespace));
    let learn_disabled = neppy_core::neppy::memory::ops::memory_learn_all(
        neppy_core::neppy::memory::ops::LearnAllParams {
    ...
    let direct_docs = neppy_core::neppy::memory::ops::doc_list(Some(
    ...
    let envelope_docs =
        neppy_core::neppy::memory::ops::memory_list_documents(ListDocumentsRequest {
    ...
    let query = neppy_core::neppy::memory::ops::context_query(
        neppy_core::neppy::memory::ops::QueryNamespaceParams {
            namespace: namespace.into(),
    ...
    let recalled = neppy_core::neppy::memory::ops::context_recall(
        neppy_core::neppy::memory::ops::RecallNamespaceParams {
            namespace: namespace.into(),
            limit: Some(5),
        },
    )
    ...
    neppy_core::neppy::memory::ops::kv_set(
        neppy_core::neppy::memory::ops::KvSetParams {
            namespace: Some(namespace.into()),
            key: "state".into(),
    ...
    .await
    .expect("kv set");
    let kv = neppy_core::neppy::memory::ops::kv_get(
        neppy_core::neppy::memory::ops::KvGetDeleteParams {
            namespace: Some(namespace.into()),
            key: "state".into(),
        },
    )
    ...
    let kv_rows = neppy_core::neppy::memory::ops::kv_list_namespace(
        neppy_core::neppy::memory::ops::NamespaceOnlyParams {
            namespace: namespace.into(),
        },
    )
    ...
    assert!(
        neppy_core::neppy::memory::ops::kv_delete(
            neppy_core::neppy::memory::ops::KvGetDeleteParams {
                namespace: Some(namespace.into()),
                key: "state".into(),
            },
    ...
    neppy_core::neppy::memory::ops::graph_upsert(
        neppy_core::neppy::memory::ops::GraphUpsertParams {
            namespace: Some(namespace.into()),
            subject: "Alice".into(),
            predicate: "OWNS".into(),
    ...
    .await
    .expect("graph upsert");
    let relations = neppy_core::neppy::memory::ops::graph_query(
        neppy_core::neppy::memory::ops::GraphQueryParams {
            namespace: Some(namespace.into()),
    ...
    let tool_rule = neppy_core::neppy::memory::ops::tool_rule_put(
        neppy_core::neppy::memory::ops::ToolRulePutParams {
    ...
    let fetched_rule = neppy_core::neppy::memory::ops::tool_rule_get(
        neppy_core::neppy::memory::ops::ToolRuleRefParams {
            tool_name: "shell".into(),
            id: "ops-rule-1".into(),
        },
    )
    ...
    let listed_rules = neppy_core::neppy::memory::ops::tool_rule_list(
        neppy_core::neppy::memory::ops::ToolRuleListParams {
            tool_name: "shell".into(),
        },
    )
    .await
    ...
    let prompt_rules = neppy_core::neppy::memory::ops::tool_rules_for_prompt(
        neppy_core::neppy::memory::ops::ToolRulesForPromptParams {
    ...
    let tool_rules_json = neppy_core::neppy::memory::ops::tool_rules_json(
        neppy_core::neppy::memory::ops::ToolRuleListParams {
            tool_name: "shell".into(),
        },
    )
    ...
    assert!(
        neppy_core::neppy::memory::ops::tool_rule_delete(
            neppy_core::neppy::memory::ops::ToolRuleRefParams {
                tool_name: "shell".into(),
                id: "ops-rule-1".into(),
            },
    ...
    assert!(neppy_core::neppy::memory::ops::tool_rule_get(
        neppy_core::neppy::memory::ops::ToolRuleRefParams {
            tool_name: "shell".into(),
            id: "ops-rule-1".into(),
        },
    ...
    let delete_missing =
        neppy_core::neppy::memory::ops::memory_delete_document(DeleteDocumentRequest {
            namespace: namespace.into(),
    ...
    let deleted = neppy_core::neppy::memory::ops::doc_delete(
        neppy_core::neppy::memory::ops::DeleteDocParams {
            namespace: namespace.into(),
            document_id,
        },
    )
    ...
    let cleared = neppy_core::neppy::memory::ops::clear_namespace(
        neppy_core::neppy::memory::ops::ClearNamespaceParams {
            namespace: namespace.into(),
        },
    )
    .await
...
async fn memory_tree_retrieval_rpc_and_schema_wrappers_cover_empty_and_invalid_paths() {
    ...
    let schemas =
        neppy_core::neppy::memory::tree::retrieval::schemas::all_controller_schemas();
    let controllers =
        neppy_core::neppy::memory::tree::retrieval::schemas::all_registered_controllers();
    assert_eq!(schemas.len(), 5);
    assert_eq!(schemas.len(), controllers.len());
    assert_eq!(
        neppy_core::neppy::memory::tree::retrieval::schemas::schemas("missing").function,
        "unknown"
    );
    assert!(schemas
        .iter()
    ...
    let source = neppy_core::neppy::memory::tree::retrieval::rpc::query_source_rpc(
        &config,
        neppy_core::neppy::memory::tree::retrieval::rpc::QuerySourceRequest {
    ...
    assert!(
        neppy_core::neppy::memory::tree::retrieval::rpc::query_source_rpc(
            &config,
            neppy_core::neppy::memory::tree::retrieval::rpc::QuerySourceRequest {
                source_id: None,
    ...
    let search = neppy_core::neppy::memory::tree::retrieval::rpc::search_entities_rpc(
        &config,
        neppy_core::neppy::memory::tree::retrieval::rpc::SearchEntitiesRequest {
            query: "alice".into(),
    ...
    assert!(
        neppy_core::neppy::memory::tree::retrieval::rpc::search_entities_rpc(
            &config,
            neppy_core::neppy::memory::tree::retrieval::rpc::SearchEntitiesRequest {
                query: "alice".into(),
    ...
    let drill = neppy_core::neppy::memory::tree::retrieval::rpc::drill_down_rpc(
        &config,
        neppy_core::neppy::memory::tree::retrieval::rpc::DrillDownRequest {
    ...
    let fetch = neppy_core::neppy::memory::tree::retrieval::rpc::fetch_leaves_rpc(
        &config,
        neppy_core::neppy::memory::tree::retrieval::rpc::FetchLeavesRequest {
...
async fn memory_query_backend_and_tree_flush_wrappers_cover_public_edges() {
    ...
    let no_stale =
        neppy_core::neppy::memory::tree::tree::flush::flush_stale_buffers_default(
            &config,
            &neppy_core::neppy::memory::tree::tree::LabelStrategy::Empty,
        )
        .await
        .expect("flush empty buffers");
    assert_eq!(no_stale, 0);
    let missing_flush = neppy_core::neppy::memory::tree::tree::flush::force_flush_tree(
        &config,
        "tree:missing",
        None,
        &neppy_core::neppy::memory::tree::tree::LabelStrategy::Empty,
...
async fn tree_summarizer_ops_cover_validation_query_and_local_provider_guards() {
    ...
    let empty_content =
        neppy_core::neppy::memory::tree::tree_runtime::ops::tree_summarizer_ingest(
    ...
    let status =
        neppy_core::neppy::memory::tree::tree_runtime::ops::tree_summarizer_status(
            &config, "ops_ns",
        )
        .await
        .expect("status");
    ...
    let query = neppy_core::neppy::memory::tree::tree_runtime::ops::tree_summarizer_query(
        &config, "ops_ns", None,
    )
    .await
    .expect("query root");
    ...
    let missing =
        neppy_core::neppy::memory::tree::tree_runtime::ops::tree_summarizer_query(
            &config,
            "ops_ns",
            Some("2026/05/29/17"),
        )
    ...
    let provider_guard =
        neppy_core::neppy::memory::tree::tree_runtime::ops::tree_summarizer_run(
            &config, "ops_ns",
        )
        .await
        .unwrap_err();
    ...
    let rebuild_guard =
        neppy_core::neppy::memory::tree::tree_runtime::ops::tree_summarizer_rebuild(
            &config, "ops_ns",
        )
        .await
        .unwrap_err();
...
fn welcome_migration_public_entrypoint_covers_empty_marker_and_transcript_paths() {
    ...
    let result = neppy_core::neppy::threads::migrate_welcome_agent_artifacts(workspace)
    ...
    let second = neppy_core::neppy::threads::migrate_welcome_agent_artifacts(workspace)
