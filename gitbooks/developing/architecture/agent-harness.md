...
> harness** via the adapter seam in [`src/neppy/agent/tinyagents/`](../../../src/neppy/agent/tinyagents/)
...
> through a `StopHookMiddleware` ([`tinyagents/stop_hooks.rs`](../../../src/neppy/agent/tinyagents/stop_hooks.rs))
...
> [`tinyagents/orchestration.rs`](../../../src/neppy/agent/tinyagents/orchestration.rs):
>
> - [`tinyagents/delegation.rs`](../../../src/neppy/agent/tinyagents/delegation.rs)
...
>   (`execute → complete | fail → done`, [`agent_teams/graph.rs`](../../../src/neppy/agent/orchestration/agent_teams/graph.rs));
...
Neppy requires `tinyagents = { version = "2.1", features = ["sqlite", "repl"] }`, patched to the vendored git submodule at `vendor/tinyagents` so SDK changes can be tested in-tree before being PR'd upstream (see [`Cargo.toml`](../../../Cargo.toml) — keep the submodule in lockstep with the version requirement). The rationale, so future upgrades don't silently regress it:

- **Native TinyAgents model interface, Neppy-owned product policy.** Every live route is an `Arc<dyn ChatModel<()>>`: TinyAgents OpenAI-compatible clients cover wire-equivalent managed, local, and BYOK routes, while host `ChatModel` implementations cover Claude SDK/Code and Codex-specific transports. Neppy still owns credential resolution, OAuth, access gates, endpoint selection, egress disclosure, billing metadata, and error classification.
- **`sqlite` feature enabled with one native sqlite chain.** Neppy's root and Tauri Cargo worlds pin `rusqlite = "=0.40.0"` and patch `rusqlite` / `libsqlite3-sys` locally to avoid the upstream `cfg_select!` build break on the current toolchain. Both worlds resolve to a single `libsqlite3-sys v0.38.0` chain. Durable graph checkpoints still run through [`SqlRunLedgerCheckpointer`](../../../src/neppy/agent/tinyagents/checkpoint.rs) until the migration re-points those rows to the crate checkpointer.
...
- **`repl` feature enabled for language workflows; `.rag` expressive language unused.** Neppy still drives _graphs_ from Rust (`GraphBuilder`), not the declarative `.rag` language. But the `repl` feature (the imperative Rhai `.ragsh` session runtime) is enabled to power the `rhai_workflows` language-workflow tool ([`neppy::flows::rhai`](../../../src/neppy/flows/rhai/README.md), see "Language workflows (Rhai)" below).
- **Ownership map:** model construction → `inference::provider::create_chat_model*`; crate SQLite checkpointer rows not yet adopted → `SqlRunLedgerCheckpointer`; generic detached executor state → `DetachedTaskRegistry`; controller-facing durability → Neppy SQL/JSON run ledgers (`running_subagents`, `workflow_runs`, `agent_teams`, `command_center`). The generic harness/graph/middleware/event primitives are used as-is.
...
Alongside identity/soul/profile/memory, the system prompt pulls in **AGENTS.md** instruction files — Neppy's analog of Claude Code's `CLAUDE.md` / Codex's `AGENTS.md`. Two layers are loaded **once**, at system-prompt build time (never re-read per turn, so the frozen-prefix / KV-cache contract holds):

- **Global** — `<workspace_dir>/AGENTS.md`, the user's Neppy workspace (where `SOUL.md` / `USER.md` live). Applies to every run.
...
The older `ToolDispatcher` trait (`src/neppy/agent/dispatcher.rs`) with its three dialects still exists, but as a **transcript-compatibility layer**, not a live routing choice:
...
Two directories under the agent's existing `action_dir` at runtime (the implementation lives in `src/neppy/agent/harness/artifact_offload/`):
...
1. **Loader-time (static).** [`agents::loader::validate_tier_hierarchy`](../../../src/neppy/agent/agents/loader.rs) runs over the merged registry (built-ins + workspace TOMLs) and refuses to boot a registry that lists a same-tier or worker-with-subagents entry. Built-in archetypes are checked at compile-test time; user-shipped TOMLs are checked at workspace load.
...
The domain lives in [`src/neppy/flows/rhai/`](../../../src/neppy/flows/rhai/README.md):
...
Every provider response carries a `UsageInfo` block - input tokens, output tokens, cached input tokens, and an authoritative `charged_amount_usd` populated by the Neppy backend. `TurnCost` sums those across every provider call inside one turn so the harness can:
...
The harness shell lives under `src/neppy/agent/`, with the tinyagents adapter seam in `src/neppy/agent/tinyagents/` and archetype definitions in `src/neppy/agent/registry/`. The README in `src/neppy/agent/` enumerates the public surface; the most load-bearing files (paths relative to `src/neppy/agent/` unless prefixed) are:
...
| `../tinyagents/middleware.rs`            | The named Neppy middleware stack (approval/security, tool policy, recovery, budgets, circuit breaker).    |
...
| `harness/engine/`                        | Surviving Neppy seams: `CheckpointStrategy`, `TurnProgress`.                                              |
...
> The bespoke `src/neppy/agent_graph/` engine, `GraphBlueprint`, and the
...
| `checkpoint/`    | `Checkpointer` trait (type-erased JSON state) → `InMemoryCheckpointer` (tests) + `SqliteCheckpointer` at `{workspace}/.neppy/agent_graph/checkpoints.db`. Durable pause/resume.                                                                                                                                                                                                                                                                                                                           |
...
Each agent folder under `src/neppy/agent/registry/agents/<name>/` (and the four agents that live in their own domains) now contains, alongside `agent.toml` + `prompt.rs`:
...
- **Detached sub-agents** (`running_subagents.rs`) use TinyAgents `DetachedTaskRegistry` for ownership-aware snapshots, wait/timeout, steering lookup, cooperative cancellation, hard abort, and terminal cleanup. Neppy retains durable task-store projection, product/session metadata, RPC and delivery semantics, and the `RunQueue` compatibility fallback.
...
- `AwaitingUser { question, options }`: the child called `ask_user_clarification`; a full checkpoint (history, question, options, overrides) is written to `{workspace}/.neppy/subagent_checkpoints/{task_id}.json`, and the run resumes from it when the user answers.
