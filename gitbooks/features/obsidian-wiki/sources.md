...
Every source is a single flat `MemorySourceEntry` (`src/neppy/memory/sources/types.rs`) whose `kind` discriminator (the `SourceKind` enum) decides which fields are required. Validation is enforced at add/update time by `validate()`, not the type system. The kinds:
...
Every kind implements one async trait, `SourceReader` (`src/neppy/memory/sources/readers/mod.rs`):
...
The mechanism lives in `src/neppy/memory/source_scope.rs`. Threading an allowlist through every memory tool and the deep `select_trees` retrieval layer would touch dozens of call sites. So, mirroring `thread_context`, the channel sets a `tokio::task_local!` around the agent turn and the retrieval layer reads it ambiently, with no explicit plumbing:
