---
description: >-
  Install Neppy, walk through the in-app onboarding (sign in, connect Gmail,
...
This page walks you through installing Neppy, going through the in-app onboarding, and running your first request.

Neppy is open source under the GNU GPL3 license. The codebase is at [github.com/tinyhumansai/openhuman](https://github.com/tinyhumansai/openhuman).
...
Neppy runs on **macOS, Windows and Linux** desktops. 4 GB+ RAM is recommended; 16 GB+ if you intend to ingest very large mailboxes or repos, or run a [local model](../features/model-routing/local-ai.md) on the same machine.

### Permissions

The first time you launch Neppy, the OS will prompt for the permissions the app needs (Accessibility on macOS, Input Monitoring for the voice hotkey). You can review and adjust these any time under **Settings**.

---

## 1. Download and install

Get the Neppy desktop app from [http://tinyhumans.ai/openhuman](http://tinyhumans.ai/openhuman) or via your platform's package manager. Open the app once it's installed.
...
{% hint style="info" %}
**No permanent lock-in.** Signing in does not grant Neppy ongoing access to anything. All third-party access requires explicit OAuth approval per integration in the steps below.
{% endhint %}

{% hint style="warning" %}
**Know what is local and what is managed.** Your Memory Tree database, Markdown vault, workspace config, and local runtime state live on your machine. The default setup still uses Neppy-hosted services for sign-in, model routing, managed integration OAuth/tool calls, and web search proxying. Use the custom setup paths if you want to bring your own model, search, or Composio credentials. Some hosted features and real-time integration triggers still require the managed backend.
{% endhint %}
...
Neppy picks the right model for each task automatically. See [Automatic Model Routing](../features/model-routing/).
...
- [**Native Voice**](../features/native-tools/voice.md) - push-to-talk dictation and TTS replies so you can talk to Neppy instead of typing.

## Join the community

Neppy is in early beta. Feedback and contributions make a real difference at this stage.
