...
| --- | --- | --- |
| `src/neppy/memory/` | ~20,700 (79 files) | Orchestration/policy layer: `Memory` trait, RPC ops/schemas, agent tools, ingest orchestrator, sync lifecycle, preferences, source-scope task-locals, redaction |
| `src/neppy/memory/store/` | ~32,800 | Storage: `UnifiedMemory`/`MemoryClient`, chunks DB, content store, vectors, KV, safety |
| `src/neppy/memory/tree/` | ~25,300 | Summary trees, retrieval, scoring, health |
| `src/neppy/memory/sync/` | ~30,700 | Live sync: Composio, MCP, OAuth, pollers |
...
- **Process glue**: `memory/global.rs` singleton + background queue worker; `memory/source_scope.rs` tokio task-locals; `memory/chat.rs` (LLM adapter over `neppy::inference`); embeddings provider wiring.
...
### The adapter seam: `src/neppy/memory/tinycortex/`

New sibling module mirroring `src/neppy/agent/tinyagents/`, holding every impl of a TinyCortex trait over an Neppy service:

- `embeddings.rs` — `impl tinycortex EmbeddingBackend` over `neppy::embeddings` (dim/model/signature from `Config`).
...
**W1 — Seam scaffolding.** Create `src/neppy/memory/tinycortex/` with the adapters in §1 (config, embeddings, chat/summariser, queue driver, sinks, bus bridge, facade). No behavior flips yet; adapters are constructed and unit-verified against the crate's inert defaults. *Deliverable: crate is linked, adapters compile, `MemoryConfig` derived from real `Config`.*
...
- `memory_store`, `memory_tree`, `memory_queue`, `memory_diff`, `memory_entities`, `memory_graph`, `memory_goals`, `memory_archivist`, engine parts of `memory_sources`/`memory_tools`/conversations deleted from `src/neppy/`; deletion ledger fully checked off.
- Host memory code = policy/surfaces only: `memory/` (RPC, tools, sync lifecycle, preferences, scope, redact, global) + `src/neppy/memory/tinycortex/` seam + `memory_sync/`.
...
- **(Amended)** `memory_sync/` engine deleted from the host per §8 (W-SYNC); host retains only
  schedulers, credentials/OAuth, event-bus bridges, and RPC wrappers. `src/neppy/inference/embeddings/`
...
| New Composio HTTP client (modeled on `src/neppy/integrations/composio/client.rs`, minus keychain) | `src/memory/sync/composio/client.rs` — **direct** (BYO key → backend.composio.dev) and **proxied** (base_url + bearer; Neppy-backend default) modes |
...
**`SyncEventSink`** trait in the crate, host adapter `src/neppy/memory/tinycortex/sync_sink.rs`
...
- **W-EMB.3 (host PR):** dual submodule bump; `src/neppy/memory/tinycortex/embeddings.rs` constructs
  tinyagents providers from `Config`; delete `src/neppy/inference/embeddings/` provider impls; keep
