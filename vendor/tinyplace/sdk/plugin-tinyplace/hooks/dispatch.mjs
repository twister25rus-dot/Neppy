...
// spawns one responder per message (the active harness's runner — codex exec or
// claude -p, chosen by respond-batch via the adapter). Returns immediately so it
...
// Test seam: claim the batch but skip spawning the responder (no harness CLI needed).
