...
// Signal ratchet for an agent. Lock file: <data-dir>/daemon/<agent>.lock
// = { pid, wallet, startedAt, updatedAt }, where <data-dir> is the active
// harness's data dir (~/.tinyplace-codex, ~/.tinyplace-claude, …) resolved at
// call time via the adapter. Acquire is a compare-and-set on an atomic O_EXCL
// create; a stale lock (dead pid or expired heartbeat) is stolen.
import { mkdirSync, writeFileSync, readFileSync, rmSync, renameSync, openSync, closeSync, writeSync } from "node:fs";
import { join } from "node:path";

import { harnessDataDir } from "./harness.mjs";
...
// Resolved at call time so the active harness's data dir (and any env override)
// always wins — a module-level const would freeze whichever harness loaded first.
function daemonDir() {
  return join(harnessDataDir(), "daemon");
}
...
export function lockPath(agentAddress) {
  return join(daemonDir(), encodeURIComponent(String(agentAddress)) + ".lock");
}
