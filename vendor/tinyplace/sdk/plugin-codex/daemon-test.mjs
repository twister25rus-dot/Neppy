...
process.env.TINYPLACE_CODEX_HOME = mkdtempSync(join(tmpdir(), "tinyplace-daemon-"));
const dataDir = process.env.TINYPLACE_CODEX_HOME;
...
function server(sessionId, extraEnv = {}) {
  const child = spawn("node", [serverPath], {
    ...
    env: { ...process.env, TINYPLACE_CODEX_HOME: dataDir, TINYPLACE_API_URL: DEAD_BACKEND, TINYPLACE_HARNESS_SESSION_ID: sessionId, ...extraEnv },
  });
  let buf = "";
...
const daemon = spawn("node", [daemonPath], {
  stdio: "ignore",
  env: { ...process.env, TINYPLACE_DAEMON_WALLET: "d1", TINYPLACE_CODEX_HOME: dataDir, TINYPLACE_API_URL: DEAD_BACKEND, TINYPLACE_DAEMON_IDLE_MS: "500", TINYPLACE_DAEMON_POLL_MS: "120", TINYPLACE_DAEMON_HEARTBEAT_MS: "120" },
});
let exited = false;
...
expect("session label is codex:1", used.label === "codex:1");

// send → an outbox job (the daemon would send it); nothing hits the relay here.
const sent = await thin.call("send", { to: "@peer", body: "hello peer", to_session: "codex:2" });
...
expect("outbox job carries to/text/fromSession/toSession", job.to === "@peer" && job.text === "hello peer" && job.fromSession === "codex:1" && job.toSession === "codex:2");
...
const inboxDir = routing.sessionInboxDir(AGENT, "codex:1");
mkdirSync(inboxDir, { recursive: true });
writeFileSync(join(inboxDir, "in1.json"), JSON.stringify({ id: "in1", from: "peerK", fromSession: "codex:3", role: "agent", text: "incoming!", inReplyTo: null, ts: new Date().toISOString() }));
...
expect("inbox surfaces fromSession/role", inbox.messages[0].fromSession === "codex:3" && inbox.messages[0].role === "agent");

// check_reply → correlates a routed reply to our sent id.
writeFileSync(join(inboxDir, "rep1.json"), JSON.stringify({ id: "rep1", from: "peerP", fromSession: "codex:2", role: "agent", text: "the answer", inReplyTo: sent.sent.id, ts: new Date().toISOString() }));
