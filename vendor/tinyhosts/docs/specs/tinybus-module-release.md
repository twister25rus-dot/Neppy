...
- The example module provides `ai.tinyhumans.tinyhosts.Hosting.Providers` at
  `/ai/tinyhumans/tinyhosts/Hosting`.
- Each release archive is named
  `tinyhosts-<version>-<platform>.<extension>` and contains only this
...
x86_64 archive through TinyBus's GitHub release API and calls `Providers` over an
in-memory bus.
