...
3. `src/neppy/modules/registry.rs:205-262` — version, `release_url`, all 11 archive names, all 11 sha256, **taken verbatim from the release's `checksum.toml`**, never recomputed locally
4. `src/neppy/modules/memory.rs:51` `ARTIFACT_CAPABILITIES_PIN` `"1.4.0"` → `"1.5.0"` — else `the_capability_list_matches_the_pinned_release` goes red
